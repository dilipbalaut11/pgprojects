...
$node_publisher->safe_psql('postgres',
	"CREATE TABLE tab1 (a int PRIMARY KEY, b int) USING leopard");
...
$node_twoways->append_conf('postgresql.conf', "shared_preload_libraries = 'leopard'");
$node_twoways->start;
$node_twoways->safe_psql('postgres', "CREATE EXTENSION leopard WITH SCHEMA public");
$node_twoways->stop;
$node_twoways->append_conf('postgresql.conf', "default_table_access_method = 'leopard'");
$node_twoways->start;
for my $db (qw(d1 d2))
{
	$node_twoways->safe_psql('postgres', "CREATE DATABASE $db");
$node_twoways->safe_psql("$db", "CREATE EXTENSION leopard WITH SCHEMA public");
	$node_twoways->safe_psql($db,        "CREATE TABLE t (f int) USING leopard");
	$node_twoways->safe_psql($db,        "CREATE TABLE t2 (f int) USING leopard");
}
...
$node_pub->append_conf('postgresql.conf', "shared_preload_libraries = 'leopard'");
$node_pub->start;
$node_pub->safe_psql('postgres', "CREATE EXTENSION leopard WITH SCHEMA public");
$node_pub->stop;
$node_pub->append_conf('postgresql.conf', "default_table_access_method = 'leopard'");
$node_pub->start;

my $node_pub_sub = get_new_node('testpublisher_subscriber');
$node_pub_sub->init(allows_streaming => 'logical');
$node_pub_sub->append_conf('postgresql.conf', "shared_preload_libraries = 'leopard'");
$node_pub_sub->start;
$node_pub_sub->safe_psql('postgres', "CREATE EXTENSION leopard WITH SCHEMA public");
$node_pub_sub->stop;
$node_pub_sub->append_conf('postgresql.conf', "default_table_access_method = 'leopard'");
$node_pub_sub->start;

my $node_sub = get_new_node('testsubscriber1');
$node_sub->init(allows_streaming => 'logical');
$node_sub->append_conf('postgresql.conf', "shared_preload_libraries = 'leopard'");
$node_sub->start;
$node_sub->safe_psql('postgres', "CREATE EXTENSION leopard WITH SCHEMA public");
$node_sub->stop;
$node_sub->append_conf('postgresql.conf', "default_table_access_method = 'leopard'");
$node_sub->start;

# Create the tables in all nodes.
$node_pub->safe_psql('postgres', "CREATE TABLE tab1 (a int) USING leopard");
$node_pub_sub->safe_psql('postgres', "CREATE TABLE tab1 (a int) USING leopard");
$node_sub->safe_psql('postgres', "CREATE TABLE tab1 (a int) USING leopard");
...
$node_publisher->safe_psql('postgres',
	"CREATE TABLE tab_replidentity_index(a int not null, b int not null) USING leopard");
$node_publisher->safe_psql('postgres',
