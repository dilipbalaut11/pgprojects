...
createdb(ParseState *pstate, const CreatedbStmt *stmt)
{
	...
	{
		/*
		 * If a user has asked to create a database with WAL then call
		 * CopyDatabaseWithWal, that will copy the database logically block
...
dbase_redo(XLogReaderState *record)
{
	...
	if (info == XLOG_DBASE_CREATE)
	{
		...
		if (!OidIsValid(xlrec->src_db_id))
		{
			...
			CreateDirAndVersionFile(dst_path, xlrec->db_id,
									xlrec->tablespace_id, true);
		}
		else
		{
			struct stat st;
