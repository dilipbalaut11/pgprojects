 ...
 * given indexoid/reloid.  If a valid global reloids list is given then only
 * invalidate the reloid entires which are related to the input global index
 * oids.
 */
void
InvalidateIndexPartitionEntries(List *reloids, Oid indexoid, Oid reloid)
{
	Relation	catalogRelation;
	SysScanDesc scan;
	ScanKeyData key;
	...
	while ((tuple = systable_getnext(scan)) != NULL)
	{
		...
		if (reloids != NULL && !list_member_oid(reloids, form->reloid))
			continue;
