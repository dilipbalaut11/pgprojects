...
/*
 * Add footer to subscription description.
 */
static void
addFooterToSubscriptionDesc(char *logdest, char *conflict_schema, Oid subid,
...
describeSubscriptions(const char *pattern)
{
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		addFooterToSubscriptionDesc(logdest, conflict_schema, subid, &cont);
