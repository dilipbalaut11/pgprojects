...
/*
 * Increase the usage frequency of the buffer.  If buffer is just got in pass
 * init as true.  And if that is set we will get start the frequency counter
 * with the higher initial value so that it will not immediately evicted out as
 * soon as it come in.
 */
static inline void
SlruAddLFUFreq(SlruShared shared, int slotno, bool init)
{
#define SLRU_LFU_INIT_COUNT		20

	if (init)
		pg_atomic_add_fetch_u64(&(shared->page_lfu_count[slotno]),
								SLRU_LFU_INIT_COUNT);
	else
		pg_atomic_add_fetch_u64(&(shared->page_lfu_count[slotno]), 1);
}

/*
 * Decay the usage frequency of the buffer.  This is called every time for each
 * buffer while finding the victim buffer.
 */
static inline void
SlruDecayLFUFreq(SlruShared shared, int slotno)
{
	while(1)
	{
		uint64	old = pg_atomic_read_u64(&shared->page_lfu_count[slotno]);
		uint64 	new = floor(old / 8);

		if (pg_atomic_compare_exchange_u64(&(shared->page_lfu_count[slotno]),
										   &old, new))
			break;
	}
}
...
static int SlruSelectVictimPage(SlruCtl ctl, int pageno);
...
SlruShmemStructSize(int nslots, int nlsns, int npartitions)
{
	...
	sz += MAXALIGN(nslots * sizeof(int64));	/* page_lfu_count[] */
...
/*
 * BufTableHashCode
 *		Compute the hash code associated with a pageno
 */
 ...
 *
 * Caller must hold at least share lock on buffer mapping hash for pageno's
 * partition
 */
static inline int
SlruBufTableLookup(SlruCtl ctl, int pageno, uint32 hashcode)
{
...
/*
 * SlruBufTableInsert
 *		Insert a hashtable entry for given pageno and buffer slotno,
 *		unless an entry already exists for that pageno
 *
 * Caller must hold exclusive lock on respective partition of the buffer
 * mapping hash
 */
static inline void
			  ...
			  SyncRequestHandler sync_handler)
{
	...
	if (npartitions > 0)
	{
		...
		appendStringInfoString(&buf, " buffer mapping hash");

		/* find or create SLRU buffer mapping hash table */
...
SimpleLruZeroPage(SlruCtl ctl, int pageno)
{
	...
	slotno = SlruSelectVictimPage(ctl, pageno);
	...
	if (ctl->buf_mapping != NULL)
		SlruAddLFUFreq(shared, slotno, true);
	else
				  ...
				  TransactionId xid)
{
	...
	for (;;)
	{
			...
			shared->page_status[slotno] != SLRU_PAGE_EMPTY)
		{
			...
			if (ctl->buf_mapping != NULL)
				SlruAddLFUFreq(shared, slotno, false);
			else
				SlruRecentlyUsed(shared, slotno);
							   ...
							   bool readonly)
{
		...
		shared->page_status[slotno] != SLRU_PAGE_READ_IN_PROGRESS)
	{
		...
		/* See comments for SlruAddLFUFreq macro */
		SlruAddLFUFreq(shared, slotno, false);
	...
	/*
	 * No luck, so find a victim slot and read page into it from disk.  Before
...
SimpleLruReadPage_ReadOnly(SlruCtl ctl, int pageno, TransactionId xid)
{
	...
	SimpleLruAcquireControlLock(ctl, LW_SHARED);
 ...
 */
static int
SlruSelectVictimPage(SlruCtl ctl, int pageno)
{
	SlruShared	shared = ctl->shared;
	...
	for (;;)
	{
		 ...
		 * the buffer pool.  As of now while using the buffer mapping hash
		 * table we are using the LFU policy for victim selection instead of
		 * LRU.
		 */
		if (ctl->buf_mapping != NULL)
		{
...
/*
 * Helper function for SlruSelectVictimPage to find the victim slot based on
...
SlruSelectLFUPage(SlruCtl ctl, int pageno)
{
	...
	/*
	 * If we find any EMPTY slot, just select that one. Else choose a
	 * victim page to replace.  We take the least frequently used valid page.
	 * We will select a slot that is already I/O busy only if there is no
	 * other choice.
	 */
	for (slotno = 0; slotno < shared->num_slots; slotno++)
	{
		...
		if (this_freq > 0)
			SlruDecayLFUFreq(shared, slotno);
	...
	/*
	 * If all pages are I/O busy, we'llhave to wait for an I/O to complete and
	 * then retry.  In that unhappy case, we choose to wait for the I/O on the
	 * least frequency used slot, on the assumption that it was likely
	 * initiated first of all the I/Os in progress and may therefore finish
	 * first.
	 */
