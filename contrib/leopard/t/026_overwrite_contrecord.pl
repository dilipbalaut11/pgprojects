...
$node->safe_psql('postgres', 'create table filler (a int, b text) USING leopard');
...
#$node->safe_psql('postgres', qq{create table foo () USING leopard});
...
$node->safe_psql('postgres',
	qq{create table foo (a text) USING leopard; insert into foo values ('hello')});
$node->safe_psql('postgres',
