			 ...
			 AlterTableUtilityContext *context)
{
	...
	ATRewriteTables(parsetree, &wqueue, lockmode, context, relid);
}
...
ATRewriteTables(AlterTableStmt *parsetree, List **wqueue, LOCKMODE lockmode,
				AlterTableUtilityContext *context, Oid parent_relid)
{
	ListCell   *ltab;
	...
	foreach(ltab, *wqueue)
	{
		...
		if (tab->rewrite > 0 && tab->relkind != RELKIND_SEQUENCE)
		{
			 ...
			 */
			ATRewriteTable(tab, OIDNewHeap, lockmode, parent_relid);
		...
		else
		{
			...
			if (tab->constraints != NIL || tab->verify_new_notnull ||
				tab->partition_constraint != NULL)
				ATRewriteTable(tab, InvalidOid, lockmode, parent_relid);
			   ...
			   Oid parent_relid)
{
	...
	if (tab->partition_constraint)
	{
		...
		/* relation must already be locked exclusively */
		rel = relation_open(parent_relid, NoLock);

		global_indexs = RelationGetGlobalIndexList(rel);

		/* close relation but don't release the lock. */
		relation_close(rel, NoLock);
	}
	...
	if (newrel || needscan)
	{
		...
		{
			int			len = list_length(global_indexs);
			ListCell   *l;
			int			i = 0;
			...
			/*
				...
				*/
			indexdescs = (RelationPtr) palloc(len * sizeof(Relation));
		...
		while (table_scan_getnextslot(scan, ForwardScanDirection, oldslot))
		{
			...
			if (global_indexs != NIL)
			{
				...
				/* need to get values from insertslot because index_insert accept the values */
				...
				for (index = 0; index < list_length(global_indexs); index++)
				{
					FormIndexDatum(indexInfoArray[index],
								   insertslot,
								   estate,
								   values,
								   isnull);

					index_insert(indexdescs[index],
								 values,
								 isnull,
								 rootTuple,
								 oldrel,
								 indexInfoArray[index]->ii_Unique ?
								 UNIQUE_CHECK_YES : UNIQUE_CHECK_NO,
								 false,
								 indexInfoArray[index]);
				}
			}
		...
		for (i = 0; i < list_length(global_indexs); i++)
