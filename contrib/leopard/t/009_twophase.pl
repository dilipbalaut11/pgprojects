...
$node_london->append_conf('postgresql.conf', "shared_preload_libraries = 'leopard'");
$node_london->start;
$node_london->safe_psql('postgres', "CREATE EXTENSION leopard WITH SCHEMA public");
$node_london->stop;
$node_london->append_conf('postgresql.conf', "default_table_access_method = 'leopard'");
...
$cur_primary->psql('postgres', "CREATE TABLE t_009_tbl (id int, msg text) USING leopard");
...
$cur_primary->psql(
	'postgres', "
	BEGIN;
	CREATE TABLE t_009_tbl2 (id int, msg text) USING leopard;
	SAVEPOINT s1;
...
$cur_primary->psql(
	'postgres', "
	BEGIN;
	CREATE TABLE t_009_tbl3 (id int, msg text) USING leopard;
	SAVEPOINT s1;
	INSERT INTO t_009_tbl3 VALUES (28, 'issued to ${cur_primary_name}');
	PREPARE TRANSACTION 'xact_009_14';
	BEGIN;
	CREATE TABLE t_009_tbl4 (id int, msg text) USING leopard;
	SAVEPOINT s1;
...
$cur_primary->psql(
	'postgres', "
	BEGIN;
	CREATE TABLE t_009_tbl5 (id int, msg text) USING leopard;
	SAVEPOINT s1;
	INSERT INTO t_009_tbl5 VALUES (30, 'issued to ${cur_primary_name}');
	PREPARE TRANSACTION 'xact_009_16';
	BEGIN;
	CREATE TABLE t_009_tbl6 (id int, msg text) USING leopard;
	SAVEPOINT s1;
