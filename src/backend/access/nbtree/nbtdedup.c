							   ...
							   TM_IndexDeleteOp *delstate)
{
	...
	for (int i = 0; i < state->nitems; i++)
	{
		 ...
		 * need to convert partitionID to reloid for every item because we are
		 * do not deduplicate across partitionID, i.e. all items in
		 * BTDedupState must belong to same partitionID.
		 */
