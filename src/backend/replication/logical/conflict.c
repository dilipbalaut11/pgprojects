					...
					TupleTableSlot *remoteslot, List *conflicttuples)
{
	...
	if (elevel >= ERROR)
	{
		if (log_dest_table)
		{
			 ...
			 * replication progress.  Then abort the failed apply transaction
			 * and insert the conflict tuple in a fresh transaction.
			 */
								 ...
								 List *conflicttuples)
{
	...
	foreach_ptr(ConflictTupleInfo, conflicttuple, conflicttuples)
	{
		...
		/* Set NULL if origin name for the tuple is InvalidReplOriginId or not found. */
		if (origin_name != NULL)
