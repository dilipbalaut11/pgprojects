				  ...
				  int options)
{
	...
	if (OidIsValid(vrel->oid))
	{
		...
		if (get_rel_has_globalindex(relid))
		{
		   ...
		   /*
			* FIXME: in case of manual vacuum we may get a lower level relation
			* here so we need to get the top level parent with global index.
			*/
		}
	}
	else
	 ...
	 */
	if (include_parts || has_globalindex)
	{
		...
		foreach(part_lc, part_oids)
		{
			...
			vacrels = lappend(vacrels, makeVacuumRelation(NULL,
														  part_oid,
														  vrel->va_cols,
														  has_globalindex ? relid : InvalidOid));
			MemoryContextSwitchTo(oldcontext);
		}
	}
