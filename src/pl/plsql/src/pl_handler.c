...
_PG_init(void)
{
	...
	DefineCustomEnumVariable("plsql.variable_conflict",
							 gettext_noop("Sets handling of conflicts between PL/pgSQL variable names and table column names."),
							 NULL,
							 &plsql_variable_conflict,
							 PLSQL_RESOLVE_ERROR,
							 variable_conflict_options,
							 PGC_SUSET, 0,
							 NULL, NULL, NULL);

	DefineCustomBoolVariable("plsql.print_strict_params",
							 ...
							 NULL,
							 &plsql_print_strict_params,
							 false,
							 PGC_USERSET, 0,
							 NULL, NULL, NULL);

	DefineCustomBoolVariable("plsql.check_asserts",
	...
	DefineCustomStringVariable("plsql.extra_warnings",
							   gettext_noop("List of programming constructs that should produce a warning."),
							   NULL,
							   &plsql_extra_warnings_string,
							   "none",
							   PGC_USERSET, GUC_LIST_INPUT,
							   plsql_extra_checks_check_hook,
							   plsql_extra_warnings_assign_hook,
							   NULL);

	DefineCustomStringVariable("plsql.extra_errors",
							   gettext_noop("List of programming constructs that should produce an error."),
							   NULL,
							   &plsql_extra_errors_string,
							   "none",
							   PGC_USERSET, GUC_LIST_INPUT,
							   plsql_extra_checks_check_hook,
							   plsql_extra_errors_assign_hook,
							   NULL);

	MarkGUCPrefixReserved("plsql");

	RegisterXactCallback(plsql_xact_cb, NULL);
	RegisterSubXactCallback(plsql_subxact_cb, NULL);

	/* Set up a rendezvous point with optional instrumentation plugin */
	plsql_plugin_ptr = (PLSQL_plugin **) find_rendezvous_variable("PLSQL_plugin");
 ...
 * ----------
 */
PG_FUNCTION_INFO_V1(plsql_call_handler);
...
plsql_call_handler(PG_FUNCTION_ARGS)
{
	...
	func = plsql_compile(fcinfo, false);
	...
	PG_TRY();
	{
		...
		if (CALLED_AS_TRIGGER(fcinfo))
			retval = PointerGetDatum(plsql_exec_trigger(func,
														  (TriggerData *) fcinfo->context));
		else if (CALLED_AS_EVENT_TRIGGER(fcinfo))
		{
			plsql_exec_event_trigger(func,
									   (EventTriggerData *) fcinfo->context);
			/* there's no return value in this case */
		}
		else
			retval = plsql_exec_function(func, fcinfo,
										   NULL, NULL,
										   procedure_resowner,
										   !nonatomic);
	}
...
/* ----------
 * plsql_inline_handler
 *
 * Called by PostgreSQL to execute an anonymous code block
 * ----------
 */
PG_FUNCTION_INFO_V1(plsql_inline_handler);
 ...
 * ----------
 */
PG_FUNCTION_INFO_V1(plsql_validator);
