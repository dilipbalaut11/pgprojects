...
static void PrintBlockRefTable(BlockRefTable *brtab);
/*
...
PrintBlockRefTable(BlockRefTable *brtab)
{
	...
	while ((entry = hash_seq_search(&seq_status)) != NULL)
	{
		...
		foreach(lc, entry->containers)
		{
			...
			appendStringInfo(&str, "container no=%d is array=%d nblocks=%d \n",
							 container->containerno, container->isarray,
							 container->nblocks);
			if (container->isarray)
			{
				int i;
				...
				for (i = 0; i < container->nblocks; i++)
					appendStringInfo(&str, "block=%d ", container->blkinfo.blkarray[i]);
			}
		}
