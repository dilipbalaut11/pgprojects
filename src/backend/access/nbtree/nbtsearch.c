			 ...
			 bool firstPage)
{
	...
	if (ScanDirectionIsForward(dir))
	{
		...
		while (offnum <= maxoff)
		{
			...
			/*
			 * For global index we also need to fetch the partitioned relation
			 * Id in order to know from which relation we need to fetch tuple.
			 * We might get an InvalidOid if the partition is already detcahed
			 * so ignore such tuple.
			 */
			if (RelationIsGlobalIndex(scan->indexRelation))
			{
				heapOid = index_tuple_fetch_partrelid(scan->indexRelation, itup);
