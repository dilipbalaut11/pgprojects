...
static inline void
SlruLFUCounterDecay(SlruShared shared, int slotno, uint64 old)
{
...
SlruSelectLFUPage(SlruCtl ctl, int pageno)
{
	 ...
	 * then retry.
	 *
	 * XXX in case of LFU which page we should use for I/O wait, because unlike
	 * least recently used case here we can not assumer that on least
	 * frequenctly slot I/O would have started first.
	 */
