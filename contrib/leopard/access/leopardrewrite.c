 ...
 *
 * These functions provide a facility to completely rewrite a leopard, while
 * preserving visibility information and update chains.
 *
 * INTERFACE
 *
 * The caller is responsible for creating the new leopard, all catalog
 * changes, supplying the tuples to be written to the new leopard, and
 ...
 * to the new leopard.
 *
 * If we encounter B first, we can insert B to the new leopard right away.
 * We then add an entry to the old_new_tid_map hash table showing B's
 * original tid (in the old leopard) and new tid (in the new leopard).
 ...
 * entirely DEAD according to LeopardTupleSatisfiesVacuum, because the test
 ...
 *
 * We can't use the normal leopard_insert function to insert into the new
 * leopard, because leopard_insert overwrites the visibility information.
 * We use a special-purpose raw_leopard_insert function instead, which
 * is optimized for bulk inserting a lot of tuples, knowing that we have
 * exclusive access to the leopard.  raw_leopard_insert builds new pages in
 ...
 * leopard's TOAST table will go through the normal bufmgr.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/leopard/leopardrewrite.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
{
	Relation	rs_old_rel;		/* source leopard */
	Relation	rs_new_rel;		/* destination leopard */
...
typedef struct
{
	...
	ItemPointerData tid;		/* tuple location in old leopard */
} TidHashKey;
...
typedef struct
{
	...
	ItemPointerData old_tid;	/* A's location in the old leopard */
	LeopardTuple	tuple;			/* A's tuple contents */
} UnresolvedTupData;
...
typedef struct
{
	...
	ItemPointerData new_tid;	/* where we put it in the new leopard */
} OldToNewMappingData;
...
static void raw_leopard_insert(RewriteState state, LeopardTuple tup);

/* internal logical remapping prototypes */
static void logical_begin_leopard_rewrite(RewriteState state);
static void logical_rewrite_leopard_tuple(RewriteState state, ItemPointerData old_tid, LeopardTuple new_tuple);
static void logical_end_leopard_rewrite(RewriteState state);


/*
 * Begin a rewrite of a table
 *
 * old_leopard		old, locked leopard relation tuples will be read from
 * new_leopard		new, locked leopard relation to insert tuples to
 ...
 */
RewriteState
begin_leopard_rewrite(Relation old_leopard, Relation new_leopard, TransactionId oldest_xmin,
				   TransactionId freeze_xid, MultiXactId cutoff_multi)
{
	...
	state->rs_old_rel = old_leopard;
	state->rs_new_rel = new_leopard;
	state->rs_buffer = (Page) palloc(BLCKSZ);
	/* new_leopard needn't be empty, just locked */
	state->rs_blockno = RelationGetNumberOfBlocks(new_leopard);
	...
	logical_begin_leopard_rewrite(state);
 ...
 */
void
end_leopard_rewrite(RewriteState state)
{
	...
	while ((unresolved = hash_seq_search(&seq_status)) != NULL)
	{
		...
		raw_leopard_insert(state, unresolved->tuple);
	}
	 ...
	 * occurring during the leopardrewrite operation won't have fsync'd data we
	...
	logical_end_leopard_rewrite(state);
...
/*
 * Add a tuple to the new leopard.
 *
 ...
 *
 * state		opaque state as returned by begin_leopard_rewrite
 * old_tuple	original tuple in the old leopard
 * new_tuple	new, rewritten tuple to be inserted to new leopard
 */
void
rewrite_leopard_tuple(RewriteState state,
				   LeopardTuple old_tuple, LeopardTuple new_tuple)
{
	MemoryContext old_cxt;
	 ...
	 */
	memcpy(&new_tuple->t_data->t_choice.t_leopard,
		   &old_tuple->t_data->t_choice.t_leopard,
		   sizeof(LeopardTupleFields));

	new_tuple->t_data->t_infomask &= ~LEOPARD_XACT_MASK;
	new_tuple->t_data->t_infomask2 &= ~LEOPARD2_XACT_MASK;
	new_tuple->t_data->t_infomask |=
		old_tuple->t_data->t_infomask & LEOPARD_XACT_MASK;

	/*
	 * While we have our hands on the tuple, we may as well freeze any
	 * eligible xmin or xmax, so that future VACUUM effort can be saved.
	 */
	leopard_freeze_tuple(new_tuple->t_data,
	 ...
	 */
	if (!((old_tuple->t_data->t_infomask & LEOPARD_XMAX_INVALID) ||
		  LeopardTupleHeaderIsOnlyLocked(old_tuple->t_data)) &&
		!LeopardTupleHeaderIndicatesMovedPartitions(old_tuple->t_data) &&
							...
							&(old_tuple->t_data->t_ctid))))
	{
		...
		hashkey.xmin = LeopardTupleHeaderGetUpdateXid(old_tuple->t_data);
		...
		else
		{
			...
			unresolved->tuple = leopard_copytuple(new_tuple);
	...
	for (;;)
	{
		...
		/* Insert the tuple and find out where it's put in new_leopard */
		raw_leopard_insert(state, new_tuple);
		new_tid = new_tuple->t_self;

		logical_rewrite_leopard_tuple(state, old_tid, new_tuple);
		 ...
		 */
		if ((new_tuple->t_data->t_infomask & LEOPARD_UPDATED) &&
			!TransactionIdPrecedes(LeopardTupleHeaderGetXmin(new_tuple->t_data),
								   state->rs_oldest_xmin))
		{
			...
			memset(&hashkey, 0, sizeof(hashkey));
			hashkey.xmin = LeopardTupleHeaderGetXmin(new_tuple->t_data);
			hashkey.tid = old_tid;
			...
			if (unresolved != NULL)
			{
				 ...
				 * to, fix its t_ctid and insert it to the new leopard.
				 */
				if (free_new)
					leopard_freetuple(new_tuple);
 ...
 */
bool
rewrite_leopard_dead_tuple(RewriteState state, LeopardTuple old_tuple)
{
	/*
	 ...
	 * LeopardTupleSatisfiesVacuum just wasn't enough to detect it. It happens
	...
	memset(&hashkey, 0, sizeof(hashkey));
	hashkey.xmin = LeopardTupleHeaderGetXmin(old_tuple->t_data);
	...
	if (unresolved != NULL)
	{
		...
		leopard_freetuple(unresolved->tuple);
		hash_search(state->rs_unresolved_tups, &hashkey,
					HASH_REMOVE, &found);
		Assert(found);
		return true;
...
/*
 * Insert a tuple to the new relation.  This has to track leopard_insert
 ...
 */
static void
raw_leopard_insert(RewriteState state, LeopardTuple tup)
{
	...
	{
		int			options = LEOPARD_INSERT_SKIP_FSM;

		/*
		 * While rewriting the leopard for VACUUM FULL / CLUSTER, make sure data
		 * for the TOAST table are not logically decoded.  The main leopard is
		 * WAL-logged as XLOG FPI records, which are not logically decoded.
		 */
		options |= LEOPARD_INSERT_NO_LOGICAL;

		leopardtup = leopard_toast_insert_or_update(state->rs_new_rel, tup, NULL,
											  options);
	}
	else
		leopardtup = tup;

	len = MAXALIGN(leopardtup->t_len); /* be conservative */

	/*
	 * If we're gonna fail for oversize tuple, do it right away
	 */
	if (len > MaxLeopardTupleSize)
		ereport(ERROR,
				(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
				 errmsg("row is too big: size %zu, maximum size %zu",
						len, MaxLeopardTupleSize)));
	...
	if (state->rs_buffer_valid)
	{
		...
		if (len + saveFreeSpace > pageFreeSpace)
		{
			 ...
			 * ourselves in end_leopard_rewrite.
			 */
			RelationOpenSmgr(state->rs_new_rel);
	...
	newoff = PageAddItem(page, (Item) leopardtup->t_data, leopardtup->t_len,
	...
	{
		ItemId		newitemid;
		LeopardTupleHeader onpage_tup;

		newitemid = PageGetItemId(page, newoff);
		onpage_tup = (LeopardTupleHeader) PageGetItem(page, newitemid);

		onpage_tup->t_ctid = tup->t_self;
	}

	/* If leopardtup is a private copy, release it. */
	if (leopardtup != tup)
 ...
 * tuples, via xl_leopard_new_cid records - leopard rewrites have to log enough
 * information to allow the decoding backend to updates its internal mapping
 * of (relfilenode,ctid) => (cmin, cmax) to be correct for the rewritten leopard.
 *
 ...
 * in memory or when end_leopard_rewrite() has been called.
 *
 ...
 * the modified leopard are on either side of the checkpoint. But since the
 ...
 * CheckPointLogicalRewriteLeopard()) has flushed the (partial) mapping file to
 ...
 * xl_leopard_rewrite_mapping records and truncating the mapping file to it
 ...
 */
static void
logical_begin_leopard_rewrite(RewriteState state)
{
	HASHCTL		hash_ctl;
 ...
 */
static void
logical_leopard_rewrite_flush_mappings(RewriteState state)
{
	HASH_SEQ_STATUS seq_status;
	...
	while ((src = (RewriteMappingFile *) hash_seq_search(&seq_status)) != NULL)
	{
		char	   *waldata;
		char	   *waldata_start;
		xl_leopard_rewrite_mapping xlrec;
		Oid			dboid;
		uint32		len;
		int			written;
		...
		/* write xlog record */
		XLogInsert(RM_LEOPARD2_ID, XLOG_LEOPARD2_REWRITE);
...
/*
 * Logical remapping part of end_leopard_rewrite().
 */
static void
logical_end_leopard_rewrite(RewriteState state)
{
	HASH_SEQ_STATUS seq_status;
	RewriteMappingFile *src;
	...
	if (state->rs_num_rewrite_mappings > 0)
		logical_leopard_rewrite_flush_mappings(state);
 ...
 * new_tuple->t_self by rewrite_leopard_tuple() if necessary for the tuple.
 */
static void
logical_rewrite_leopard_tuple(RewriteState state, ItemPointerData old_tid,
						   LeopardTuple new_tuple)
{
	...
	xmin = LeopardTupleHeaderGetXmin(new_tuple->t_data);
	/* use *GetUpdateXid to correctly deal with multixacts */
	xmax = LeopardTupleHeaderGetUpdateXid(new_tuple->t_data);
	...
	else if (LEOPARD_XMAX_IS_LOCKED_ONLY(new_tuple->t_data->t_infomask))
	{
...
/*
 * Replay XLOG_LEOPARD2_REWRITE records
 */
void
leopard_xlog_logical_rewrite(XLogReaderState *r)
{
	char		path[MAXPGPATH];
	int			fd;
	xl_leopard_rewrite_mapping *xlrec;
	...
	xlrec = (xl_leopard_rewrite_mapping *) XLogRecGetData(r);
 ...
 */
void
CheckPointLogicalRewriteLeopard(void)
{
	XLogRecPtr	cutoff;
	XLogRecPtr	redo;
	DIR		   *mappings_dir;
