					...
					TupleTableSlot *remoteslot, List *conflicttuples)
{
	...
	/*
	 * Get the conflict log destination and relation descriptor for conflict
	 * log table if there is one.
	 */
	conflictlogrel = GetConflictLogDestAndTable(&dest);

	log_to_table = ((dest & CONFLICT_LOG_DEST_TABLE) != 0);
	log_to_logfile = ((dest & CONFLICT_LOG_DEST_LOG) != 0);
...
/*
 * Helper function to extract the "raw" index key Datums(values[]) and their
 * null flags(isnull[]) from a TupleTableSlot, given an already open index
 * descriptor.
 */
static void
