 ...
 */
void
DropRelFileLocatorsAllBuffers(SMgrRelation *smgr_reln, int nnodes)
{
	int			i;
	int			j;
	int			n = 0;
	...
	pfree(block);
	nodes = palloc(sizeof(RelFileLocator) * n);	/* non-local relations */
	for (i = 0; i < n; i++)
		nodes[i] = rels[i]->smgr_rlocator.locator;
	...
	if (use_bsearch)
		pg_qsort(nodes, n, sizeof(RelFileLocator), rnode_comparator);
	...
	for (i = 0; i < NBuffers; i++)
	{
		...
		if (!use_bsearch)
		{
			...
			for (j = 0; j < n; j++)
			{
				if (RelFileLocatorEquals(bufHdr->tag.rlocator, nodes[j]))
				{
					rlocator = &nodes[j];
					break;
				}
...
buffertag_comparator(const BufferTag *ba, const BufferTag *bb)
{
	...
	ret = rnode_comparator(&ba->rlocator, &bb->rlocator);
