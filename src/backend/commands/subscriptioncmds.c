							 ...
							 Oid relid)
{
	...
	if (get_rel_relkind(relid) != RELKIND_RELATION)
		ereport(ERROR,
				...
				errmsg("cannot use relation \"%s\" for storing conflict log",
						conflictrel),
	...
	if (!ConditionalLockRelationOid(relid, RowExclusiveLock))
		ereport(ERROR,
				...
				errmsg("can not use table \"%s\" for storing conflict log because it was being locked",
						conflictrel));
	...
	if (!DatumGetBool(value))
		ereport(ERROR,
				...
				errhint("Ensure current user have enough privilege on \"%s\" for conflict log history table",
						conflictrel));
	...
	if (attcnt != MAX_CONFLICT_ATTR_NUM || !tbl_ok)
		ereport(ERROR,
				errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("table \"%s\" cannot be used for storing conflict log",
						conflictrel),
				errdetail("Table \"%s\" data definition is not suitable for storing conflict log",
							conflictrel));
