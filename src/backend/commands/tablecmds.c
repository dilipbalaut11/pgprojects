...
ATSimplePermissions(AlterTableType cmdtype, Relation rel, int allowed_targets)
{
	 ...
	 * skips these tables, so a conflict log table does not normally reach here;
...
AlterTableMoveAll(AlterTableMoveAllStmt *stmt)
{
	...
	while ((tuple = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		 ...
		 * commands directly.
		 *
		 * Also, explicitly avoid any shared tables, temp tables, TOAST
		 * (TOAST will be moved with the main table).
		 *
		 * Conflict log tables are system-managed for logical replication and
		 * cannot be altered directly, so skip them as well; otherwise a single
		 * such table in the source tablespace would abort the whole bulk move.
		 */
