 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/leopard/leopardtoast.c
 *
 *
 * INTERFACE ROUTINES
 ...
 */
void
leopard_toast_delete(Relation rel, LeopardTuple oldtup, bool is_speculative)
{
	TupleDesc	tupleDesc;
	Datum		toast_values[MaxLeopardAttributeNumber];
	bool		toast_isnull[MaxLeopardAttributeNumber];
	 ...
	 * leopard_getattr() only the varlena columns.  The latter could win if there
	...
	Assert(tupleDesc->natts <= MaxLeopardAttributeNumber);
	leopard_deform_tuple(oldtup, tupleDesc, toast_values, toast_isnull);
 ...
 *	options: options to be passed to leopard_insert() for toast rows
 * Result:
 ...
 */
LeopardTuple
leopard_toast_insert_or_update(Relation rel, LeopardTuple newtup, LeopardTuple oldtup,
							int options)
{
	LeopardTuple	result_tuple;
	TupleDesc	tupleDesc;
	int			numAttrs;
	...
	bool		toast_isnull[MaxLeopardAttributeNumber];
	bool		toast_oldisnull[MaxLeopardAttributeNumber];
	Datum		toast_values[MaxLeopardAttributeNumber];
	Datum		toast_oldvalues[MaxLeopardAttributeNumber];
	ToastAttrInfo toast_attr[MaxLeopardAttributeNumber];
	...
	Assert(numAttrs <= MaxLeopardAttributeNumber);
	leopard_deform_tuple(newtup, tupleDesc, toast_values, toast_isnull);
	if (oldtup != NULL)
		leopard_deform_tuple(oldtup, tupleDesc, toast_oldvalues, toast_oldisnull);
	...
	/* compute header overhead --- this should match leopard_form_tuple() */
	hoff = SizeofLeopardTupleHeader;
	 ...
	 * external.
	 */
	while (leopard_compute_data_size(tupleDesc,
	...
	/*
	 * In the case we toasted any values, we need to build a new leopard tuple
	 * with the changed values.
	 */
	if ((ttc.ttc_flags & TOAST_NEEDS_CHANGE) != 0)
	{
		LeopardTupleHeader olddata = newtup->t_data;
		LeopardTupleHeader new_data;
		 ...
		 */
		new_header_len = SizeofLeopardTupleHeader;
		if ((ttc.ttc_flags & TOAST_HAS_NULLS) != 0)
			new_header_len += BITMAPLEN(numAttrs);
		new_header_len = MAXALIGN(new_header_len);
		new_data_len = leopard_compute_data_size(tupleDesc,
											  toast_values, toast_isnull);
		new_tuple_len = new_header_len + new_data_len;

		/*
		 * Allocate and zero the space needed, and fill LeopardTupleData fields.
		 */
		result_tuple = (LeopardTuple) palloc0(LEOPARDTUPLESIZE + new_tuple_len);
		result_tuple->t_len = new_tuple_len;
		result_tuple->t_self = newtup->t_self;
		result_tuple->t_tableOid = newtup->t_tableOid;
		new_data = (LeopardTupleHeader) ((char *) result_tuple + LEOPARDTUPLESIZE);
		result_tuple->t_data = new_data;

		/*
		 * Copy the existing tuple header, but adjust natts and t_hoff.
		 */
		memcpy(new_data, olddata, SizeofLeopardTupleHeader);
		LeopardTupleHeaderSetNatts(new_data, numAttrs);
		new_data->t_hoff = new_header_len;

		/* Copy over the data, and fill the null bitmap if needed */
		leopard_fill_tuple(tupleDesc,
						toast_values,
						toast_isnull,
 ...
 *
 *	Note: we expect the caller already checked LeopardTupleHasExternal(tup),
 *	so there is no need for a short-circuit path.
 * ----------
 */
LeopardTuple
toast_flatten_tuple(LeopardTuple tup, TupleDesc tupleDesc)
{
	...
	leopard_deform_tuple(tup, tupleDesc, toast_values, toast_isnull);
	 ...
	 */
	new_tuple = leopard_form_tuple(tupleDesc, toast_values, toast_isnull);
 ...
 *
 *	However, we don't want to say that tuples represented as LeopardTuples
 *	can't contain toasted fields, so instead this routine should be called
 *	when such a LeopardTuple is being converted into a Datum.
 *
 ...
 *	format anyway within leopard_fill_tuple.
 * ----------
 */
Datum
toast_flatten_tuple_to_datum(LeopardTupleHeader tup,
							 uint32 tup_len,
							 TupleDesc tupleDesc)
{
	LeopardTupleHeader new_data;
	...
	/* Build a temporary LeopardTuple control structure */
	 ...
	 */
	Assert(numAttrs <= MaxTupleAttributeNumber);
	leopard_deform_tuple(&tmptup, tupleDesc, toast_values, toast_isnull);
	...
	new_data = (LeopardTupleHeader) palloc0(new_tuple_len);

	/*
	 * Copy the existing tuple header, but adjust natts and t_hoff.
	 */
	memcpy(new_data, tup, SizeofLeopardTupleHeader);
	LeopardTupleHeaderSetNatts(new_data, numAttrs);
	new_data->t_hoff = new_header_len;

	/* Set the composite-Datum header fields correctly */
	LeopardTupleHeaderSetDatumLength(new_data, new_tuple_len);
	LeopardTupleHeaderSetTypeId(new_data, tupleDesc->tdtypeid);
	LeopardTupleHeaderSetTypMod(new_data, tupleDesc->tdtypmod);
 ...
 *	(This does not eliminate compressed or short-header datums.)
 *
 *	This is essentially just like leopard_form_tuple, except that it will
							...
							bool *isnull)
{
	...
	/*
	 * We can pass the caller's isnull array directly to leopard_form_tuple, but
	 ...
	 * Form the reconfigured tuple.
	 */
	new_tuple = leopard_form_tuple(tupleDesc, new_values, isnull);
...
/*
 * Fetch a TOAST slice from a leopard table.
 *
 ...
 */
void
leopard_fetch_toast_slice(Relation toastrel, Oid valueid, int32 attrsize,
					   ...
					   struct varlena *result)
{
	...
	while ((ttup = systable_getnext_ordered_leopardtup(toastscan, ForwardScanDirection)) != NULL)
	{
		int32		curchunk;
		Pointer		chunk;
		bool		isnull;
		 ...
		 */
		curchunk = DatumGetInt32(leopardfastgetattr(ttup, 2, toasttupDesc, &isnull));
		Assert(!isnull);
		chunk = DatumGetPointer(leopardfastgetattr(ttup, 3, toasttupDesc, &isnull));
		Assert(!isnull);
