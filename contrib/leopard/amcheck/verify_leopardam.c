 ...
 *
 * verify_leopardam.c
 *	  Functions to check postgresql leopard relations for corruption
 *
 ...
 *
 *	  contrib/amcheck/verify_leopardam.c
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
PG_FUNCTION_INFO_V1(verify_leopardam);

/* The number of columns in tuples returned by verify_leopardam */
#define LEOPARDCHECK_RELATION_COLS 4
 ...
 * a lifetime of a verify_leopardam execution.
 */
typedef struct LeopardCheckContext
{
	/*
	...
	/* Values concerning the leopard relation being checked */
	Relation	rel;
	...
	/* Whether verify_leopardam has yet encountered any corrupt tuples */
	bool		is_corrupt;

	/* The descriptor and tuplestore for verify_leopardam's result tuples */
	TupleDesc	tupdesc;
...
static void check_tuple(LeopardCheckContext *ctx);
static void check_toast_tuple(LeopardTuple toasttup, LeopardCheckContext *ctx,
							  ToastedAttribute *ta, int32 *expected_chunk_seq,
							  uint32 extsize);

static bool check_tuple_attribute(LeopardCheckContext *ctx);
static void check_toasted_attribute(LeopardCheckContext *ctx,
									ToastedAttribute *ta);

static bool check_tuple_header(LeopardCheckContext *ctx);
static bool check_tuple_visibility(LeopardCheckContext *ctx);

static void report_corruption(LeopardCheckContext *ctx, char *msg);
static void report_toast_corruption(LeopardCheckContext *ctx,
...
static FullTransactionId FullTransactionIdFromXidAndCtx(TransactionId xid,
														const LeopardCheckContext *ctx);
static void update_cached_xid_range(LeopardCheckContext *ctx);
static void update_cached_mxid_range(LeopardCheckContext *ctx);
static XidBoundsViolation check_mxid_in_range(MultiXactId mxid,
											  LeopardCheckContext *ctx);
...
/*
 * Scan and report corruption in leopard pages, optionally reconciling toasted
 * attributes with entries in the associated toast table.  Intended to be
 * called from SQL with the following parameters:
 *
 *   relation:
 *     The Oid of the leopard relation to be checked.
 *
 *   on_error_stop:
 ...
 *
 *   skip:
 *     What kinds of pages in the leopard relation should be skipped.  Valid
 *     options are "all-visible", "all-frozen", and "none".
 *
 * Returns to the SQL caller a set of tuples, each containing the location
 * and a description of a corruption found in the leopard.
 *
 ...
 */
Datum
verify_leopardam(PG_FUNCTION_ARGS)
{
	...
	memset(&ctx, 0, sizeof(LeopardCheckContext));
	...
	for (ctx.blkno = first_block; ctx.blkno <= last_block; ctx.blkno++)
	{
		...
		if (skip_option != SKIP_PAGES_NONE)
		{
			...
			mapbits = (int32) leopard_vismap_get_status(ctx.rel, ctx.blkno,
													   &vmbuffer);
			if (skip_option == SKIP_PAGES_ALL_FROZEN)
			{
				if ((mapbits & LEOPARDVISMAP_ALL_FROZEN) != 0)
					continue;
			}

			if (skip_option == SKIP_PAGES_ALL_VISIBLE)
			{
				if ((mapbits & LEOPARDVISMAP_ALL_VISIBLE) != 0)
					continue;
			}
		}
			 ...
			 ctx.offnum = OffsetNumberNext(ctx.offnum))
		{
			...
			if (ctx.lp_len < MAXALIGN(SizeofLeopardTupleHeader))
			{
				report_corruption(&ctx,
								  psprintf("line pointer length %u is less than the minimum tuple header size %u",
										   ctx.lp_len,
										   (unsigned) MAXALIGN(SizeofLeopardTupleHeader)));
				continue;
			}
			...
			ctx.tuphdr = (LeopardTupleHeader) PageGetItem(ctx.page, ctx.itemid);
			ctx.natts = LeopardTupleHeaderGetNatts(ctx.tuphdr);
...
sanity_check_relation(Relation rel)
{
	...
	if (rel->rd_rel->relam != LEOPARD_TABLE_AM_OID)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("only leopard AM is supported")));
}
...
{
	Datum		values[LEOPARDCHECK_RELATION_COLS];
	bool		nulls[LEOPARDCHECK_RELATION_COLS];
	LeopardTuple	tuple;
	...
	tuple = leopard_form_tuple(tupdesc, values, nulls);
 ...
 */
static void
report_corruption(LeopardCheckContext *ctx, char *msg)
{
 ...
 */
static void
report_toast_corruption(LeopardCheckContext *ctx, ToastedAttribute *ta,
						char *msg)
{
...
verify_heapam_tupdesc(void)
{
	...
	tupdesc = CreateTemplateTupleDesc(LEOPARDCHECK_RELATION_COLS);
	...
	Assert(a == LEOPARDCHECK_RELATION_COLS);
 ...
 */
static bool
check_tuple_header(LeopardCheckContext *ctx)
{
	LeopardTupleHeader tuphdr = ctx->tuphdr;
	...
	if ((ctx->tuphdr->t_infomask & LEOPARD_XMAX_COMMITTED) &&
		(ctx->tuphdr->t_infomask & LEOPARD_XMAX_IS_MULTI))
	{
		report_corruption(ctx,
	...
	if (infomask & LEOPARD_HASNULL)
		expected_hoff = MAXALIGN(SizeofLeopardTupleHeader + BITMAPLEN(ctx->natts));
	else
		expected_hoff = MAXALIGN(SizeofLeopardTupleHeader);
	if (ctx->tuphdr->t_hoff != expected_hoff)
	{
		if ((infomask & LEOPARD_HASNULL) && ctx->natts == 1)
			report_corruption(ctx,
		...
		else if ((infomask & LEOPARD_HASNULL))
			report_corruption(ctx,
 ...
 */
static bool
check_tuple_visibility(LeopardCheckContext *ctx)
{
	TransactionId xmin;
	TransactionId xvac;
	TransactionId xmax;
	...
	xmin = LeopardTupleHeaderGetXmin(tuphdr);
	 ...
	 */
	if (!LeopardTupleHeaderXminCommitted(tuphdr))
	{
		if (LeopardTupleHeaderXminInvalid(tuphdr))
			return false;		/* inserter aborted, don't check */
		/* Used by pre-9.0 binary upgrades */
		else if (tuphdr->t_infomask & LEOPARD_MOVED_OFF)
		{
			xvac = LeopardTupleHeaderGetXvac(tuphdr);
		...
		/* Used by pre-9.0 binary upgrades */
		else if (tuphdr->t_infomask & LEOPARD_MOVED_IN)
		{
			xvac = LeopardTupleHeaderGetXvac(tuphdr);
	...
	if (tuphdr->t_infomask & LEOPARD_XMAX_IS_MULTI)
	{
		/*
		 * xmax is a multixact, so sanity-check the MXID. Note that we do this
		 * prior to checking for LEOPARD_XMAX_INVALID or
		 * LEOPARD_XMAX_IS_LOCKED_ONLY. This might therefore complain about
		 ...
		 * clear that flag anyway if LEOPARD_XMAX_INVALID is set or if
		 * LEOPARD_XMAX_IS_LOCKED_ONLY is true, but for now we err on the side of
		 * avoiding possibly-bogus complaints about missing TOAST entries.
		 */
		xmax = LeopardTupleHeaderGetRawXmax(tuphdr);
	...
	if (tuphdr->t_infomask & LEOPARD_XMAX_INVALID)
	{
		/*
	...
	if (LEOPARD_XMAX_IS_LOCKED_ONLY(tuphdr->t_infomask))
	{
		/*
	...
	if (tuphdr->t_infomask & LEOPARD_XMAX_IS_MULTI)
	{
		 ...
		 */
		xmax = LeopardTupleGetUpdateXid(tuphdr);
 ...
 *
 * This is not equivalent to running verify_leopardam on the toast table itself,
 ...
 */
static void
check_toast_tuple(LeopardTuple toasttup, LeopardCheckContext *ctx,
				  ToastedAttribute *ta, int32 *expected_chunk_seq,
				  uint32 extsize)
{
	int32		chunk_seq;
	...
	chunk_seq = DatumGetInt32(leopardfastgetattr(toasttup, 2,
	...
	*expected_chunk_seq = chunk_seq + 1;
	...
	chunk = DatumGetPointer(leopardfastgetattr(toasttup, 3,
										ctx->toast_rel->rd_att, &isnull));
	if (isnull)
	{
		report_toast_corruption(ctx, ta,
	...
	{
		/*
		 * could happen due to leopard_form_tuple doing its thing
		 */
 ...
 *
 * This function follows the logic performed by leopard_deform_tuple(), and in the
 ...
 */
static bool
check_tuple_attribute(LeopardCheckContext *ctx)
{
	Datum		attdatum;
	struct varlena *attr;
	...
	/* Skip null values */
	if (infomask & LEOPARD_HASNULL && att_isnull(ctx->attnum, ctx->tuphdr->t_bits))
		return true;
	...
	/*
	 * leopard_deform_tuple would be done with this attribute at this point,
	...
	if (!(infomask & LEOPARD_HASEXTERNAL))
	{
		report_corruption(ctx,
						  psprintf("toast value %u is external but tuple header flag LEOPARD_HASEXTERNAL not set",
 ...
 */
static void
check_toasted_attribute(LeopardCheckContext *ctx, ToastedAttribute *ta)
{
	SnapshotData SnapshotToast;
	ScanKeyData toastkey;
	SysScanDesc toastscan;
	bool		found_toasttup;
	LeopardTuple	toasttup;
	uint32		extsize;
	...
	while ((toasttup =
			systable_getnext_ordered_leopardtup(toastscan,
 ...
 */
static void
check_tuple(LeopardCheckContext *ctx)
{
	/*
...
FullTransactionIdFromXidAndCtx(TransactionId xid, const LeopardCheckContext *ctx)
{
	uint32		epoch;
 ...
 */
static void
update_cached_xid_range(LeopardCheckContext *ctx)
{
	/* Make cached copies */
 ...
 */
static void
update_cached_mxid_range(LeopardCheckContext *ctx)
{
 ...
 */
static inline bool
fxid_in_cached_range(FullTransactionId fxid, const LeopardCheckContext *ctx)
{
...
check_mxid_in_range(MultiXactId mxid, LeopardCheckContext *ctx)
{
...
/*
 * Checks whether the given mxid is valid to appear in the leopard being checked,
 ...
 */
static XidBoundsViolation
check_mxid_valid_in_rel(MultiXactId mxid, LeopardCheckContext *ctx)
{
 ...
 * in the leopard being checked, or whether it is too old or too new to appear in
 ...
 */
static XidBoundsViolation
get_xid_status(TransactionId xid, LeopardCheckContext *ctx,
			   XidCommitStatus *status)
{
