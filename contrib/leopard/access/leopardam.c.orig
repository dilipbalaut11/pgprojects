 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/leopard/leopardam.c
 *
 *
 * INTERFACE ROUTINES
 *		leopard_beginscan	- begin relation scan
 *		leopard_rescan		- restart a relation scan
 *		leopard_endscan	- end relation scan
 *		leopard_getnext	- retrieve next tuple in scan
 *		leopard_fetch		- retrieve tuple with given tid
 *		leopard_insert		- insert tuple into a relation
 *		leopard_multi_insert - insert multiple tuples into a relation
 *		leopard_delete		- delete a tuple from a relation
 *		leopard_update		- replace a tuple in a relation with another tuple
 *
 * NOTES
 *	  This file contains the leopard_ routines which implement
 *	  the POSTGRES leopard access method used for all POSTGRES
 *	  relations.
 *
 *-------------------------------------------------------------------------
 */
...
static XLogRecPtr log_leopard_update(Relation reln, Buffer oldbuf,
								  Buffer newbuf, LeopardTuple oldtup,
								  LeopardTuple newtup, LeopardTuple old_key_tuple,
								  bool all_visible_cleared, bool new_all_visible_cleared);
static Bitmapset *LeopardDetermineColumnsInfo(Relation relation,
										   Bitmapset *interesting_cols,
										   Bitmapset *external_cols,
										   LeopardTuple oldtup, LeopardTuple newtup,
										   bool *has_external);
static bool leopard_acquire_tuplock(Relation relation, ItemPointer tid,
								 LockTupleMode mode, LockWaitPolicy wait_policy,
								 bool *have_tuple_lock);
static void leopard_compute_new_xmax_infomask(TransactionId xmax, uint16 old_infomask,
									  uint16 old_infomask2, TransactionId add_to_xmax,
									  LockTupleMode mode, bool is_update,
									  TransactionId *result_xmax, uint16 *result_infomask,
									  uint16 *result_infomask2);
static TM_Result leopard_lock_updated_tuple(Relation rel, LeopardTuple tuple,
										 ItemPointer ctid, TransactionId xid,
										 LockTupleMode mode);
static void LeopardGetMultiXactIdHintBits(MultiXactId multi, uint16 *new_infomask,
								   uint16 *new_infomask2);
static TransactionId LeopardMultiXactIdGetUpdateXid(TransactionId xmax,
											 uint16 t_infomask);
static bool LeopardDoesMultiXactIdConflict(MultiXactId multi, uint16 infomask,
									LockTupleMode lockmode, bool *current_is_member);
static void LeopardMultiXactIdWait(MultiXactId multi, MultiXactStatus status, uint16 infomask,
							Relation rel, ItemPointer ctid, XLTW_Oper oper,
							int *remaining);
static bool LeopardConditionalMultiXactIdWait(MultiXactId multi, MultiXactStatus status,
									   uint16 infomask, Relation rel, int *remaining);
static void leopard_index_delete_sort(TM_IndexDeleteOp *delstate);
static int	leopard_bottomup_sort_and_shrink(TM_IndexDeleteOp *delstate);
static XLogRecPtr log_leopard_new_cid(Relation relation, LeopardTuple tup);
...
#ifdef USE_PREFETCH
/*
 * leopard_index_delete_tuples and leopard_index_delete_prefetch_buffer use this
...
/* leopard_index_delete_tuples bottom-up index deletion costing constants */
#define BOTTOMUP_MAX_NBLOCKS			6
#define BOTTOMUP_TOLERANCE_NBLOCKS		3

/*
 * leopard_index_delete_tuples uses this when determining which leopard blocks it
...
/* ----------------------------------------------------------------
 *						 leopard support routines
 * ----------------------------------------------------------------
 */

/* ----------------
 *		leopard_initscan - scan code common to leopard_beginscan and leopard_rescan
 * ----------------
 */
static void
leopard_initscan(LeopardScanDesc scan, ScanKey key, bool keep_startblock)
{
	...
	/*
	 * Currently, we only have a stats counter for sequential leopard scans (but
	 * e.g for bitmap scans the underlying bitmap index scans will be counted,
	 * and for sample scans we update stats for tuple fetches).
	 */
	if (scan->rs_base.rs_flags & SO_TYPE_SEQSCAN)
		pgstat_count_leopard_scan(scan->rs_base.rs_rd);
}

/*
 * leopard_setscanlimits - restrict range of a leopardscan
 *
 * startBlk is the page to start at
 * numBlks is number of pages to scan (InvalidBlockNumber means "all")
 */
void
leopard_setscanlimits(TableScanDesc sscan, BlockNumber startBlk, BlockNumber numBlks)
{
	LeopardScanDesc scan = (LeopardScanDesc) sscan;
...
/*
 * leopardgetpage - subroutine for leopardgettup()
 *
 ...
 */
void
leopardgetpage(TableScanDesc sscan, BlockNumber page)
{
	LeopardScanDesc scan = (LeopardScanDesc) sscan;
	Buffer		buffer;
	Snapshot	snapshot;
	 ...
	 */
	leopard_page_prune_opt(scan->rs_base.rs_rd, buffer);
	 ...
	 * crucial difference between index-only scans and leopard scans is that the
	 * index-only scan completely relies on the visibility map where as leopard
...
/* ----------------
 *		leopardgettup - fetch next leopard tuple
 *
 ...
 */
static void
leopardgettup(LeopardScanDesc scan,
		   ScanDirection dir,
		   int nkeys,
		   ScanKey key)
{
	LeopardTuple	tuple = &(scan->rs_ctup);
	...
	if (ScanDirectionIsForward(dir))
	{
		if (!scan->rs_inited)
		{
			...
			leopardgetpage((TableScanDesc) scan, page);
	...
	else if (backward)
	{
		...
		if (!scan->rs_inited)
		{
			 ...
			 * rs_numblocks if it's been adjusted by leopard_setscanlimits().
			 */
	...
	else
	{
		...
		tuple->t_data = (LeopardTupleHeader) PageGetItem((Page) dp, lpp);
	...
	for (;;)
	{
		...
		while (linesleft > 0)
		{
			if (ItemIdIsNormal(lpp))
			{
				 ...
				 */
				valid = LeopardTupleSatisfiesVisibility(tuple,
													 snapshot,
													 scan->rs_cbuf);
				...
				if (valid && key != NULL)
					LeopardKeyTest(tuple, RelationGetDescr(scan->rs_base.rs_rd),
								nkeys, key, valid);
...
/* ----------------
 *		leopardgettup_pagemode - fetch next leopard tuple in page-at-a-time mode
 *
 *		Same API as leopardgettup, but used in page-at-a-time mode
 *
 * The internal logic is much the same as leopardgettup's too, but there are some
 * differences: we do not take the buffer content lock (that only needs to
 * happen inside leopardgetpage), and we iterate through just the tuples listed
 * in rs_vistuples[] rather than all tuples on the page.  Notice that
 * lineindex is 0-based, where the corresponding loop variable lineoff in
 * leopardgettup is 1-based.
 * ----------------
 */
static void
leopardgettup_pagemode(LeopardScanDesc scan,
					ScanDirection dir,
					int nkeys,
					ScanKey key)
{
 ...
 * definition in access/leopardtup_details.h is maintained.
 */
Datum
leopardfastgetattr(LeopardTuple tup, int attnum, TupleDesc tupleDesc,
			bool *isnull)
{
	return (
			(attnum) > 0 ?
...
/* ----------------------------------------------------------------
 *					 leopard access method interface
 * ----------------------------------------------------------------
 */


TableScanDesc
leopard_beginscan(Relation relation, Snapshot snapshot,
			   int nkeys, ScanKey key,
			   ParallelTableScanDesc parallel_scan,
			   uint32 flags)
{
	LeopardScanDesc scan;
	 ...
	 */
	scan = (LeopardScanDesc) palloc(sizeof(LeopardScanDescData));
	...
	scan->rs_strategy = NULL;	/* set in leopard_initscan */
	 ...
	 * covering the range specified in the scan qual, but in a leopard scan there
	 ...
	 * any matching leopard tuples. For sample scan we could optimize the locking
	...
	/*
	 * we do this here instead of in leopard_initscan() because leopard_rescan also calls
	 * leopard_initscan() and we don't want to allocate memory again
	 */
	if (nkeys > 0)
	...
	leopard_initscan(scan, key, false);

	return (TableScanDesc) scan;
}

void
leopard_rescan(TableScanDesc sscan, ScanKey key, bool set_params,
			bool allow_strat, bool allow_sync, bool allow_pagemode)
{
	 ...
	 */
	leopard_initscan(scan, key, true);
}

void
leopard_endscan(TableScanDesc sscan)
{
	LeopardScanDesc scan = (LeopardScanDesc) sscan;
...
LeopardTuple
leopard_getnext(TableScanDesc sscan, ScanDirection direction)
{
	LeopardScanDesc scan = (LeopardScanDesc) sscan;
	 ...
	 * that create another AM reusing the leopard handler.
	 */
	if (unlikely(sscan->rs_rd->rd_tableam != GetLeopardamTableAmRoutine()))
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg_internal("only leopard AM is supported")));

	/*
	 * We don't expect direct calls to leopard_getnext with valid CheckXidAlive
	...
	if (unlikely(TransactionIdIsValid(CheckXidAlive) && !bsysscan))
		elog(ERROR, "unexpected leopard_getnext call during logical decoding");

	/* Note: no locking manipulations needed */

	if (scan->rs_base.rs_flags & SO_ALLOW_PAGEMODE)
		leopardgettup_pagemode(scan, direction,
							scan->rs_base.rs_nkeys, scan->rs_base.rs_key);
	else
		leopardgettup(scan, direction,
				   scan->rs_base.rs_nkeys, scan->rs_base.rs_key);
	...
	pgstat_count_leopard_getnext(scan->rs_base.rs_rd);

	return &scan->rs_ctup;
}

bool
leopard_getnextslot(TableScanDesc sscan, ScanDirection direction, TupleTableSlot *slot)
{
	LeopardScanDesc scan = (LeopardScanDesc) sscan;
	...
	if (sscan->rs_flags & SO_ALLOW_PAGEMODE)
		leopardgettup_pagemode(scan, direction, sscan->rs_nkeys, sscan->rs_key);
	else
		leopardgettup(scan, direction, sscan->rs_nkeys, sscan->rs_key);
	...
	ExecStoreBufferLeopardTuple(&scan->rs_ctup, slot,
							 scan->rs_cbuf);
	return true;
}

void
leopard_set_tidrange(TableScanDesc sscan, ItemPointer mintid,
				  ItemPointer maxtid)
{
	LeopardScanDesc scan = (LeopardScanDesc) sscan;
	...
	if (ItemPointerCompare(&highestItem, &lowestItem) < 0)
	{
		...
		leopard_setscanlimits(sscan, 0, 0);
		return;
	}
	...
	leopard_setscanlimits(sscan, startBlk, numBlks);
...
bool
leopard_getnextslot_tidrange(TableScanDesc sscan, ScanDirection direction,
						  TupleTableSlot *slot)
{
	LeopardScanDesc scan = (LeopardScanDesc) sscan;
	...
	for (;;)
	{
		...
		/*
		 * leopard_set_tidrange will have used leopard_setscanlimits to limit the
	...
	ExecStoreBufferLeopardTuple(&scan->rs_ctup, slot, scan->rs_cbuf);
	return true;
}
 ...
 *
 * leopard_fetch does not follow HOT chains: only the exact TID requested will
 * be fetched.
 *
 ...
 */
bool
leopard_fetch(Relation relation,
		   Snapshot snapshot,
		   LeopardTuple tuple,
		   Buffer *userbuf)
{
	return leopard_fetch_extended(relation, snapshot, tuple, userbuf, false);
}

/*
 *	leopard_fetch_extended		- fetch tuple even if it fails snapshot test
 *
 ...
 * If keep_buf is false then this behaves identically to leopard_fetch().
 */
bool
leopard_fetch_extended(Relation relation,
					Snapshot snapshot,
					LeopardTuple tuple,
					Buffer *userbuf,
					bool keep_buf)
{
	 ...
	 */
	tuple->t_data = (LeopardTupleHeader) PageGetItem(page, lp);
	 ...
	 */
	valid = LeopardTupleSatisfiesVisibility(tuple, snapshot, buffer);

	if (valid)
		PredicateLockTID(relation, &(tuple->t_self), snapshot,
						 LeopardTupleHeaderGetXmin(tuple->t_data));

	LeopardCheckForSerializableConflictOut(valid, relation, tuple, buffer, snapshot);
...
/*
 *	leopard_hot_search_buffer	- search HOT chain for tuple satisfying snapshot
 *
 ...
 *
 * leopardTuple is a caller-supplied buffer.  When a match is found, we return
					   ...
					   bool *all_dead, bool first_call)
{
	...
	for (;;)
	{
		...
		/*
		 * Update leopardTuple to point to the element of the HOT chain we're
		 * currently investigating. Having t_self set correctly is important
		 * because the SSI checks and the *Satisfies routine for historical
		 * MVCC snapshots need the correct tid to decide about the visibility.
		 */
		leopardTuple->t_data = (LeopardTupleHeader) PageGetItem(dp, lp);
		leopardTuple->t_len = ItemIdGetLength(lp);
		leopardTuple->t_tableOid = RelationGetRelid(relation);
		ItemPointerSet(&leopardTuple->t_self, blkno, offnum);
		 ...
		 * return the first tuple we find.  But on later passes, leopardTuple
		...
		if (!skip)
		{
			...
			valid = LeopardTupleSatisfiesVisibility(leopardTuple, snapshot, buffer);
			LeopardCheckForSerializableConflictOut(valid, relation, leopardTuple,
												buffer, snapshot);
...
//								 LeopardTupleHeaderGetXmin(leopardTuple->t_data));
		...
		if (all_dead && *all_dead)
		{
			...
			if (!LeopardTupleIsSurelyDead(leopardTuple, vistest))
				*all_dead = false;
		}
		...
		{
			Assert(ItemPointerGetBlockNumber(&leopardTuple->t_data->t_ctid) ==
				   blkno);
			offnum = ItemPointerGetOffsetNumber(&leopardTuple->t_data->t_ctid);
			at_chain_start = false;
			prev_xmax = LeopardTupleHeaderGetUpdateXid(leopardTuple->t_data);
		}
		else
			break;				/* end of chain */
	}
...
/*
 *	leopard_get_latest_tid -  get the latest tid of a specified tuple
 *
 ...
 */
void
leopard_get_latest_tid(TableScanDesc sscan,
					ItemPointer tid)
{
	...
	for (;;)
	{
		...
		tp.t_self = ctid;
		tp.t_data = (LeopardTupleHeader) PageGetItem(page, lp);
		...
		if (TransactionIdIsValid(priorXmax) &&
			!TransactionIdEquals(priorXmax, LeopardTupleHeaderGetXmin(tp.t_data)))
		{
			UnlockReleaseBuffer(buffer);
			break;
		}
		 ...
		 * candidate.
		 */
		valid = LeopardTupleSatisfiesVisibility(&tp, snapshot, buffer);
		LeopardCheckForSerializableConflictOut(valid, relation, &tp, buffer, snapshot);
		if (valid)
			*tid = ctid;
		 ...
		 */
		if ((tp.t_data->t_infomask & LEOPARD_XMAX_INVALID) ||
			LeopardTupleHeaderIsOnlyLocked(tp.t_data) ||
			LeopardTupleHeaderIndicatesMovedPartitions(tp.t_data) ||
		...
		priorXmax = LeopardTupleHeaderGetUpdateXid(tp.t_data);
		UnlockReleaseBuffer(buffer);
	}							/* end of loop */
}
...
/*
 * LeopardUpdateXmaxHintBits - update tuple hint bits after xmax transaction ends
 *
 ...
 *
 * Note that if the transaction was a locker only, we set LEOPARD_XMAX_INVALID
 * even if it commits.
 *
 ...
 */
static void
LeopardUpdateXmaxHintBits(LeopardTupleHeader tuple, Buffer buffer, TransactionId xid)
{
	Assert(TransactionIdEquals(LeopardTupleHeaderGetRawXmax(tuple), xid));
	Assert(!(tuple->t_infomask & LEOPARD_XMAX_IS_MULTI));

	if (!(tuple->t_infomask & (LEOPARD_XMAX_COMMITTED | LEOPARD_XMAX_INVALID)))
	{
		if (!LEOPARD_XMAX_IS_LOCKED_ONLY(tuple->t_infomask) &&
			TransactionIdDidCommit(xid))
			LeopardTupleSetHintBits(tuple, buffer, LEOPARD_XMAX_COMMITTED,
								 xid);
		else
			LeopardTupleSetHintBits(tuple, buffer, LEOPARD_XMAX_INVALID,
...
/*
 * LeopardGetBulkInsertState - prepare status object for a bulk insert
 */
BulkInsertState
LeopardGetBulkInsertState(void)
{
...
/*
 * LeopardFreeBulkInsertState - clean up after finishing a bulk insert
 */
void
LeopardFreeBulkInsertState(BulkInsertState bistate)
{
...
/*
 * LeopardReleaseBulkInsertStatePin - release a buffer currently held in bistate
 */
void
LeopardReleaseBulkInsertStatePin(BulkInsertState bistate)
{
	if (bistate->current_buf != InvalidBuffer)
		ReleaseBuffer(bistate->current_buf);
	bistate->current_buf = InvalidBuffer;
...
/*
 *	leopard_insert		- insert tuple into a leopard
 *
 ...
 *
 * There's corresponding LEOPARD_INSERT_ options to all the TABLE_INSERT_
 * options, and there additionally is LEOPARD_INSERT_SPECULATIVE which is used to
 ...
 */
void
leopard_insert(Relation relation, LeopardTuple tup, CommandId cid,
			int options, BulkInsertState bistate)
{
	...
	Assert(LeopardTupleHeaderGetNatts(tup->t_data) <=
		   RelationGetNumberOfAttributes(relation));

	/*
	 * Fill in tuple header fields and toast the tuple if necessary.
	 *
	 * Note: below this point, leopardtup is the data we actually intend to store
	 * into the relation; tup is the caller's original untoasted data.
	 */
	leopardtup = leopard_prepare_insert(relation, tup, xid, cid, options);

	/*
	 * Find buffer to insert this tuple into.  If the page is all visible,
	 * this will also pin the requisite visibility map page.
	 */
	buffer = RelationGetBufferForTuple(relation, leopardtup->t_len,
	 ...
	 *
	 * For a leopard insert, we only need to check for table-level SSI locks. Our
	 * new tuple can't possibly conflict with existing tuple locks, and leopard
	...
	RelationPutLeopardTuple(relation, buffer, leopardtup,
						 (options & LEOPARD_INSERT_SPECULATIVE) != 0);

	if (PageIsAllVisible(BufferGetPage(buffer)))
	{
		all_visible_cleared = true;
		PageClearAllVisible(BufferGetPage(buffer));
		leopard_vismap_clear(relation,
							ItemPointerGetBlockNumber(&(leopardtup->t_self)),
							vmbuffer, LEOPARDVISMAP_VALID_BITS);
	}
	 ...
	 *
	 * If you do add PageSetPrunable here, add it in leopard_xlog_insert too.
	 */
	...
	{
		xl_leopard_insert xlrec;
		xl_leopard_header xlhdr;
		XLogRecPtr	recptr;
		Page		page = BufferGetPage(buffer);
		uint8		info = XLOG_LEOPARD_INSERT;
		int			bufflags = 0;
		...
		if (RelationIsAccessibleInLogicalDecoding(relation))
			log_leopard_new_cid(relation, leopardtup);
		 ...
		 */
		if (ItemPointerGetOffsetNumber(&(leopardtup->t_self)) == FirstOffsetNumber &&
			PageGetMaxOffsetNumber(page) == FirstOffsetNumber)
		{
			info |= XLOG_LEOPARD_INIT_PAGE;
			bufflags |= REGBUF_WILL_INIT;
		}

		xlrec.offnum = ItemPointerGetOffsetNumber(&leopardtup->t_self);
		xlrec.flags = 0;
		if (all_visible_cleared)
			xlrec.flags |= XLLEOPARD_INSERT_ALL_VISIBLE_CLEARED;
		if (options & LEOPARD_INSERT_SPECULATIVE)
			xlrec.flags |= XLLEOPARD_INSERT_IS_SPECULATIVE;
		Assert(ItemPointerGetBlockNumber(&leopardtup->t_self) == BufferGetBlockNumber(buffer));
		...
		if (RelationIsLogicallyLogged(relation) &&
			!(options & LEOPARD_INSERT_NO_LOGICAL))
		{
			xlrec.flags |= XLLEOPARD_INSERT_CONTAINS_NEW_TUPLE;
			bufflags |= REGBUF_KEEP_DATA;

			if (IsToastRelation(relation))
				xlrec.flags |= XLLEOPARD_INSERT_ON_TOAST_RELATION;
		}

		XLogBeginInsert();
		XLogRegisterData((char *) &xlrec, SizeOfLeopardInsert);

		xlhdr.t_infomask2 = leopardtup->t_data->t_infomask2;
		xlhdr.t_infomask = leopardtup->t_data->t_infomask;
		xlhdr.t_hoff = leopardtup->t_data->t_hoff;

		/*
		 * note we mark xlhdr as belonging to buffer; if XLogInsert decides to
		 * write the whole page to the xlog, we don't need to store
		 * xl_leopard_header in the xlog.
		 */
		XLogRegisterBuffer(0, buffer, REGBUF_STANDARD | bufflags);
		XLogRegisterBufData(0, (char *) &xlhdr, SizeOfLeopardHeader);
		/* PG73FORMAT: write bitmap [+ padding] [+ oid] + data */
		XLogRegisterBufData(0,
							(char *) leopardtup->t_data + SizeofLeopardTupleHeader,
							leopardtup->t_len - SizeofLeopardTupleHeader);

		/* filtering by origin on a row level is much more efficient */
		XLogSetRecordFlags(XLOG_INCLUDE_ORIGIN);

		recptr = XLogInsert(RM_LEOPARD_ID, info);
	 ...
	 * the leopardtup data structure is all in local memory, not in the shared
	 * buffer.
	 */
	CacheInvalidateLeopardTuple(relation, leopardtup, NULL);

	/* Note: speculative insertions are counted too, even if aborted later */
	pgstat_count_leopard_insert(relation, 1);

	/*
	 * If leopardtup is a private copy, release it.  Don't forget to copy t_self
	 * back to the caller's image, too.
	 */
	if (leopardtup != tup)
	{
		tup->t_self = leopardtup->t_self;
		leopard_freetuple(leopardtup);
	}
}

/*
 * Subroutine for leopard_insert(). Prepares a tuple for insertion. This sets the
 ...
 */
static LeopardTuple
leopard_prepare_insert(Relation relation, LeopardTuple tup, TransactionId xid,
					CommandId cid, int options)
{
	...
	tup->t_data->t_infomask &= ~(LEOPARD_XACT_MASK);
	tup->t_data->t_infomask2 &= ~(LEOPARD2_XACT_MASK);
	tup->t_data->t_infomask |= LEOPARD_XMAX_INVALID;
	LeopardTupleHeaderSetXmin(tup->t_data, xid);
	if (options & LEOPARD_INSERT_FROZEN)
		LeopardTupleHeaderSetXminFrozen(tup->t_data);

	LeopardTupleHeaderSetCmin(tup->t_data, cid);
	LeopardTupleHeaderSetXmax(tup->t_data, 0); /* for cleanliness */
		...
		relation->rd_rel->relkind != RELKIND_MATVIEW)
	{
		...
		Assert(!LeopardTupleHasExternal(tup));
		return tup;
	}
	else if (LeopardTupleHasExternal(tup) || tup->t_len > TOAST_TUPLE_THRESHOLD)
		return leopard_toast_insert_or_update(relation, tup, NULL, options);
	else
		return tup;
}

/*
 *	leopard_multi_insert	- insert multiple tuples into a leopard
 *
 * This is like leopard_insert(), but inserts multiple tuples in one operation.
 * That's faster than calling leopard_insert() in a loop, because when multiple
 ...
 */
void
leopard_multi_insert(Relation relation, TupleTableSlot **slots, int ntuples,
				  CommandId cid, int options, BulkInsertState bistate)
{
	TransactionId xid = GetCurrentTransactionId();
	LeopardTuple  *leopardtuples;
	int			i;
	int			ndone;
	...
	/* currently not needed (thus unsupported) for leopard_multi_insert() */
	AssertArg(!(options & LEOPARD_INSERT_NO_LOGICAL));
	...
	leopardtuples = palloc(ntuples * sizeof(LeopardTuple));
	for (i = 0; i < ntuples; i++)
	{
		LeopardTuple	tuple;

		tuple = ExecFetchSlotLeopardTuple(slots[i], true, NULL);
		slots[i]->tts_tableOid = RelationGetRelid(relation);
		tuple->t_tableOid = slots[i]->tts_tableOid;
		leopardtuples[i] = leopard_prepare_insert(relation, tuple, xid, cid,
											options);
	}
	 ...
	 *
	 * For leopard inserts, we only need to check for table-level SSI locks. Our
	 * new tuples can't possibly conflict with existing tuple locks, and leopard
	 * page locks are only consolidated versions of tuple locks; they do not
	 * lock "gaps" as index page locks do.  So we don't need to specify a
	 * buffer when making the call, which makes for a faster check.
	 */
	...
	while (ndone < ntuples)
	{
		 ...
		 */
		buffer = RelationGetBufferForTuple(relation, leopardtuples[ndone]->t_len,
										   InvalidBuffer, options, bistate,
										   &vmbuffer, NULL);
		page = BufferGetPage(buffer);
		...
		if (starting_with_empty_page && (options & LEOPARD_INSERT_FROZEN))
			all_frozen_set = true;
		 ...
		 */
		RelationPutLeopardTuple(relation, buffer, leopardtuples[ndone], false);
		...
		if (needwal && need_cids)
			log_leopard_new_cid(relation, leopardtuples[ndone]);

		for (nthispage = 1; ndone + nthispage < ntuples; nthispage++)
		{
			LeopardTuple	leopardtup = leopardtuples[ndone + nthispage];

			if (PageGetHeapFreeSpace(page) < MAXALIGN(leopardtup->t_len) + saveFreeSpace)
				break;

			RelationPutLeopardTuple(relation, buffer, leopardtup, false);
		 ...
		 */
		if (PageIsAllVisible(page) && !(options & LEOPARD_INSERT_FROZEN))
		{
			all_visible_cleared = true;
		...
		/*
		 * XXX Should we set PageSetPrunable on this page ? See leopard_insert()
		 */
		...
		{
			XLogRecPtr	recptr;
			xl_leopard_multi_insert *xlrec;
			uint8		info = XLOG_LEOPARD2_MULTI_INSERT;
			char	   *tupledata;
			int			totaldatalen;
			...
			/* allocate xl_leopard_multi_insert struct from the scratch area */
			xlrec = (xl_leopard_multi_insert *) scratchptr;
			scratchptr += SizeOfLeopardMultiInsert;
			...
			if (all_visible_cleared)
				xlrec->flags = XLLEOPARD_INSERT_ALL_VISIBLE_CLEARED;
			if (all_frozen_set)
				xlrec->flags = XLLEOPARD_INSERT_ALL_FROZEN_SET;
			...
			{
				LeopardTuple	leopardtup = leopardtuples[ndone + i];
				xl_multi_insert_tuple *tuphdr;
				int			datalen;

				if (!init)
					xlrec->offsets[i] = ItemPointerGetOffsetNumber(&leopardtup->t_self);
				/* xl_multi_insert_tuple needs two-byte alignment. */
				tuphdr = (xl_multi_insert_tuple *) SHORTALIGN(scratchptr);
				scratchptr = ((char *) tuphdr) + SizeOfMultiInsertTuple;

				tuphdr->t_infomask2 = leopardtup->t_data->t_infomask2;
				tuphdr->t_infomask = leopardtup->t_data->t_infomask;
				tuphdr->t_hoff = leopardtup->t_data->t_hoff;

				/* write bitmap [+ padding] [+ oid] + data */
				datalen = leopardtup->t_len - SizeofLeopardTupleHeader;
				memcpy(scratchptr,
					   (char *) leopardtup->t_data + SizeofLeopardTupleHeader,
					   datalen);
			...
			/*
			 * Signal that this is the last xl_leopard_multi_insert record
			 * emitted by this call to leopard_multi_insert(). Needed for logical
			 * decoding so it knows when to cleanup temporary data.
			 */
			if (ndone + nthispage == ntuples)
				xlrec->flags |= XLLEOPARD_INSERT_LAST_IN_MULTI;
			...
			recptr = XLogInsert(RM_LEOPARD2_ID, info);
		...
		if (all_frozen_set)
		{
			...
			Assert(leopard_vismap_pin_ok(BufferGetBlockNumber(buffer), vmbuffer));

			/*
			 * It's fine to use InvalidTransactionId here - this is only used
			 * when LEOPARD_INSERT_FROZEN is specified, which intentionally
			 * violates visibility rules.
			 */
			leopard_vismap_set(relation, BufferGetBlockNumber(buffer), buffer,
							  InvalidXLogRecPtr, vmbuffer,
							  InvalidTransactionId,
							  LEOPARDVISMAP_ALL_VISIBLE | LEOPARDVISMAP_ALL_FROZEN);
		}
		 ...
		 * likely that we'll insert into subsequent leopard pages that are likely
	 ...
	 * this final check, a sequential scan of the leopard may have locked the
	 ...
	 * because the leopardtuples data structure is all in local memory, not in
	 * the shared buffer.
	 */
	if (IsCatalogRelation(relation))
	{
		for (i = 0; i < ntuples; i++)
			CacheInvalidateLeopardTuple(relation, leopardtuples[i], NULL);
	}

	/* copy t_self fields back to the caller's slots */
	for (i = 0; i < ntuples; i++)
		slots[i]->tts_tid = leopardtuples[i]->t_self;

	pgstat_count_leopard_insert(relation, ntuples);
}

/*
 *	simple_leopard_insert - insert a tuple
 *
 * Currently, this routine differs from leopard_insert only in supplying
 * a default command ID and not allowing access to the speedup options.
 *
 * This should be used rather than using leopard_insert directly in most places
 * where we are modifying system catalogs.
 */
void
simple_leopard_insert(Relation relation, LeopardTuple tup)
{
	leopard_insert(relation, tup, GetCurrentCommandId(true), 0, NULL);
}

/*
 * Given infomask/infomask2, compute the bits that must be saved in the
 * "infobits" field of xl_leopard_delete, xl_leopard_update, xl_leopard_lock,
 * xl_leopard_lock_updated WAL records.
 *
 * See leopard_fix_infomask_from_infobits.
 */
static uint8
leopard_compute_infobits(uint16 infomask, uint16 infomask2)
{
	return
		((infomask & LEOPARD_XMAX_IS_MULTI) != 0 ? XLHL_XMAX_IS_MULTI : 0) |
		((infomask & LEOPARD_XMAX_LOCK_ONLY) != 0 ? XLHL_XMAX_LOCK_ONLY : 0) |
		((infomask & LEOPARD_XMAX_EXCL_LOCK) != 0 ? XLHL_XMAX_EXCL_LOCK : 0) |
	/* note we ignore LEOPARD_XMAX_SHR_LOCK here */
		((infomask & LEOPARD_XMAX_KEYSHR_LOCK) != 0 ? XLHL_XMAX_KEYSHR_LOCK : 0) |
		((infomask2 & LEOPARD_KEYS_UPDATED) != 0 ?
		 XLHL_KEYS_UPDATED : 0);
}
 ...
 */
static inline bool
leopard_xmax_infomask_changed(uint16 new_infomask, uint16 old_infomask)
{
	const uint16 interesting =
	LEOPARD_XMAX_IS_MULTI | LEOPARD_XMAX_LOCK_ONLY | LEOPARD_LOCK_MASK;
 ...
 */
TM_Result
leopard_delete(Relation relation, ItemPointer tid,
			...
			TM_FailureData *tmfd, bool changingPart)
{
	...
	bool		iscombo;
	bool		all_visible_cleared = false;
	LeopardTuple	old_key_tuple = NULL;	/* replica identity of the tuple */
	...
	if (PageIsAllVisible(page))
		leopard_vismap_pin(relation, block, &vmbuffer);
...
l1:
	result = LeopardTupleSatisfiesUpdate(&tp, cid, buffer);
	...
	else if (result == TM_BeingModified && wait)
	{
		...
		xwait = LeopardTupleHeaderGetRawXmax(tp.t_data);
		 ...
		 * priority for the tuple (see leopard_lock_tuple).  LockTuple will
		 ...
		 * tuple state.
		 */
		if (infomask & LEOPARD_XMAX_IS_MULTI)
		{
			bool		current_is_member = false;

			if (LeopardDoesMultiXactIdConflict((MultiXactId) xwait, infomask,
										LockTupleExclusive, &current_is_member))
			{
				...
				if (!current_is_member)
					leopard_acquire_tuplock(relation, &(tp.t_self), LockTupleExclusive,
										 LockWaitBlock, &have_tuple_lock);

				/* wait for multixact */
				LeopardMultiXactIdWait((MultiXactId) xwait, MultiXactStatusUpdate, infomask,
				 ...
				 */
				if (leopard_xmax_infomask_changed(tp.t_data->t_infomask, infomask) ||
					!TransactionIdEquals(LeopardTupleHeaderGetRawXmax(tp.t_data),
										 xwait))
					goto l1;
			}
		...
		else if (!TransactionIdIsCurrentTransactionId(xwait))
		{
			...
			LeopardUpdateXmaxHintBits(tp.t_data, buffer, xwait);
		}
		 ...
		 */
		if ((tp.t_data->t_infomask & LEOPARD_XMAX_INVALID) ||
			LEOPARD_XMAX_IS_LOCKED_ONLY(tp.t_data->t_infomask) ||
			LeopardTupleHeaderIsOnlyLocked(tp.t_data))
			result = TM_Ok;
	...
	if (crosscheck != InvalidSnapshot && result == TM_Ok)
	{
		...
		if (!LeopardTupleSatisfiesVisibility(&tp, crosscheck, buffer))
			result = TM_Updated;
	}
	...
	if (result != TM_Ok)
	{
		...
		Assert(!(tp.t_data->t_infomask & LEOPARD_XMAX_INVALID));
		Assert(result != TM_Updated ||
			   !ItemPointerEquals(&tp.t_self, &tp.t_data->t_ctid));
		tmfd->ctid = tp.t_data->t_ctid;
		tmfd->xmax = LeopardTupleHeaderGetUpdateXid(tp.t_data);
		if (result == TM_SelfModified)
			tmfd->cmax = LeopardTupleHeaderGetCmax(tp.t_data);
		else
	...
	LeopardTupleHeaderAdjustCmax(tp.t_data, &cid, &iscombo);

	/*
	 * Compute replica identity tuple before entering the critical section so
	 * we don't PANIC upon a memory allocation failure.
	 */
	old_key_tuple = LeopardExtractReplicaIdentity(relation, &tp, true, &old_key_copied);
	...
	leopard_compute_new_xmax_infomask(LeopardTupleHeaderGetRawXmax(tp.t_data),
	...
	if (PageIsAllVisible(page))
	{
		all_visible_cleared = true;
		PageClearAllVisible(page);
		leopard_vismap_clear(relation, BufferGetBlockNumber(buffer),
							vmbuffer, LEOPARDVISMAP_VALID_BITS);
	}

	/* store transaction information of xact deleting the tuple */
	tp.t_data->t_infomask &= ~(LEOPARD_XMAX_BITS);
	tp.t_data->t_infomask2 &= ~LEOPARD_KEYS_UPDATED;
	tp.t_data->t_infomask |= new_infomask;
	tp.t_data->t_infomask2 |= new_infomask2;
	LeopardTupleHeaderClearHotUpdated(tp.t_data);
	LeopardTupleHeaderSetXmax(tp.t_data, new_xmax);
	LeopardTupleHeaderSetCmax(tp.t_data, cid, iscombo);
	/* Make sure there is no forward chain link in t_ctid */
	tp.t_data->t_ctid = tp.t_self;

	/* Signal that this is actually a move into another partition */
	if (changingPart)
		LeopardTupleHeaderSetMovedPartitions(tp.t_data);
	 ...
	 * XLOG stuff
	 *
	 * NB: leopard_abort_speculative() uses the same xlog record and replay
	 * routines.
	 */
	if (RelationNeedsWAL(relation))
	{
		xl_leopard_delete xlrec;
		xl_leopard_header xlhdr;
		XLogRecPtr	recptr;
		 ...
		 */
		if (RelationIsAccessibleInLogicalDecoding(relation))
			log_leopard_new_cid(relation, &tp);

		xlrec.flags = 0;
		if (all_visible_cleared)
			xlrec.flags |= XLLEOPARD_DELETE_ALL_VISIBLE_CLEARED;
		if (changingPart)
			xlrec.flags |= XLLEOPARD_DELETE_IS_PARTITION_MOVE;
		xlrec.infobits_set = leopard_compute_infobits(tp.t_data->t_infomask,
		...
		if (old_key_tuple != NULL)
		{
			if (relation->rd_rel->relreplident == REPLICA_IDENTITY_FULL)
				xlrec.flags |= XLLEOPARD_DELETE_CONTAINS_OLD_TUPLE;
			else
				xlrec.flags |= XLLEOPARD_DELETE_CONTAINS_OLD_KEY;
		}

		XLogBeginInsert();
		XLogRegisterData((char *) &xlrec, SizeOfLeopardDelete);
		...
		if (old_key_tuple != NULL)
		{
			...
			XLogRegisterData((char *) &xlhdr, SizeOfLeopardHeader);
			XLogRegisterData((char *) old_key_tuple->t_data
							 + SizeofLeopardTupleHeader,
							 old_key_tuple->t_len
							 - SizeofLeopardTupleHeader);
		}
		...
		recptr = XLogInsert(RM_LEOPARD_ID, XLOG_LEOPARD_DELETE);
	...
	{
		/* toast table entries should never be recursively toasted */
		Assert(!LeopardTupleHasExternal(&tp));
	}
	else if (LeopardTupleHasExternal(&tp))
		leopard_toast_delete(relation, &tp, false);
	 ...
	 */
	CacheInvalidateLeopardTuple(relation, &tp, NULL);
	...
	pgstat_count_leopard_delete(relation);

	if (old_key_tuple != NULL && old_key_copied)
		leopard_freetuple(old_key_tuple);

	return TM_Ok;
}

/*
 *	simple_leopard_delete - delete a tuple
 *
 ...
 */
void
simple_leopard_delete(Relation relation, ItemPointer tid)
{
	TM_Result	result;
	TM_FailureData tmfd;

	result = leopard_delete(relation, tid,
	...
	switch (result)
	{
		...
		default:
			elog(ERROR, "unrecognized leopard_delete status: %u", result);
			break;
	}
}

/*
 *	leopard_update - replace a tuple
 *
 ...
 */
TM_Result
leopard_update(Relation relation, ItemPointer otid, LeopardTuple newtup,
			CommandId cid, Snapshot crosscheck, bool wait,
			TM_FailureData *tmfd, LockTupleMode *lockmode)
{
	...
	ItemId		lp;
	LeopardTupleData oldtup;
	LeopardTuple	leopardtup;
	LeopardTuple	old_key_tuple = NULL;
	bool		old_key_copied = false;
	Page		page;
	BlockNumber block;
	...
	/* Cheap, simplistic check that the tuple matches the rel's rowtype. */
	Assert(LeopardTupleHeaderGetNatts(newtup->t_data) <=
		   RelationGetNumberOfAttributes(relation));
	...
	/*
	 * Fill in enough data in oldtup for LeopardDetermineColumnsInfo to work
	 * properly.
	 */
	oldtup.t_tableOid = RelationGetRelid(relation);
	oldtup.t_data = (LeopardTupleHeader) PageGetItem(page, lp);
	 ...
	 * LeopardExtractReplicaIdentity.
	 */
	modified_attrs = LeopardDetermineColumnsInfo(relation, interesting_attrs,
											  id_attrs, &oldtup,
...
l2:
	...
	result = LeopardTupleSatisfiesUpdate(&oldtup, cid, buffer);
	...
	else if (result == TM_BeingModified && wait)
	{
		 ...
		 * leopard_update directly.
		 */

		/* must copy state data before unlocking buffer */
		xwait = LeopardTupleHeaderGetRawXmax(oldtup.t_data);
		...
		if (infomask & LEOPARD_XMAX_IS_MULTI)
		{
										...
										*lockmode, &current_is_member))
			{
				 ...
				 */
				if (!current_is_member)
					leopard_acquire_tuplock(relation, &(oldtup.t_self), *lockmode,
										 LockWaitBlock, &have_tuple_lock);

				/* wait for multixact */
				LeopardMultiXactIdWait((MultiXactId) xwait, mxact_status, infomask,
				 ...
				 * for xmax change, and start over if so.
				 */
				if (leopard_xmax_infomask_changed(oldtup.t_data->t_infomask,
										  infomask) ||
					!TransactionIdEquals(LeopardTupleHeaderGetRawXmax(oldtup.t_data),
										 xwait))
					goto l2;
			}
			 ...
			 * subxact aborts.
			 */
			if (!LEOPARD_XMAX_IS_LOCKED_ONLY(oldtup.t_data->t_infomask))
				update_xact = LeopardTupleGetUpdateXid(oldtup.t_data);
			else
		...
		else if (LEOPARD_XMAX_IS_KEYSHR_LOCKED(infomask) && key_intact)
		{
			/*
		...
		else
		{
			 ...
			 * Check for xmax change, and start over if so.
			 */
			if (leopard_xmax_infomask_changed(oldtup.t_data->t_infomask, infomask) ||
				!TransactionIdEquals(xwait,
									 LeopardTupleHeaderGetRawXmax(oldtup.t_data)))
				goto l2;

			/* Otherwise check if it committed or aborted */
			LeopardUpdateXmaxHintBits(oldtup.t_data, buffer, xwait);
			if (oldtup.t_data->t_infomask & LEOPARD_XMAX_INVALID)
				can_continue = true;
		}
	...
	{
		/* Perform additional check for transaction-snapshot mode RI updates */
		if (!LeopardTupleSatisfiesVisibility(&oldtup, crosscheck, buffer))
		{
			result = TM_Updated;
	...
	if (result != TM_Ok)
	{
		Assert(result == TM_SelfModified ||
			   ...
			   result == TM_Deleted ||
			   result == TM_BeingModified);
		Assert(!(oldtup.t_data->t_infomask & LEOPARD_XMAX_INVALID));
		Assert(result != TM_Updated ||
			   !ItemPointerEquals(&oldtup.t_self, &oldtup.t_data->t_ctid));
		tmfd->ctid = oldtup.t_data->t_ctid;
		tmfd->xmax = LeopardTupleHeaderGetUpdateXid(oldtup.t_data);
		if (result == TM_SelfModified)
			tmfd->cmax = LeopardTupleHeaderGetCmax(oldtup.t_data);
		else
			tmfd->cmax = InvalidCommandId;
		UnlockReleaseBuffer(buffer);
		if (have_tuple_lock)
	 ...
	 */
	leopard_compute_new_xmax_infomask(LeopardTupleHeaderGetRawXmax(oldtup.t_data),
	 ...
	 * LEOPARD_XMAX_INVALID bit set; that's fine.)
	 */
	if ((oldtup.t_data->t_infomask & LEOPARD_XMAX_INVALID) ||
		LEOPARD_LOCKED_UPGRADED(oldtup.t_data->t_infomask) ||
		(checked_lockers && !locker_remains))
		xmax_new_tuple = InvalidTransactionId;
	else
		xmax_new_tuple = LeopardTupleHeaderGetRawXmax(oldtup.t_data);

	if (!TransactionIdIsValid(xmax_new_tuple))
	{
		infomask_new_tuple = LEOPARD_XMAX_INVALID;
	...
	else
	{
		 ...
		 */
		if (oldtup.t_data->t_infomask & LEOPARD_XMAX_IS_MULTI)
		{
			LeopardGetMultiXactIdHintBits(xmax_new_tuple, &infomask_new_tuple,
								   &infomask2_new_tuple);
		}
		else
		{
			infomask_new_tuple = LEOPARD_XMAX_KEYSHR_LOCK | LEOPARD_XMAX_LOCK_ONLY;
			infomask2_new_tuple = 0;
		}
	}
	 ...
	 */
	newtup->t_data->t_infomask &= ~(LEOPARD_XACT_MASK);
	newtup->t_data->t_infomask2 &= ~(LEOPARD2_XACT_MASK);
	LeopardTupleHeaderSetXmin(newtup->t_data, xid);
	LeopardTupleHeaderSetCmin(newtup->t_data, cid);
	newtup->t_data->t_infomask |= LEOPARD_UPDATED | infomask_new_tuple;
	newtup->t_data->t_infomask2 |= infomask2_new_tuple;
	LeopardTupleHeaderSetXmax(newtup->t_data, xmax_new_tuple);

	/*
	 * Replace cid with a combo CID if necessary.  Note that we already put
	 * the plain cid into the new tuple.
	 */
	LeopardTupleHeaderAdjustCmax(oldtup.t_data, &cid, &iscombo);
	...
	{
		/* toast table entries should never be recursively toasted */
		Assert(!LeopardTupleHasExternal(&oldtup));
		Assert(!LeopardTupleHasExternal(newtup));
		need_toast = false;
	}
	else
		need_toast = (LeopardTupleHasExternal(&oldtup) ||
					  LeopardTupleHasExternal(newtup) ||
	...
	if (need_toast || newtupsize > pagefree)
	{
		 ...
		 * temporary modification.  We can reuse xl_leopard_lock for this
		...
		Assert(LEOPARD_XMAX_IS_LOCKED_ONLY(infomask_lock_old_tuple));

		START_CRIT_SECTION();

		/* Clear obsolete visibility flags ... */
		oldtup.t_data->t_infomask &= ~(LEOPARD_XMAX_BITS);
		oldtup.t_data->t_infomask2 &= ~LEOPARD_KEYS_UPDATED;
		LeopardTupleClearHotUpdated(&oldtup);
		/* ... and store info about transaction updating this tuple */
		Assert(TransactionIdIsValid(xmax_lock_old_tuple));
		LeopardTupleHeaderSetXmax(oldtup.t_data, xmax_lock_old_tuple);
		oldtup.t_data->t_infomask |= infomask_lock_old_tuple;
		oldtup.t_data->t_infomask2 |= infomask2_lock_old_tuple;
		LeopardTupleHeaderSetCmax(oldtup.t_data, cid, iscombo);
		 ...
		 */
		if (PageIsAllVisible(page) &&
			leopard_vismap_clear(relation, block, vmbuffer,
								LEOPARDVISMAP_ALL_FROZEN))
		...
		if (RelationNeedsWAL(relation))
		{
			...
			xlrec.infobits_set = leopard_compute_infobits(oldtup.t_data->t_infomask,
												  oldtup.t_data->t_infomask2);
			xlrec.flags =
				cleared_all_frozen ? XLLEOPARD_LOCK_ALL_FROZEN_CLEARED : 0;
			XLogRegisterData((char *) &xlrec, SizeOfLeopardLock);
			recptr = XLogInsert(RM_LEOPARD_ID, XLOG_LEOPARD_LOCK);
			PageSetLSN(page, recptr);
		}
		 ...
		 *
		 * Note: below this point, leopardtup is the data we actually intend to
		...
		if (need_toast)
		{
			...
			leopardtup = leopard_toast_insert_or_update(relation, newtup, &oldtup, 0);
			newtupsize = MAXALIGN(leopardtup->t_len);
		}
		else
			leopardtup = newtup;
		...
		for (;;)
		{
			if (newtupsize > pagefree)
			{
				...
				newbuf = RelationGetBufferForTuple(relation, leopardtup->t_len,
												   buffer, 0, NULL,
	 ...
	 * Compute replica identity tuple before entering the critical section so
	 * we don't PANIC upon a memory allocation failure.
	 * LeopardExtractReplicaIdentity() will return NULL if nothing needs to be
	 * logged.  Pass old key required as true only if the replica identity key
	 * columns are modified or it has external data.
	 */
	old_key_tuple = LeopardExtractReplicaIdentity(relation, &oldtup,
	 ...
	 * not to optimize for aborts.  Note that leopard_xlog_update must be kept in
	...
	if (use_hot_update)
	{
		...
		LeopardTupleSetHotUpdated(&oldtup);
		/* And mark the new tuple as leopard-only */
		LeopardTupleSetLeopardOnly(leopardtup);
		/* Mark the caller's copy too, in case different from leopardtup */
		LeopardTupleSetLeopardOnly(newtup);
	}
	else
	{
		/* Make sure tuples are correctly marked as not-HOT */
		LeopardTupleClearHotUpdated(&oldtup);
		LeopardTupleClearLeopardOnly(leopardtup);
		LeopardTupleClearLeopardOnly(newtup);
	}

	RelationPutLeopardTuple(relation, newbuf, leopardtup, false); /* insert new tuple */
	...
	LeopardTupleHeaderSetXmax(oldtup.t_data, xmax_old_tuple);
	oldtup.t_data->t_infomask |= infomask_old_tuple;
	oldtup.t_data->t_infomask2 |= infomask2_old_tuple;
	LeopardTupleHeaderSetCmax(oldtup.t_data, cid, iscombo);

	/* record address of new tuple in t_ctid of old one */
	oldtup.t_data->t_ctid = leopardtup->t_self;
	...
	if (newbuf != buffer && PageIsAllVisible(BufferGetPage(newbuf)))
	{
		...
		leopard_vismap_clear(relation, BufferGetBlockNumber(newbuf),
							vmbuffer_new, LEOPARDVISMAP_VALID_BITS);
	}
	...
	if (RelationNeedsWAL(relation))
	{
		...
		if (RelationIsAccessibleInLogicalDecoding(relation))
		{
			log_leopard_new_cid(relation, &oldtup);
			log_leopard_new_cid(relation, leopardtup);
		}

		recptr = log_leopard_update(relation, buffer,
								 newbuf, &oldtup, leopardtup,
								 old_key_tuple,
								 all_visible_cleared,
	 ...
	 * buffer.  (leopardtup is all in local memory, but it's necessary to process
	 * both tuple versions in one call to inval.c so we can avoid redundant
	 * sinval messages.)
	 */
	CacheInvalidateLeopardTuple(relation, &oldtup, leopardtup);
	...
	pgstat_count_leopard_update(relation, use_hot_update);

	/*
	 * If leopardtup is a private copy, release it.  Don't forget to copy t_self
	 * back to the caller's image, too.
	 */
	if (leopardtup != newtup)
	{
		newtup->t_self = leopardtup->t_self;
		leopard_freetuple(leopardtup);
	}
 ...
 * LeopardDetermineColumnsInfo.
 */
static bool
leopard_attr_equals(TupleDesc tupdesc, int attrnum, Datum value1, Datum value2,
 ...
 * since this is invoked at most once in leopard_update.
 */
static Bitmapset *
LeopardDetermineColumnsInfo(Relation relation,
						 Bitmapset *interesting_cols,
						 Bitmapset *external_cols,
						 LeopardTuple oldtup, LeopardTuple newtup,
						 bool *has_external)
{
	...
	while ((attrnum = bms_first_member(interesting_cols)) >= 0)
	{
		...
		attrnum += FirstLowInvalidLeopardAttributeNumber;
		...
		if (attrnum == 0)
		{
			modified = bms_add_member(modified,
									  attrnum -
									  FirstLowInvalidLeopardAttributeNumber);
			continue;
		}
		 ...
		 * leopard_deform_tuple call on each tuple, instead?	But that doesn't
		 * work for system columns ...
		 */
		value1 = leopard_getattr(oldtup, attrnum, tupdesc, &isnull1);
		value2 = leopard_getattr(newtup, attrnum, tupdesc, &isnull2);

		if (!leopard_attr_equals(tupdesc, attrnum, value1,
		...
		if (VARATT_IS_EXTERNAL((struct varlena *) DatumGetPointer(value1)) &&
			bms_is_member(attrnum - FirstLowInvalidLeopardAttributeNumber,
						  external_cols))
			*has_external = true;
	}
...
/*
 *	simple_leopard_update - replace a tuple
 *
 ...
 */
void
simple_leopard_update(Relation relation, ItemPointer otid, LeopardTuple tup)
{
	TM_Result	result;
	TM_FailureData tmfd;
	LockTupleMode lockmode;

	result = leopard_update(relation, otid, tup,
						 GetCurrentCommandId(true), InvalidSnapshot,
						 true /* wait for commit */ ,
						 &tmfd, &lockmode);
	switch (result)
	{
		...
		default:
			elog(ERROR, "unrecognized leopard_update status: %u", result);
			break;
	}
}
...
/*
 *	leopard_lock_tuple - lock a tuple in shared or exclusive mode
 *
 ...
 */
TM_Result
leopard_lock_tuple(Relation relation, LeopardTuple tuple,
...
l3:
	result = LeopardTupleSatisfiesUpdate(tuple, cid, *buffer);
			 ...
			 result == TM_Deleted)
	{
		...
		/* must copy state data before unlocking buffer */
		xwait = LeopardTupleHeaderGetRawXmax(tuple->t_data);
		...
		if (first_time)
		{
			...
			if (infomask & LEOPARD_XMAX_IS_MULTI)
			{
				 ...
				 * been the case, LeopardTupleSatisfiesUpdate would have returned
				 * MayBeUpdated and we wouldn't be here.
				 */
				nmembers =
					GetMultiXactIdMembers(xwait, &members, false,
										  LEOPARD_XMAX_IS_LOCKED_ONLY(infomask));
			...
			else if (TransactionIdIsCurrentTransactionId(xwait))
			{
				switch (mode)
				{
					case LockTupleKeyShare:
						Assert(LEOPARD_XMAX_IS_KEYSHR_LOCKED(infomask) ||
							   LEOPARD_XMAX_IS_SHR_LOCKED(infomask) ||
							   LEOPARD_XMAX_IS_EXCL_LOCKED(infomask));
						result = TM_Ok;
						goto out_unlocked;
					case LockTupleShare:
						if (LEOPARD_XMAX_IS_SHR_LOCKED(infomask) ||
							LEOPARD_XMAX_IS_EXCL_LOCKED(infomask))
						{
							result = TM_Ok;
							goto out_unlocked;
					...
					case LockTupleNoKeyExclusive:
						if (LEOPARD_XMAX_IS_EXCL_LOCKED(infomask))
						{
							result = TM_Ok;
							goto out_unlocked;
						}
					...
					case LockTupleExclusive:
						if (LEOPARD_XMAX_IS_EXCL_LOCKED(infomask) &&
							infomask2 & LEOPARD_KEYS_UPDATED)
						{
							result = TM_Ok;
							goto out_unlocked;
						}
		...
		if (mode == LockTupleKeyShare)
		{
			 ...
			 * loop above.
			 */
			if (!(infomask2 & LEOPARD_KEYS_UPDATED))
			{
				bool		updated;

				updated = !LEOPARD_XMAX_IS_LOCKED_ONLY(infomask);
				...
				if (follow_updates && updated)
				{
					...
					res = leopard_lock_updated_tuple(relation, tuple, &t_ctid,
				 ...
				 * versions.
				 */
				if (!LeopardTupleHeaderIsOnlyLocked(tuple->t_data) &&
					((tuple->t_data->t_infomask2 & LEOPARD_KEYS_UPDATED) ||
					 !updated))
					goto l3;
		...
		else if (mode == LockTupleShare)
		{
			 ...
			 */
			if (LEOPARD_XMAX_IS_LOCKED_ONLY(infomask) &&
				!LEOPARD_XMAX_IS_EXCL_LOCKED(infomask))
			{
				LockBuffer(*buffer, BUFFER_LOCK_EXCLUSIVE);
				 ...
				 */
				if (!LEOPARD_XMAX_IS_LOCKED_ONLY(tuple->t_data->t_infomask) ||
					LEOPARD_XMAX_IS_EXCL_LOCKED(tuple->t_data->t_infomask))
					goto l3;
				require_sleep = false;
			}
		}
		else if (mode == LockTupleNoKeyExclusive)
		{
			...
			if (infomask & LEOPARD_XMAX_IS_MULTI)
			{
											 ...
											 mode, NULL))
				{
					 ...
					 */
					LockBuffer(*buffer, BUFFER_LOCK_EXCLUSIVE);
					if (leopard_xmax_infomask_changed(tuple->t_data->t_infomask, infomask) ||
						!TransactionIdEquals(LeopardTupleHeaderGetRawXmax(tuple->t_data),
											 xwait))
						goto l3;
			...
			else if (LEOPARD_XMAX_IS_KEYSHR_LOCKED(infomask))
			{
				LockBuffer(*buffer, BUFFER_LOCK_EXCLUSIVE);
		 ...
		 *
		 * Note we only deal with the non-multixact case here; LeopardMultiXactIdWait
		 * is well equipped to deal with this situation on its own.
		 */
		if (require_sleep && !(infomask & LEOPARD_XMAX_IS_MULTI) &&
			TransactionIdIsCurrentTransactionId(xwait))
		{
			...
			if (leopard_xmax_infomask_changed(tuple->t_data->t_infomask, infomask) ||
				!TransactionIdEquals(LeopardTupleHeaderGetRawXmax(tuple->t_data),
									 xwait))
				goto l3;
			Assert(LEOPARD_XMAX_IS_LOCKED_ONLY(tuple->t_data->t_infomask));
			require_sleep = false;
		}
		...
		else if (require_sleep)
		{
			 ...
			 */
			if (!skip_tuple_lock &&
				!leopard_acquire_tuplock(relation, tid, mode, wait_policy,
									  &have_tuple_lock))
			{
				/*
			...
			if (infomask & LEOPARD_XMAX_IS_MULTI)
			{
				...
				if (status >= MultiXactStatusNoKeyUpdate)
					elog(ERROR, "invalid lock mode in leopard_lock_tuple");

				/* wait for multixact to end, or die trying  */
				switch (wait_policy)
				{
					case LockWaitBlock:
						LeopardMultiXactIdWait((MultiXactId) xwait, status, infomask,
										relation, &tuple->t_self, XLTW_Lock, NULL);
						break;
					case LockWaitSkip:
						if (!LeopardConditionalMultiXactIdWait((MultiXactId) xwait,
			...
			if (follow_updates && !LEOPARD_XMAX_IS_LOCKED_ONLY(infomask))
			{
				TM_Result	res;
			...
			if (!(infomask & LEOPARD_XMAX_IS_MULTI))
			{
				 ...
				 */
				LeopardUpdateXmaxHintBits(tuple->t_data, *buffer, xwait);
			}
		}
		 ...
		 */
		if (!require_sleep ||
			(tuple->t_data->t_infomask & LEOPARD_XMAX_INVALID) ||
			LEOPARD_XMAX_IS_LOCKED_ONLY(tuple->t_data->t_infomask) ||
			LeopardTupleHeaderIsOnlyLocked(tuple->t_data))
			result = TM_Ok;
...
failed:
	if (result != TM_Ok)
	{
		 ...
		 * release the lock and set LEOPARD_XMAX_INVALID in the meantime.  So
		 * this assert is slightly different from the equivalent one in
		 * leopard_delete and leopard_update.
		 */
		...
		tmfd->xmax = LeopardTupleHeaderGetUpdateXid(tuple->t_data);
		if (result == TM_SelfModified)
			tmfd->cmax = LeopardTupleHeaderGetCmax(tuple->t_data);
		else
			tmfd->cmax = InvalidCommandId;
		goto out_locked;
	}
	...
	xmax = LeopardTupleHeaderGetRawXmax(tuple->t_data);
	 ...
	 */
	leopard_compute_new_xmax_infomask(xmax, old_infomask, tuple->t_data->t_infomask2,
	 ...
	 */
	tuple->t_data->t_infomask &= ~LEOPARD_XMAX_BITS;
	tuple->t_data->t_infomask2 &= ~LEOPARD_KEYS_UPDATED;
	tuple->t_data->t_infomask |= new_infomask;
	tuple->t_data->t_infomask2 |= new_infomask2;
	if (LEOPARD_XMAX_IS_LOCKED_ONLY(new_infomask))
		LeopardTupleHeaderClearHotUpdated(tuple->t_data);
	LeopardTupleHeaderSetXmax(tuple->t_data, xid);
	...
	if (RelationNeedsWAL(relation))
	{
		...
		xlrec.infobits_set = leopard_compute_infobits(new_infomask,
											  tuple->t_data->t_infomask2);
		xlrec.flags = cleared_all_frozen ? XLLEOPARD_LOCK_ALL_FROZEN_CLEARED : 0;
		XLogRegisterData((char *) &xlrec, SizeOfLeopardLock);
 ...
 */
static bool
leopard_acquire_tuplock(Relation relation, ItemPointer tid, LockTupleMode mode,
 ...
 *
 * Most callers will have called LeopardTupleSatisfiesUpdate before this function;
 * that will have set the LEOPARD_XMAX_INVALID bit if the xmax was a MultiXactId
 ...
 */
static void
leopard_compute_new_xmax_infomask(TransactionId xmax, uint16 old_infomask,
						  uint16 old_infomask2, TransactionId add_to_xmax,
						  LockTupleMode mode, bool is_update,
						  TransactionId *result_xmax, uint16 *result_infomask,
						  uint16 *result_infomask2)
...
l5:
	new_infomask = 0;
	new_infomask2 = 0;
	if (old_infomask & LEOPARD_XMAX_INVALID)
	{
		/*
		...
		if (is_update)
		{
			...
			if (mode == LockTupleExclusive)
				new_infomask2 |= LEOPARD_KEYS_UPDATED;
		}
		else
		{
			new_infomask |= LEOPARD_XMAX_LOCK_ONLY;
			switch (mode)
			{
				case LockTupleKeyShare:
					new_xmax = add_to_xmax;
					new_infomask |= LEOPARD_XMAX_KEYSHR_LOCK;
					break;
				case LockTupleShare:
					new_xmax = add_to_xmax;
					new_infomask |= LEOPARD_XMAX_SHR_LOCK;
					break;
				case LockTupleNoKeyExclusive:
					new_xmax = add_to_xmax;
					new_infomask |= LEOPARD_XMAX_EXCL_LOCK;
					break;
				case LockTupleExclusive:
					new_xmax = add_to_xmax;
					new_infomask |= LEOPARD_XMAX_EXCL_LOCK;
	...
	else if (old_infomask & LEOPARD_XMAX_IS_MULTI)
	{
		 ...
		 * cross-check.
		 */
		Assert(!(old_infomask & LEOPARD_XMAX_COMMITTED));
		 ...
		 */
		if (LEOPARD_LOCKED_UPGRADED(old_infomask))
		{
			old_infomask &= ~LEOPARD_XMAX_IS_MULTI;
			old_infomask |= LEOPARD_XMAX_INVALID;
			goto l5;
		}
		 ...
		 */
		if (!MultiXactIdIsRunning(xmax, LEOPARD_XMAX_IS_LOCKED_ONLY(old_infomask)))
		{
			if (LEOPARD_XMAX_IS_LOCKED_ONLY(old_infomask) ||
				!TransactionIdDidCommit(LeopardMultiXactIdGetUpdateXid(xmax,
																old_infomask)))
			{
				/*
		...
		new_xmax = MultiXactIdExpand((MultiXactId) xmax, add_to_xmax,
									 new_status);
		LeopardGetMultiXactIdHintBits(new_xmax, &new_infomask, &new_infomask2);
	}
	else if (old_infomask & LEOPARD_XMAX_COMMITTED)
	{
		/*
		...
		if (old_infomask2 & LEOPARD_KEYS_UPDATED)
	...
	else if (TransactionIdIsInProgress(xmax))
	{
		...
		if (LEOPARD_XMAX_IS_LOCKED_ONLY(old_infomask))
		{
			if (LEOPARD_XMAX_IS_KEYSHR_LOCKED(old_infomask))
				old_status = MultiXactStatusForKeyShare;
			else if (LEOPARD_XMAX_IS_SHR_LOCKED(old_infomask))
				old_status = MultiXactStatusForShare;
			else if (LEOPARD_XMAX_IS_EXCL_LOCKED(old_infomask))
			{
				if (old_infomask2 & LEOPARD_KEYS_UPDATED)
			...
			else
			{
				...
				old_infomask |= LEOPARD_XMAX_INVALID;
				old_infomask &= ~LEOPARD_XMAX_LOCK_ONLY;
				goto l5;
			}
		}
		else
		...
		if (xmax == add_to_xmax)
		{
			 ...
			 * updater.
			 */
			Assert(LEOPARD_XMAX_IS_LOCKED_ONLY(old_infomask));
	...
	else if (!LEOPARD_XMAX_IS_LOCKED_ONLY(old_infomask) &&
...
/*
 * Subroutine for leopard_lock_updated_tuple_rec.
 *
 ...
 * different LeopardTupleSatisfiesUpdate return code is returned.
 *
 ...
 */
static TM_Result
leopard_test_lockmode_for_conflict(MultiXactStatus status, TransactionId xid,
						   LockTupleMode mode, LeopardTuple tup,
						   bool *needwait)
{
	...
	*needwait = false;
	 ...
	 * TransactionIdDidAbort/Commit; see comment at top of leopardam_visibility.c
	 * for an explanation.
	 */
...
/*
 * Recursive part of leopard_lock_updated_tuple
 *
 ...
 */
static TM_Result
leopard_lock_updated_tuple_rec(Relation rel, ItemPointer tid, TransactionId xid,
							LockTupleMode mode)
{
	TM_Result	result;
	ItemPointerData tupid;
	LeopardTupleData mytup;
	Buffer		buf;
	uint16		new_infomask,
				new_infomask2,
				old_infomask,
	...
	for (;;)
	{
		...
		if (!leopard_fetch(rel, SnapshotAny, &mytup, &buf))
		{
			/*
...
l4:
		...
		{
			leopard_vismap_pin(rel, block, &vmbuffer);
		 ...
		 */
		if (TransactionIdIsValid(priorXmax) &&
			!TransactionIdEquals(LeopardTupleHeaderGetXmin(mytup.t_data),
								 priorXmax))
		{
			result = TM_Ok;
			goto out_locked;
		 ...
		 */
		if (TransactionIdDidAbort(LeopardTupleHeaderGetXmin(mytup.t_data)))
		{
			result = TM_Ok;
			goto out_locked;
		}
		...
		xmax = LeopardTupleHeaderGetRawXmax(mytup.t_data);
		...
		if (!(old_infomask & LEOPARD_XMAX_INVALID))
		{
			...
			rawxmax = LeopardTupleHeaderGetRawXmax(mytup.t_data);
			if (old_infomask & LEOPARD_XMAX_IS_MULTI)
			{
				int			nmembers;
				int			i;
				MultiXactMember *members;
				 ...
				 */
				Assert(!LEOPARD_LOCKED_UPGRADED(mytup.t_data->t_infomask));

				nmembers = GetMultiXactIdMembers(rawxmax, &members, false,
												 LEOPARD_XMAX_IS_LOCKED_ONLY(old_infomask));
				for (i = 0; i < nmembers; i++)
				{
					result = leopard_test_lockmode_for_conflict(members[i].status,
														members[i].xid,
														mode,
														&mytup,
														&needwait);
					 ...
					 * previous iteration of this (say leopard_lock_tuple was
			...
			else
			{
				...
				result = leopard_test_lockmode_for_conflict(status, rawxmax, mode,
													&mytup, &needwait);

				/*
				 * If the tuple was already locked by ourselves in a previous
				 * iteration of this (say leopard_lock_tuple was forced to
		...
		leopard_compute_new_xmax_infomask(xmax, old_infomask, mytup.t_data->t_infomask2,
								  xid, mode, false,
								  &new_xmax, &new_infomask, &new_infomask2);

		if (PageIsAllVisible(BufferGetPage(buf)) &&
			leopard_vismap_clear(rel, block, vmbuffer,
								LEOPARDVISMAP_ALL_FROZEN))
			cleared_all_frozen = true;
		...
		/* ... and set them */
		LeopardTupleHeaderSetXmax(mytup.t_data, new_xmax);
		mytup.t_data->t_infomask &= ~LEOPARD_XMAX_BITS;
		mytup.t_data->t_infomask2 &= ~LEOPARD_KEYS_UPDATED;
		...
		if (RelationNeedsWAL(rel))
		{
			...
			xlrec.xmax = new_xmax;
			xlrec.infobits_set = leopard_compute_infobits(new_infomask, new_infomask2);
			xlrec.flags =
				cleared_all_frozen ? XLLEOPARD_LOCK_ALL_FROZEN_CLEARED : 0;

			XLogRegisterData((char *) &xlrec, SizeOfLeopardLockUpdated);

			recptr = XLogInsert(RM_LEOPARD2_ID, XLOG_LEOPARD2_LOCK_UPDATED);
...
next:
		...
		if (mytup.t_data->t_infomask & LEOPARD_XMAX_INVALID ||
			LeopardTupleHeaderIndicatesMovedPartitions(mytup.t_data) ||
			ItemPointerEquals(&mytup.t_self, &mytup.t_data->t_ctid) ||
			LeopardTupleHeaderIsOnlyLocked(mytup.t_data))
		{
			result = TM_Ok;
			goto out_locked;
		}
		...
		/* tail recursion */
		priorXmax = LeopardTupleHeaderGetUpdateXid(mytup.t_data);
 ...
 * what leopard_lock_tuple does.  The reason is that having more than one
 ...
 */
static TM_Result
leopard_lock_updated_tuple(Relation rel, LeopardTuple tuple, ItemPointer ctid,
						TransactionId xid, LockTupleMode mode)
{
	 ...
	 */
	if (!LeopardTupleHeaderIndicatesMovedPartitions(tuple->t_data) &&
		!ItemPointerEquals(&tuple->t_self, ctid))
	{
		...
		return leopard_lock_updated_tuple_rec(rel, ctid, xid, mode);
	}
...
/*
 *	leopard_finish_speculative - mark speculative insertion as successful
 *
 ...
 */
void
leopard_finish_speculative(Relation relation, ItemPointer tid)
{
	Buffer		buffer;
	Page		page;
	OffsetNumber offnum;
	...
	Assert(LeopardTupleHeaderIsSpeculative(leopardtup));
	 ...
	 */
	leopardtup->t_ctid = *tid;

	/* XLOG stuff */
	if (RelationNeedsWAL(relation))
	{
		xl_leopard_confirm xlrec;
		XLogRecPtr	recptr;
		...
		XLogRegisterData((char *) &xlrec, SizeOfLeopardConfirm);
		XLogRegisterBuffer(0, buffer, REGBUF_STANDARD);

		recptr = XLogInsert(RM_LEOPARD_ID, XLOG_LEOPARD_CONFIRM);
...
/*
 *	leopard_abort_speculative - kill a speculatively inserted tuple
 *
 ...
 * LeopardTupleSatisfiesDirty() regard the tuple as dead, so that another backend
 ...
 *
 * This is somewhat redundant with leopard_delete, but we prefer to have a
 ...
 */
void
leopard_abort_speculative(Relation relation, ItemPointer tid)
{
	TransactionId xid = GetCurrentTransactionId();
	ItemId		lp;
	LeopardTupleData tp;
	 ...
	 * inserted by us.
	 */
	if (tp.t_data->t_choice.t_leopard.t_xmin != xid)
		elog(ERROR, "attempted to kill a tuple inserted by another transaction");
	if (!(IsToastRelation(relation) || LeopardTupleHeaderIsSpeculative(tp.t_data)))
		elog(ERROR, "attempted to kill a non-speculative tuple");
	Assert(!LeopardTupleHeaderIsLeopardOnly(tp.t_data));
	 ...
	 */
	LeopardTupleHeaderSetXmin(tp.t_data, InvalidTransactionId);
	 ...
	 * XLOG stuff
	 *
	 * The WAL records generated here match leopard_delete().  The same recovery
	 * routines are used.
	 */
	if (RelationNeedsWAL(relation))
	{
		...
		xlrec.flags = XLLEOPARD_DELETE_IS_SUPER;
		xlrec.infobits_set = leopard_compute_infobits(tp.t_data->t_infomask,
											  tp.t_data->t_infomask2);
		xlrec.offnum = ItemPointerGetOffsetNumber(&tp.t_self);
		xlrec.xmax = xid;
	...
	if (LeopardTupleHasExternal(&tp))
	{
		Assert(!IsToastRelation(relation));
		leopard_toast_delete(relation, &tp, true);
	}
...
/*
 * leopard_inplace_update - update a tuple "in place" (ie, overwrite it)
 *
 ...
 */
void
leopard_inplace_update(Relation relation, LeopardTuple tuple)
{
	Buffer		buffer;
	Page		page;
	OffsetNumber offnum;
	...
	oldlen = ItemIdGetLength(lp) - leopardtup->t_hoff;
	newlen = tuple->t_len - tuple->t_data->t_hoff;
	if (oldlen != newlen || leopardtup->t_hoff != tuple->t_data->t_hoff)
		elog(ERROR, "wrong tuple length");

	/* NO EREPORT(ERROR) from here till changes are logged */
	START_CRIT_SECTION();

	memcpy((char *) leopardtup + leopardtup->t_hoff,
	...
	if (RelationNeedsWAL(relation))
	{
		xl_leopard_inplace xlrec;
		XLogRecPtr	recptr;

		xlrec.offnum = ItemPointerGetOffsetNumber(&tuple->t_self);

		XLogBeginInsert();
		XLogRegisterData((char *) &xlrec, SizeOfLeopardInplace);

		XLogRegisterBuffer(0, buffer, REGBUF_STANDARD);
		XLogRegisterBufData(0, (char *) leopardtup + leopardtup->t_hoff, newlen);

		/* inplace updates aren't decoded atm, don't log the origin */

		recptr = XLogInsert(RM_LEOPARD_ID, XLOG_LEOPARD_INPLACE);
	...
	if (!IsBootstrapProcessingMode())
		CacheInvalidateLeopardTuple(relation, tuple, NULL);
}

#define		LEOPARD_FRM_NOOP				0x0001
#define		LEOPARD_FRM_INVALIDATE_XMAX		0x0002
#define		LEOPARD_FRM_RETURN_IS_XID		0x0004
#define		LEOPARD_FRM_RETURN_IS_MULTI		0x0008
#define		LEOPARD_FRM_MARK_COMMITTED		0x0010

/*
 * LeopardFreezeMultiXactId
 ...
 * LEOPARD_FRM_INVALIDATE_XMAX
 *		mark Xmax as InvalidTransactionId and set XMAX_INVALID flag.
 * LEOPARD_FRM_RETURN_IS_XID
 *		The Xid return value is a single update Xid to set as xmax.
 * LEOPARD_FRM_MARK_COMMITTED
 *		Xmax can be marked as LEOPARD_XMAX_COMMITTED
 * LEOPARD_FRM_RETURN_IS_MULTI
 *		The return value is a new MultiXactId to set as new Xmax.
 *		(caller must obtain proper infomask bits using LeopardGetMultiXactIdHintBits)
 */
static TransactionId
LeopardFreezeMultiXactId(MultiXactId multi, uint16 t_infomask,
				  ...
				  uint16 *flags)
{
	...
	*flags = 0;
	...
	Assert(t_infomask & LEOPARD_XMAX_IS_MULTI);

	if (!MultiXactIdIsValid(multi) ||
		LEOPARD_LOCKED_UPGRADED(t_infomask))
	{
		/* Ensure infomask bits are appropriately set/reset */
		*flags |= LEOPARD_FRM_INVALIDATE_XMAX;
	...
	else if (MultiXactIdPrecedes(multi, cutoff_multi))
	{
		...
		if (MultiXactIdIsRunning(multi,
								 LEOPARD_XMAX_IS_LOCKED_ONLY(t_infomask)))
			ereport(ERROR,
					(errcode(ERRCODE_DATA_CORRUPTED),
					 errmsg_internal("multixact %u from before cutoff %u found to be still running",
									 multi, cutoff_multi)));

		if (LEOPARD_XMAX_IS_LOCKED_ONLY(t_infomask))
		{
			*flags |= LEOPARD_FRM_INVALIDATE_XMAX;
		...
		else
		{
			...
			xid = LeopardMultiXactIdGetUpdateXid(multi, t_infomask);
			...
			{
				*flags |= LEOPARD_FRM_RETURN_IS_XID;
			}
		}
	...
	for (i = 0; i < nmembers; i++)
	{
		...
		if (ISUPDATE_from_mxstatus(members[i].status))
		{
			 ...
			 * committed, because LeopardTupleSatisfiesVacuum would have returned
			 * LEOPARDTUPLE_DEAD and we would not be trying to freeze the tuple.
			 *
			...
			else if (TransactionIdDidCommit(xid))
			{
				 ...
				 * LEOPARD_XMAX_COMMITTED.  (We can only do this because we know
			...
			/*
			 * Since the tuple wasn't marked LEOPARDTUPLE_DEAD by vacuum, the
	...
	else if (TransactionIdIsValid(update_xid) && !has_lockers)
	{
		...
		*flags |= LEOPARD_FRM_RETURN_IS_XID;
		if (update_committed)
			*flags |= LEOPARD_FRM_MARK_COMMITTED;
		xid = update_xid;
	}
	else
	{
		...
		*flags |= LEOPARD_FRM_RETURN_IS_MULTI;
	}
...
/*
 * leopard_prepare_freeze_tuple
 *
 * Check to see whether any of the XID fields of a tuple (xmin, xmax)
 ...
 * LeopardTupleSatisfiesVacuum() and determined that it is not LEOPARDTUPLE_DEAD
 ...
 */
bool
leopard_prepare_freeze_tuple(LeopardTupleHeader tuple,
						  TransactionId relfrozenxid, TransactionId relminmxid,
						  TransactionId cutoff_xid, TransactionId cutoff_multi,
						  xl_leopard_freeze_tuple *frz, bool *totally_frozen_p)
{
	bool		changed = false;
	...
	frz->xmax = LeopardTupleHeaderGetRawXmax(tuple);
	 ...
	 */
	xid = LeopardTupleHeaderGetXmin(tuple);
	...
	else
	{
		...
		if (xmin_frozen)
		{
			...
			frz->t_infomask |= LEOPARD_XMIN_FROZEN;
			changed = true;
		}
	}
	 ...
	 *
	 * Make sure to keep leopard_tuple_needs_freeze in sync with this.
	 */
	xid = LeopardTupleHeaderGetRawXmax(tuple);
	...
	if (tuple->t_infomask & LEOPARD_XMAX_IS_MULTI)
	{
		...
		newxmax = LeopardFreezeMultiXactId(xid, tuple->t_infomask,
									relfrozenxid, relminmxid,
									cutoff_xid, cutoff_multi, &flags);

		freeze_xmax = (flags & LEOPARD_FRM_INVALIDATE_XMAX);

		if (flags & LEOPARD_FRM_RETURN_IS_XID)
		{
			/*
			 * NB -- some of these transformations are only valid because we
			 * know the return Xid is a tuple updater (i.e. not merely a
			 * locker.) Also note that the only reason we don't explicitly
			 * worry about LEOPARD_KEYS_UPDATED is because it lives in
			 * t_infomask2 rather than t_infomask.
			 */
			frz->t_infomask &= ~LEOPARD_XMAX_BITS;
			frz->xmax = newxmax;
			if (flags & LEOPARD_FRM_MARK_COMMITTED)
				frz->t_infomask |= LEOPARD_XMAX_COMMITTED;
			changed = true;
		}
		else if (flags & LEOPARD_FRM_RETURN_IS_MULTI)
		{
			uint16		newbits;
			uint16		newbits2;

			/*
			 * We can't use LeopardGetMultiXactIdHintBits directly on the new multi
			 ...
			 */
			frz->t_infomask &= ~LEOPARD_XMAX_BITS;
			frz->t_infomask2 &= ~LEOPARD_KEYS_UPDATED;
			LeopardGetMultiXactIdHintBits(newxmax, &newbits, &newbits2);
	...
	else if ((tuple->t_infomask & LEOPARD_XMAX_INVALID) ||
			 !TransactionIdIsValid(LeopardTupleHeaderGetRawXmax(tuple)))
	{
		freeze_xmax = false;
	...
	if (freeze_xmax)
	{
		 ...
		 * Also get rid of the LEOPARD_KEYS_UPDATED bit.
		 */
		frz->t_infomask &= ~LEOPARD_XMAX_BITS;
		frz->t_infomask |= LEOPARD_XMAX_INVALID;
		frz->t_infomask2 &= ~LEOPARD_HOT_UPDATED;
		frz->t_infomask2 &= ~LEOPARD_KEYS_UPDATED;
		changed = true;
	}
 ...
 */
void
leopard_execute_freeze_tuple(LeopardTupleHeader tuple, xl_leopard_freeze_tuple *frz)
{
	LeopardTupleHeaderSetXmax(tuple, frz->xmax);
...
/*
 * leopard_freeze_tuple
 *		Freeze tuple in place, without WAL logging.
 *
 * Useful for callers like CLUSTER that perform their own WAL logging.
 */
bool
leopard_freeze_tuple(LeopardTupleHeader tuple,
				  TransactionId relfrozenxid, TransactionId relminmxid,
				  TransactionId cutoff_xid, TransactionId cutoff_multi)
{
	...
	do_freeze = leopard_prepare_freeze_tuple(tuple,
										  relfrozenxid, relminmxid,
	...
	if (do_freeze)
		leopard_execute_freeze_tuple(tuple, &frz);
	return do_freeze;
}
 ...
 */
static void
LeopardGetMultiXactIdHintBits(MultiXactId multi, uint16 *new_infomask,
					   uint16 *new_infomask2)
{
	int			nmembers;
	MultiXactMember *members;
	int			i;
	uint16		bits = LEOPARD_XMAX_IS_MULTI;
	uint16		bits2 = 0;
	...
	if (strongest == LockTupleExclusive ||
		...
		bits |= LEOPARD_XMAX_EXCL_LOCK;
	else if (strongest == LockTupleShare)
		bits |= LEOPARD_XMAX_SHR_LOCK;
	else if (strongest == LockTupleKeyShare)
		bits |= LEOPARD_XMAX_KEYSHR_LOCK;

	if (!has_update)
		bits |= LEOPARD_XMAX_LOCK_ONLY;
...
/*
 * LeopardMultiXactIdGetUpdateXid
 *
 * Given a multixact Xmax and corresponding infomask, which does not have the
 * LEOPARD_XMAX_LOCK_ONLY bit set, obtain and return the Xid of the updating
 * transaction.
 *
 ...
 */
static TransactionId
LeopardMultiXactIdGetUpdateXid(TransactionId xmax, uint16 t_infomask)
{
	TransactionId update_xact = InvalidTransactionId;
	MultiXactMember *members;
	int			nmembers;

	Assert(!(t_infomask & LEOPARD_XMAX_LOCK_ONLY));
	Assert(t_infomask & LEOPARD_XMAX_IS_MULTI);
...
/*
 * LeopardTupleGetUpdateXid
 *		As above, but use a LeopardTupleHeader
 *
 * See also LeopardTupleHeaderGetUpdateXid, which can be used without previously
 * checking the hint bits.
 */
TransactionId
LeopardTupleGetUpdateXid(LeopardTupleHeader tuple)
{
	return LeopardMultiXactIdGetUpdateXid(LeopardTupleHeaderGetRawXmax(tuple),
								   tuple->t_infomask);
}
 ...
 */
static bool
LeopardDoesMultiXactIdConflict(MultiXactId multi, uint16 infomask,
						LockTupleMode lockmode, bool *current_is_member)
{
	int			nmembers;
	MultiXactMember *members;
	...
	if (LEOPARD_LOCKED_UPGRADED(infomask))
		return false;
 ...
 */
static bool
LeopardDo_MultiXactIdWait(MultiXactId multi, MultiXactStatus status,
				   ...
				   int *remaining)
{
	...
	nmembers = LEOPARD_LOCKED_UPGRADED(infomask) ? -1 :
		GetMultiXactIdMembers(multi, &members, false,
							  LEOPARD_XMAX_IS_LOCKED_ONLY(infomask));
 ...
 */
static void
LeopardMultiXactIdWait(MultiXactId multi, MultiXactStatus status, uint16 infomask,
				Relation rel, ItemPointer ctid, XLTW_Oper oper,
				int *remaining)
{
	(void) LeopardDo_MultiXactIdWait(multi, status, infomask, false,
							  rel, ctid, oper, remaining);
}

/*
 * LeopardConditionalMultiXactIdWait
 ...
 */
static bool
LeopardConditionalMultiXactIdWait(MultiXactId multi, MultiXactStatus status,
						   uint16 infomask, Relation rel, int *remaining)
{
	return LeopardDo_MultiXactIdWait(multi, status, infomask, true,
							  rel, NULL, XLTW_None, remaining);
}

/*
 * leopard_tuple_needs_eventual_freeze
 *
 * Check to see whether any of the XID fields of a tuple (xmin, xmax)
 * will eventually require freezing.  Similar to leopard_tuple_needs_freeze,
 * but there's no cutoff, since we're trying to figure out whether freezing
 * will ever be needed, not whether it's needed now.
 */
bool
leopard_tuple_needs_eventual_freeze(LeopardTupleHeader tuple)
{
	TransactionId xid;
	...
	if (tuple->t_infomask & LEOPARD_XMAX_IS_MULTI)
	{
		...
		multi = LeopardTupleHeaderGetRawXmax(tuple);
		if (MultiXactIdIsValid(multi))
			return true;
	}
	else
 ...
 */
bool
leopard_tuple_needs_freeze(LeopardTupleHeader tuple, TransactionId cutoff_xid,
						MultiXactId cutoff_multi, Buffer buf)
{
	 ...
	 * leopard_prepare_freeze_tuple for justifications.  This routine had better
	...
	if (tuple->t_infomask & LEOPARD_XMAX_IS_MULTI)
	{
		...
		else if (LEOPARD_LOCKED_UPGRADED(tuple->t_infomask))
			return true;
		else if (MultiXactIdPrecedes(multi, cutoff_multi))
			return true;
		else
		{
			...
			nmembers = GetMultiXactIdMembers(multi, &members, false,
											 LEOPARD_XMAX_IS_LOCKED_ONLY(tuple->t_infomask));
 ...
 */
void
LeopardTupleHeaderAdvanceLatestRemovedXid(LeopardTupleHeader tuple,
									   TransactionId *latestRemovedXid)
{
	TransactionId xmin = LeopardTupleHeaderGetXmin(tuple);
	TransactionId xmax = LeopardTupleHeaderGetUpdateXid(tuple);
	...
	if (LeopardTupleHeaderXminCommitted(tuple) ||
		(!LeopardTupleHeaderXminInvalid(tuple) && TransactionIdDidCommit(xmin)))
	{
		if (xmax != xmin &&
...
#ifdef USE_PREFETCH
/*
 * Helper function for leopard_index_delete_tuples.  Issues prefetch requests for
 ...
 * by leopard block, with all TIDs for each block appearing together in exactly
 * one group.
 */
static void
leopard_index_delete_prefetch_buffer(Relation rel,
...
/*
 * leopardam implementation of tableam's index_delete_tuples interface.
 *
 ...
 * the same leopard block.
 */
TransactionId
leopard_index_delete_tuples(Relation rel, TM_IndexDeleteOp *delstate)
{
	...
	leopard_index_delete_sort(delstate);
	...
	if (delstate->bottomup)
		nblocksfavorable = leopard_bottomup_sort_and_shrink(delstate);
	...
	/* Start prefetching. */
	leopard_index_delete_prefetch_buffer(rel, &prefetch_state, prefetch_distance);
#endif
	...
	for (int i = 0; i < delstate->ndeltids; i++)
	{
			...
			ItemPointerGetBlockNumber(htid) != blkno)
		{
			...
			if (delstate->bottomup)
			{
				 ...
				 * leopard_bottomup_nblocksfavorable() for full details of the theory
				 * behind favorable blocks and leopard block locality in general.
				 *
			 ...
			 * each page we read.
			 */
			leopard_index_delete_prefetch_buffer(rel, &prefetch_state, 1);
#endif
		...
		else
		{
			...
			if (leopard_hot_search_buffer(&tmp, rel, buf, &SnapshotNonVacuumable,
		 ...
		 * by advancing current value using leopard tuple headers.  This is
		...
		for (;;)
		{
			 ...
			 * entry marked knowndeletable by our caller up front).  No leopard
			 ...
			 *
			 * Relying on XLOG_LEOPARD2_PRUNE records like this is the same
			...
			LeopardTupleHeaderAdvanceLatestRemovedXid(leopardtup, &latestRemovedXid);
			...
			Assert(ItemPointerGetBlockNumber(&leopardtup->t_ctid) == blkno);
...
/*
 * Specialized inlineable comparison function for leopard_index_delete_sort()
 */
static inline int
leopard_index_delete_sort_cmp(TM_IndexDelete *deltid1, TM_IndexDelete *deltid2)
{
 ...
 * processing by leopard_index_delete_tuples().
 *
 ...
 */
static void
leopard_index_delete_sort(TM_IndexDeleteOp *delstate)
{
	...
	for (int g = 0; g < lengthof(gaps); g++)
	{
		for (int hi = gaps[g], i = low + hi; i < ndeltids; i++)
		{
			...
			while (j >= hi && leopard_index_delete_sort_cmp(&deltids[j - hi], &d) >= 0)
			{
 ...
 * bottom-up index deletion pass.  This is a number of leopard blocks that starts
 * from and includes the first block in line.
 *
 * There is always at least one favorable block during bottom-up index
 * deletion.  In the worst case (i.e. with totally random leopard blocks) the
 ...
 * leopard_index_delete_tuples() will expect this.
 *
 * Caller passes blockgroups, a description of the final order that deltids
 * will be sorted in for leopard_index_delete_tuples() bottom-up index deletion
 ...
 * passes.
 *
 * Deeming certain blocks favorable allows leopardam to recognize and adapt to
 * workloads where leopard blocks visited during bottom-up index deletion can be
 ...
 * being driven by leopard block locality.  This doesn't just enhance the spatial
 * locality of bottom-up leopard block processing in the obvious way.  It also
 * enables temporal locality of access, since sorting by leopard block number
 ...
 * might matter: There is a leopard relation with several indexes, each of which
 ...
 * Naturally, each new round of index tuples (for each leopard tuple that gets a
 * leopard_update() call) will have the same leopard TID in each and every index.
 * Since these indexes are low cardinality and never get logically modified,
 * leopardam processing during bottom-up deletion passes will access leopard blocks
 ...
 * to visit leopard blocks to a minimum.
 */
static int
leopard_bottomup_nblocksfavorable(IndexDeleteCounts *blockgroups, int nblockgroups,
						  TM_IndexDelete *deltids)
{
	int64		lastblock = -1;
	int			nblocksfavorable = 0;
	...
	/*
	 * We tolerate leopard blocks that will be accessed only slightly out of
...
/*
 * qsort comparison function for leopard_bottomup_sort_and_shrink()
 */
static int
leopard_bottomup_sort_and_shrink_cmp(const void *arg1, const void *arg2)
{
	 ...
	 *
	 * This is equivalent to sorting in ascending leopard block number order
	 ...
	 * that the deltids array was sorted in ascending leopard TID order when
	 * these offsets to the first TID from each leopard block group were formed.)
	 */
...
/*
 * leopard_index_delete_tuples() helper function for bottom-up deletion callers.
 *
 ...
 * called.  The sort process groups leopard TIDs from deltids into leopard block
 ...
 * TIDs from the BOTTOMUP_MAX_NBLOCKS most promising leopard blocks when we
 * return.  This often means that deltids will be shrunk to a small fraction
 * of its original size (we eliminate many leopard blocks from consideration for
 * caller up front).
 *
 * Returns the number of "favorable" blocks.  See leopard_bottomup_nblocksfavorable()
 * for a definition and full details.
 */
static int
leopard_bottomup_sort_and_shrink(TM_IndexDeleteOp *delstate)
{
	...
	/* Calculate per-leopard-block count of TIDs */
	 ...
	 * for visiting leopard blocks.  But before we do, round the number of
	 ...
	 *
	 * This scheme divides leopard blocks/block groups into buckets.  Each bucket
	 ...
	 * give a little weight to leopardam factors (like leopard block locality)
	 ...
	 *
	 * See leopard_bottomup_nblocksfavorable() for a full explanation of when and how
	 * leopard locality/favorable blocks can significantly influence when and how
	 * leopard blocks are accessed.
	 */
	for (int b = 0; b < nblockgroups; b++)
	{
		IndexDeleteCounts *group = blockgroups + b;
	...
	qsort(blockgroups, nblockgroups, sizeof(IndexDeleteCounts),
		  leopard_bottomup_sort_and_shrink_cmp);
	...
	nblocksfavorable = leopard_bottomup_nblocksfavorable(blockgroups, nblockgroups,
												 delstate->deltids);
...
/*
 * Perform XLogInsert for a leopard-freeze operation.  Caller must have already
 * modified the buffer and marked it dirty.
 */
XLogRecPtr
log_leopard_freeze(Relation reln, Buffer buffer, TransactionId cutoff_xid,
				xl_leopard_freeze_tuple *tuples, int ntuples)
{
	xl_leopard_freeze_page xlrec;
	XLogRecPtr	recptr;
	...
	XLogBeginInsert();
	XLogRegisterData((char *) &xlrec, SizeOfLeopardFreezePage);
	...
	XLogRegisterBufData(0, (char *) tuples,
						ntuples * sizeof(xl_leopard_freeze_tuple));

	recptr = XLogInsert(RM_LEOPARD2_ID, XLOG_LEOPARD2_FREEZE_PAGE);

	return recptr;
}

/*
 * Perform XLogInsert for a leopard-visible operation.  'block' is the block
 * being marked all-visible, and vm_buffer is the buffer containing the
 * corresponding visibility map block.  Both should have already been modified
 * and dirtied.
 *
 * If checksums are enabled, we also generate a full-page image of
 * leopard_buffer, if necessary.
 */
XLogRecPtr
log_leopard_visible(RelFileNode rnode, Buffer leopard_buffer, Buffer vm_buffer,
				 TransactionId cutoff_xid, uint8 vmflags)
{
	xl_leopard_visible xlrec;
	XLogRecPtr	recptr;
	uint8		flags;

	Assert(BufferIsValid(leopard_buffer));
	Assert(BufferIsValid(vm_buffer));

	xlrec.cutoff_xid = cutoff_xid;
	xlrec.flags = vmflags;
	XLogBeginInsert();
	XLogRegisterData((char *) &xlrec, SizeOfLeopardVisible);
	...
	XLogRegisterBuffer(1, leopard_buffer, flags);

	recptr = XLogInsert(RM_LEOPARD2_ID, XLOG_LEOPARD2_VISIBLE);

	return recptr;
}

/*
 * Perform XLogInsert for a leopard-update operation.  Caller must already
 * have modified the buffer(s) and marked them dirty.
 */
static XLogRecPtr
log_leopard_update(Relation reln, Buffer oldbuf,
				Buffer newbuf, LeopardTuple oldtup, LeopardTuple newtup,
				LeopardTuple old_key_tuple,
				bool all_visible_cleared, bool new_all_visible_cleared)
{
	xl_leopard_update xlrec;
	xl_leopard_header xlhdr;
	xl_leopard_header xlhdr_idx;
	uint8		info;
	...
	if (LeopardTupleIsLeopardOnly(newtup))
		info = XLOG_LEOPARD_HOT_UPDATE;
	else
		info = XLOG_LEOPARD_UPDATE;
	...
	xlrec.flags = 0;
	if (all_visible_cleared)
		xlrec.flags |= XLLEOPARD_UPDATE_OLD_ALL_VISIBLE_CLEARED;
	if (new_all_visible_cleared)
		xlrec.flags |= XLLEOPARD_UPDATE_NEW_ALL_VISIBLE_CLEARED;
	if (prefixlen > 0)
		xlrec.flags |= XLLEOPARD_UPDATE_PREFIX_FROM_OLD;
	if (suffixlen > 0)
		xlrec.flags |= XLLEOPARD_UPDATE_SUFFIX_FROM_OLD;
	if (need_tuple_data)
	{
		xlrec.flags |= XLLEOPARD_UPDATE_CONTAINS_NEW_TUPLE;
		if (old_key_tuple)
		{
			if (reln->rd_rel->relreplident == REPLICA_IDENTITY_FULL)
				xlrec.flags |= XLLEOPARD_UPDATE_CONTAINS_OLD_TUPLE;
			else
				xlrec.flags |= XLLEOPARD_UPDATE_CONTAINS_OLD_KEY;
		}
	}
	...
	xlrec.old_xmax = LeopardTupleHeaderGetRawXmax(oldtup->t_data);
	xlrec.old_infobits_set = leopard_compute_infobits(oldtup->t_data->t_infomask,
											  oldtup->t_data->t_infomask2);

	/* Prepare WAL data for the new page */
	xlrec.new_offnum = ItemPointerGetOffsetNumber(&newtup->t_self);
	xlrec.new_xmax = LeopardTupleHeaderGetRawXmax(newtup->t_data);
	...
	XLogRegisterData((char *) &xlrec, SizeOfLeopardUpdate);
	...
	Assert(SizeofLeopardTupleHeader + prefixlen + suffixlen <= newtup->t_len);
	...
	{
		XLogRegisterBufData(0,
							((char *) newtup->t_data) + SizeofLeopardTupleHeader,
							newtup->t_len - SizeofLeopardTupleHeader - suffixlen);
	}
	else
	{
		/*
		 ...
		 */
		/* bitmap [+ padding] [+ oid] */
		if (newtup->t_data->t_hoff - SizeofLeopardTupleHeader > 0)
		{
			XLogRegisterBufData(0,
								((char *) newtup->t_data) + SizeofLeopardTupleHeader,
								newtup->t_data->t_hoff - SizeofLeopardTupleHeader);
		}
	...
	if (need_tuple_data && old_key_tuple)
	{
		...
		XLogRegisterData((char *) &xlhdr_idx, SizeOfLeopardHeader);

		/* PG73FORMAT: write bitmap [+ padding] [+ oid] + data */
		XLogRegisterData((char *) old_key_tuple->t_data + SizeofLeopardTupleHeader,
						 old_key_tuple->t_len - SizeofLeopardTupleHeader);
	}
...
/*
 * Perform XLogInsert of an XLOG_LEOPARD2_NEW_CID record
 *
 * This is only used in wal_level >= WAL_LEVEL_LOGICAL, and only for catalog
 * tuples.
 */
static XLogRecPtr
log_leopard_new_cid(Relation relation, LeopardTuple tup)
{
	xl_leopard_new_cid xlrec;

	XLogRecPtr	recptr;
	LeopardTupleHeader hdr = tup->t_data;
	 ...
	 */
	if (hdr->t_infomask & LEOPARD_COMBOCID)
	{
		Assert(!(hdr->t_infomask & LEOPARD_XMAX_INVALID));
		Assert(!LeopardTupleHeaderXminInvalid(hdr));
		xlrec.cmin = LeopardTupleHeaderGetCmin(hdr);
		xlrec.cmax = LeopardTupleHeaderGetCmax(hdr);
		xlrec.combocid = LeopardTupleHeaderGetRawCommandId(hdr);
	}
	...
	else
	{
		 ...
		 * inserted.
		 */
		if (hdr->t_infomask & LEOPARD_XMAX_INVALID ||
			LEOPARD_XMAX_IS_LOCKED_ONLY(hdr->t_infomask))
		{
			xlrec.cmin = LeopardTupleHeaderGetRawCommandId(hdr);
		...
		else
		{
			...
			xlrec.cmax = LeopardTupleHeaderGetRawCommandId(hdr);
	 ...
	 */
	XLogBeginInsert();
	XLogRegisterData((char *) &xlrec, SizeOfLeopardNewCid);

	/* will be looked at irrespective of origin */

	recptr = XLogInsert(RM_LEOPARD2_ID, XLOG_LEOPARD2_NEW_CID);

	return recptr;
}

/*
 * Build a leopard tuple representing the configured REPLICA IDENTITY to represent
 ...
 */
static LeopardTuple
LeopardExtractReplicaIdentity(Relation relation, LeopardTuple tp, bool key_required,
					   bool *copy)
{
	TupleDesc	desc = RelationGetDescr(relation);
	char		replident = relation->rd_rel->relreplident;
	Bitmapset  *idattrs;
	LeopardTuple	key_tuple;
	bool		nulls[MaxLeopardAttributeNumber];
	Datum		values[MaxLeopardAttributeNumber];
	...
	*copy = false;
	 ...
	 * (This case should not be reachable from leopard_update, since that should
	 * calculate key_required accurately.  But leopard_delete just passes
	 ...
	 */
	leopard_deform_tuple(tp, desc, values, nulls);

	for (int i = 0; i < desc->natts; i++)
	{
		if (bms_is_member(i + 1 - FirstLowInvalidLeopardAttributeNumber,
						  idattrs))
			Assert(!nulls[i]);
		else
			nulls[i] = true;
	...
	key_tuple = leopard_form_tuple(desc, values, nulls);
	*copy = true;
	 ...
	 */
	if (LeopardTupleHasExternal(key_tuple))
	{
		LeopardTuple	oldtup = key_tuple;

		key_tuple = toast_flatten_tuple(oldtup, desc);
		leopard_freetuple(oldtup);
	}
...
/*
 * Handles XLOG_LEOPARD2_PRUNE record type.
 *
 * Acquires a super-exclusive lock.
 */
static void
leopard_xlog_prune(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_prune *xlrec = (xl_leopard_prune *) XLogRecGetData(record);
	Buffer		buffer;
	RelFileNode rnode;
	BlockNumber blkno;
	...
	if (action == BLK_NEEDS_REDO)
	{
		...
		leopard_page_prune_execute(buffer,
								...
								nowdead, ndead,
								nowunused, nunused, true /* do_partial_cleanup */);
...
/*
 * Handles XLOG_LEOPARD2_VACUUM record type.
 *
 * Acquires an exclusive lock only.
 */
static void
leopard_xlog_vacuum(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_vacuum *xlrec = (xl_leopard_vacuum *) XLogRecGetData(record);
	Buffer		buffer;
	BlockNumber blkno;
	XLogRedoAction action;
...
/*
 * Replay XLOG_LEOPARD2_VISIBLE record.
 *
 ...
 */
static void
leopard_xlog_visible(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_visible *xlrec = (xl_leopard_visible *) XLogRecGetData(record);
	Buffer		vmbuffer = InvalidBuffer;
	Buffer		buffer;
	Page		page;
	RelFileNode rnode;
	 ...
	 * might incorrectly decide that an index-only scan can skip a leopard fetch.
	 *
	 * NB: It might be better to throw some kind of "soft" conflict here that
	 * forces any index-only scan that is in flight to perform leopard fetches,
	...
	/*
	 * Read the leopard page, if it still exists. If the leopard file has dropped or
	...
	{
		/*
		 * We don't bump the LSN of the leopard page when setting the visibility
	...
	{
		/*
		 * If leopard block was backed up, we already restored it and there's
	...
	if (BufferIsValid(buffer))
	{
		 ...
		 * FSM data is not in the page anyway.
		 */
		if (xlrec->flags & LEOPARDVISMAP_VALID_BITS)
			XLogRecordPageWithFreeSpace(rnode, blkno, space);
	}

	/*
	 * Even if we skipped the leopard page update due to the LSN interlock, it's
									  ...
									  &vmbuffer) == BLK_NEEDS_REDO)
	{
		 ...
		 * leopard_vismap_set will handle locking itself.
		 */
		LockBuffer(vmbuffer, BUFFER_LOCK_UNLOCK);

		reln = CreateFakeRelcacheEntry(rnode);
		leopard_vismap_pin(reln, blkno, &vmbuffer);
		 ...
		 * we did for the leopard page.  If this results in a dropped bit, no
		 * real harm is done; and the next VACUUM will fix it.
		 */
		if (lsn > PageGetLSN(vmpage))
			leopard_vismap_set(reln, blkno, InvalidBuffer, lsn, vmbuffer,
...
/*
 * Replay XLOG_LEOPARD2_FREEZE_PAGE records
 */
static void
leopard_xlog_freeze_page(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_freeze_page *xlrec = (xl_leopard_freeze_page *) XLogRecGetData(record);
	...
	{
		Page		page = BufferGetPage(buffer);
		xl_leopard_freeze_tuple *tuples;

		tuples = (xl_leopard_freeze_tuple *) XLogRecGetBlockData(record, 0, NULL);
		...
		for (ntup = 0; ntup < xlrec->ntuples; ntup++)
		{
			...
			ItemId		lp;
			LeopardTupleHeader tuple;

			xlrec_tp = &tuples[ntup];
			lp = PageGetItemId(page, xlrec_tp->offset); /* offsets are one-based */
			tuple = (LeopardTupleHeader) PageGetItem(page, lp);

			leopard_execute_freeze_tuple(tuple, xlrec_tp);
		}
 ...
 *
 * (This is the reverse of leopard_compute_infobits).
 */
static void
leopard_fix_infomask_from_infobits(uint8 infobits, uint16 *infomask, uint16 *infomask2)
{
	*infomask &= ~(LEOPARD_XMAX_IS_MULTI | LEOPARD_XMAX_LOCK_ONLY |
				   LEOPARD_XMAX_KEYSHR_LOCK | LEOPARD_XMAX_EXCL_LOCK);
	*infomask2 &= ~LEOPARD_KEYS_UPDATED;

	if (infobits & XLHL_XMAX_IS_MULTI)
		*infomask |= LEOPARD_XMAX_IS_MULTI;
	if (infobits & XLHL_XMAX_LOCK_ONLY)
		*infomask |= LEOPARD_XMAX_LOCK_ONLY;
	if (infobits & XLHL_XMAX_EXCL_LOCK)
		*infomask |= LEOPARD_XMAX_EXCL_LOCK;
	/* note LEOPARD_XMAX_SHR_LOCK isn't considered here */
	if (infobits & XLHL_XMAX_KEYSHR_LOCK)
		*infomask |= LEOPARD_XMAX_KEYSHR_LOCK;
...
static void
leopard_xlog_delete(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_delete *xlrec = (xl_leopard_delete *) XLogRecGetData(record);
	Buffer		buffer;
	Page		page;
	ItemId		lp = NULL;
	LeopardTupleHeader leopardtup;
	...
	/*
	 * The visibility map may need to be fixed even if the leopard page is
	 * already up-to-date.
	 */
	if (xlrec->flags & XLLEOPARD_DELETE_ALL_VISIBLE_CLEARED)
	{
		Relation	reln = CreateFakeRelcacheEntry(target_node);
		Buffer		vmbuffer = InvalidBuffer;

		leopard_vismap_pin(reln, blkno, &vmbuffer);
		leopard_vismap_clear(reln, blkno, vmbuffer, LEOPARDVISMAP_VALID_BITS);
		ReleaseBuffer(vmbuffer);
		FreeFakeRelcacheEntry(reln);
	}
	...
	if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO)
	{
		...
		leopardtup->t_infomask &= ~(LEOPARD_XMAX_BITS);
		leopardtup->t_infomask2 &= ~LEOPARD_KEYS_UPDATED;
		LeopardTupleHeaderClearHotUpdated(leopardtup);
		leopard_fix_infomask_from_infobits(xlrec->infobits_set,
								   &leopardtup->t_infomask, &leopardtup->t_infomask2);
		if (!(xlrec->flags & XLLEOPARD_DELETE_IS_SUPER))
			LeopardTupleHeaderSetXmax(leopardtup, xlrec->xmax);
		else
			LeopardTupleHeaderSetXmin(leopardtup, InvalidTransactionId);
		LeopardTupleHeaderSetCmax(leopardtup, FirstCommandId, false);
		...
		if (xlrec->flags & XLLEOPARD_DELETE_IS_PARTITION_MOVE)
			LeopardTupleHeaderSetMovedPartitions(leopardtup);
		else
			leopardtup->t_ctid = target_tid;
		PageSetLSN(page, lsn);
		MarkBufferDirty(buffer);
	}
	if (BufferIsValid(buffer))
...
static void
leopard_xlog_insert(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_insert *xlrec = (xl_leopard_insert *) XLogRecGetData(record);
	Buffer		buffer;
	Page		page;
	union
	{
		LeopardTupleHeaderData hdr;
		char		data[MaxLeopardTupleSize];
	}			tbuf;
	LeopardTupleHeader leopardtup;
	xl_leopard_header xlhdr;
	uint32		newlen;
	 ...
	 * already up-to-date.
	 */
	if (xlrec->flags & XLLEOPARD_INSERT_ALL_VISIBLE_CLEARED)
	{
		Relation	reln = CreateFakeRelcacheEntry(target_node);
		Buffer		vmbuffer = InvalidBuffer;
	 ...
	 * page from scratch.
	 */
	if (XLogRecGetInfo(record) & XLOG_LEOPARD_INIT_PAGE)
	{
	...
	if (action == BLK_NEEDS_REDO)
	{
		...
		newlen = datalen - SizeOfLeopardHeader;
		Assert(datalen > SizeOfLeopardHeader && newlen <= MaxLeopardTupleSize);
		memcpy((char *) &xlhdr, data, SizeOfLeopardHeader);
		data += SizeOfLeopardHeader;

		leopardtup = &tbuf.hdr;
		MemSet((char *) leopardtup, 0, SizeofLeopardTupleHeader);
		/* PG73FORMAT: get bitmap [+ padding] [+ oid] + data */
		memcpy((char *) leopardtup + SizeofLeopardTupleHeader,
			   data,
			   newlen);
		newlen += SizeofLeopardTupleHeader;
		leopardtup->t_infomask2 = xlhdr.t_infomask2;
		leopardtup->t_infomask = xlhdr.t_infomask;
		leopardtup->t_hoff = xlhdr.t_hoff;
		LeopardTupleHeaderSetXmin(leopardtup, XLogRecGetXid(record));
		LeopardTupleHeaderSetCmin(leopardtup, FirstCommandId);
		leopardtup->t_ctid = target_tid;

		if (PageAddItem(page, (Item) leopardtup, newlen, xlrec->offnum,
		...
		/* XLLEOPARD_INSERT_ALL_FROZEN_SET implies that all tuples are visible */
		if (xlrec->flags & XLLEOPARD_INSERT_ALL_FROZEN_SET)
			PageSetAllVisible(page);
 ...
 */
static void
leopard_xlog_multi_insert(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_multi_insert *xlrec;
	RelFileNode rnode;
	...
	Size		freespace = 0;
	int			i;
	bool		isinit = (XLogRecGetInfo(record) & XLOG_LEOPARD_INIT_PAGE) != 0;
	XLogRedoAction action;
	 ...
	 * required.
	 */
	xlrec = (xl_leopard_multi_insert *) XLogRecGetData(record);

	XLogRecGetBlockTag(record, 0, &rnode, NULL, &blkno);

	/* check that the mutually exclusive flags are not both set */
	Assert(!((xlrec->flags & XLLEOPARD_INSERT_ALL_VISIBLE_CLEARED) &&
			 (xlrec->flags & XLLEOPARD_INSERT_ALL_FROZEN_SET)));
	...
	if (action == BLK_NEEDS_REDO)
	{
		...
		for (i = 0; i < xlrec->ntuples; i++)
		{
			...
			Assert(newlen <= MaxLeopardTupleSize);
			leopardtup = &tbuf.hdr;
			MemSet((char *) leopardtup, 0, SizeofLeopardTupleHeader);
			/* PG73FORMAT: get bitmap [+ padding] [+ oid] + data */
			memcpy((char *) leopardtup + SizeofLeopardTupleHeader,
			...
			LeopardTupleHeaderSetXmin(leopardtup, XLogRecGetXid(record));
			LeopardTupleHeaderSetCmin(leopardtup, FirstCommandId);
			ItemPointerSetBlockNumber(&leopardtup->t_ctid, blkno);
			ItemPointerSetOffsetNumber(&leopardtup->t_ctid, offnum);

			offnum = PageAddItem(page, (Item) leopardtup, newlen, offnum, true, true);
 ...
 */
static void
leopard_xlog_update(XLogReaderState *record, bool hot_update)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_update *xlrec = (xl_leopard_update *) XLogRecGetData(record);
	RelFileNode rnode;
	BlockNumber oldblk;
	BlockNumber newblk;
	 ...
	 * already up-to-date.
	 */
	if (xlrec->flags & XLLEOPARD_UPDATE_OLD_ALL_VISIBLE_CLEARED)
	{
		Relation	reln = CreateFakeRelcacheEntry(rnode);
		Buffer		vmbuffer = InvalidBuffer;

		leopard_vismap_pin(reln, oldblk, &vmbuffer);
		leopard_vismap_clear(reln, oldblk, vmbuffer, LEOPARDVISMAP_VALID_BITS);
		ReleaseBuffer(vmbuffer);
		FreeFakeRelcacheEntry(reln);
	}
	...
	if (oldaction == BLK_NEEDS_REDO)
	{
		...
		oldtup.t_data = leopardtup;
		oldtup.t_len = ItemIdGetLength(lp);

		leopardtup->t_infomask &= ~(LEOPARD_XMAX_BITS);
		leopardtup->t_infomask2 &= ~LEOPARD_KEYS_UPDATED;
		if (hot_update)
			LeopardTupleHeaderSetHotUpdated(leopardtup);
		else
			LeopardTupleHeaderClearHotUpdated(leopardtup);
		leopard_fix_infomask_from_infobits(xlrec->old_infobits_set, &leopardtup->t_infomask,
								   &leopardtup->t_infomask2);
		LeopardTupleHeaderSetXmax(leopardtup, xlrec->old_xmax);
		LeopardTupleHeaderSetCmax(leopardtup, FirstCommandId, false);
		/* Set forward chain link in t_ctid */
		leopardtup->t_ctid = newtid;
	...
	else if (XLogRecGetInfo(record) & XLOG_LEOPARD_INIT_PAGE)
	{
	 ...
	 * already up-to-date.
	 */
	if (xlrec->flags & XLLEOPARD_UPDATE_NEW_ALL_VISIBLE_CLEARED)
	{
		Relation	reln = CreateFakeRelcacheEntry(rnode);
		Buffer		vmbuffer = InvalidBuffer;

		leopard_vismap_pin(reln, newblk, &vmbuffer);
		leopard_vismap_clear(reln, newblk, vmbuffer, LEOPARDVISMAP_VALID_BITS);
		ReleaseBuffer(vmbuffer);
		FreeFakeRelcacheEntry(reln);
	}
	...
	if (newaction == BLK_NEEDS_REDO)
	{
		...
		if (xlrec->flags & XLLEOPARD_UPDATE_PREFIX_FROM_OLD)
		{
			Assert(newblk == oldblk);
			memcpy(&prefixlen, recdata, sizeof(uint16));
			recdata += sizeof(uint16);
		}
		if (xlrec->flags & XLLEOPARD_UPDATE_SUFFIX_FROM_OLD)
		{
			Assert(newblk == oldblk);
		...
		memcpy((char *) &xlhdr, recdata, SizeOfLeopardHeader);
		recdata += SizeOfLeopardHeader;

		tuplen = recdata_end - recdata;
		Assert(tuplen <= MaxLeopardTupleSize);
		 ...
		 */
		newp = (char *) leopardtup + SizeofLeopardTupleHeader;
		if (prefixlen > 0)
		{
			int			len;

			/* copy bitmap [+ padding] [+ oid] from WAL record */
			len = xlhdr.t_hoff - SizeofLeopardTupleHeader;
			...
			len = tuplen - (xlhdr.t_hoff - SizeofLeopardTupleHeader);
			memcpy(newp, recdata, len);
			recdata += len;
			newp += len;
		}
		...
		newlen = SizeofLeopardTupleHeader + tuplen + prefixlen + suffixlen;
		leopardtup->t_infomask2 = xlhdr.t_infomask2;
		leopardtup->t_infomask = xlhdr.t_infomask;
		leopardtup->t_hoff = xlhdr.t_hoff;
		...
		LeopardTupleHeaderSetXmin(leopardtup, XLogRecGetXid(record));
		LeopardTupleHeaderSetCmin(leopardtup, FirstCommandId);
		LeopardTupleHeaderSetXmax(leopardtup, xlrec->new_xmax);
		/* Make sure there is no forward chain link in t_ctid */
		leopardtup->t_ctid = newtid;
...
static void
leopard_xlog_confirm(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_confirm *xlrec = (xl_leopard_confirm *) XLogRecGetData(record);
	Buffer		buffer;
	Page		page;
	OffsetNumber offnum;
	ItemId		lp = NULL;
	...
	if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO)
	{
		 ...
		 */
		ItemPointerSet(&leopardtup->t_ctid, BufferGetBlockNumber(buffer), offnum);
...
static void
leopard_xlog_lock(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_lock *xlrec = (xl_leopard_lock *) XLogRecGetData(record);
	Buffer		buffer;
	Page		page;
	OffsetNumber offnum;
	ItemId		lp = NULL;
	 ...
	 * already up-to-date.
	 */
	if (xlrec->flags & XLLEOPARD_LOCK_ALL_FROZEN_CLEARED)
	{
		RelFileNode rnode;
		Buffer		vmbuffer = InvalidBuffer;
		BlockNumber block;
		...
		leopard_vismap_pin(reln, block, &vmbuffer);
		leopard_vismap_clear(reln, block, vmbuffer, LEOPARDVISMAP_ALL_FROZEN);
	...
	if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO)
	{
		...
		leopardtup->t_infomask &= ~(LEOPARD_XMAX_BITS);
		leopardtup->t_infomask2 &= ~LEOPARD_KEYS_UPDATED;
		leopard_fix_infomask_from_infobits(xlrec->infobits_set, &leopardtup->t_infomask,
								   &leopardtup->t_infomask2);
		 ...
		 * there's no update.
		 */
		if (LEOPARD_XMAX_IS_LOCKED_ONLY(leopardtup->t_infomask))
		{
			LeopardTupleHeaderClearHotUpdated(leopardtup);
			/* Make sure there is no forward chain link in t_ctid */
			ItemPointerSet(&leopardtup->t_ctid,
						   BufferGetBlockNumber(buffer),
						   offnum);
		}
		LeopardTupleHeaderSetXmax(leopardtup, xlrec->locking_xid);
		LeopardTupleHeaderSetCmax(leopardtup, FirstCommandId, false);
		PageSetLSN(page, lsn);
		MarkBufferDirty(buffer);
	}
...
static void
leopard_xlog_lock_updated(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_lock_updated *xlrec;
	Buffer		buffer;
	...
	xlrec = (xl_leopard_lock_updated *) XLogRecGetData(record);
...
static void
leopard_xlog_inplace(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_leopard_inplace *xlrec = (xl_leopard_inplace *) XLogRecGetData(record);
	Buffer		buffer;
	Page		page;
	OffsetNumber offnum;
	ItemId		lp = NULL;
	...
	if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO)
	{
		...
		memcpy((char *) leopardtup + leopardtup->t_hoff, newtup, newlen);
...
void
leopard_redo(XLogReaderState *record)
{
	uint8		info = XLogRecGetInfo(record) & ~XLR_INFO_MASK;

	/*
	 * These operations don't overwrite MVCC data so no conflict processing is
	 * required. The ones in leopard2 rmgr do.
	 */

	switch (info & XLOG_LEOPARD_OPMASK)
	{
		case XLOG_LEOPARD_INSERT:
			leopard_xlog_insert(record);
			break;
		case XLOG_LEOPARD_DELETE:
			leopard_xlog_delete(record);
			break;
		case XLOG_LEOPARD_UPDATE:
			leopard_xlog_update(record, false);
			break;
		...
		case XLOG_LEOPARD_HOT_UPDATE:
			leopard_xlog_update(record, true);
			break;
		case XLOG_LEOPARD_CONFIRM:
			leopard_xlog_confirm(record);
			break;
		case XLOG_LEOPARD_LOCK:
			leopard_xlog_lock(record);
			break;
		case XLOG_LEOPARD_INPLACE:
			leopard_xlog_inplace(record);
			break;
		default:
			elog(PANIC, "leopard_redo: unknown op code %u", info);
	}
}

void
leopard2_redo(XLogReaderState *record)
{
	uint8		info = XLogRecGetInfo(record) & ~XLR_INFO_MASK;
	...
	switch (info & XLOG_LEOPARD_OPMASK)
	{
		case XLOG_LEOPARD2_PRUNE:
			leopard_xlog_prune(record);
			break;
		case XLOG_LEOPARD2_VACUUM:
			leopard_xlog_vacuum(record);
			break;
		case XLOG_LEOPARD2_FREEZE_PAGE:
			leopard_xlog_freeze_page(record);
			break;
		case XLOG_LEOPARD2_VISIBLE:
			leopard_xlog_visible(record);
			break;
		case XLOG_LEOPARD2_MULTI_INSERT:
			leopard_xlog_multi_insert(record);
			break;
		case XLOG_LEOPARD2_LOCK_UPDATED:
			leopard_xlog_lock_updated(record);
			break;
		...
		case XLOG_LEOPARD2_REWRITE:
			leopard_xlog_logical_rewrite(record);
			break;
		default:
			elog(PANIC, "leopard2_redo: unknown op code %u", info);
	}
}

/*
 * Mask a leopard page before performing consistency checks on it.
 */
void
leopard_mask(char *pagedata, BlockNumber blkno)
{
	...
	for (off = 1; off <= PageGetMaxOffsetNumber(page); off++)
	{
		...
		{
			LeopardTupleHeader page_htup = (LeopardTupleHeader) page_item;
			 ...
			 * emitting WAL.
			 */
			if (!LeopardTupleHeaderXminFrozen(page_htup))
				page_htup->t_infomask &= ~LEOPARD_XACT_MASK;
			else
			{
				/* Still we need to mask xmax hint bits. */
				page_htup->t_infomask &= ~LEOPARD_XMAX_INVALID;
				page_htup->t_infomask &= ~LEOPARD_XMAX_COMMITTED;
			}

			/*
			 * During replay, we set Command Id to FirstCommandId. Hence, mask
			 * it. See leopard_xlog_insert() for details.
			 */
			page_htup->t_choice.t_leopard.t_cid = MASK_MARKER;

			/*
			 * For a speculative tuple, leopard_insert() does not set ctid in the
			 * caller-passed leopard tuple itself, leaving the ctid field to
			 ...
			 * any circumstances.
			 *
			 * During redo, leopard_xlog_insert() sets t_ctid to current block
			 * number and self offset number. It doesn't care about any
			 * speculative insertions on the primary. Hence, we set t_ctid to
			 * current block number and self offset number to ignore any
			 * inconsistency.
			 */
			if (LeopardTupleHeaderIsSpeculative(page_htup))
				ItemPointerSet(&page_htup->t_ctid, blkno, off);

			/*
			 * NB: Not ignoring ctid changes due to the tuple having moved
			 * (i.e. LeopardTupleHeaderIndicatesMovedPartitions), because that's
...
/*
 * LeopardCheckForSerializableConflictOut
 ...
 *
 * This function should be called just about anywhere in leopardam.c where a
 * tuple has been read. The caller must hold at least a shared lock on the
 * buffer, because this function might set hint bits on the tuple. There is
 * currently no known reason to call this function from an index AM.
 */
void
LeopardCheckForSerializableConflictOut(bool visible, Relation relation,
									LeopardTuple tuple, Buffer buffer,
									Snapshot snapshot)
{
	TransactionId xid;
	 ...
	 * tuple is visible to us, while LeopardTupleSatisfiesVacuum checks what else
	 * is going on with it.
	 *
	 ...
	 */
	htsvResult = LeopardTupleSatisfiesVacuum(tuple, TransactionXmin, buffer);
	switch (htsvResult)
	{
		case LEOPARDTUPLE_LIVE:
			if (visible)
				return;
			xid = LeopardTupleHeaderGetXmin(tuple->t_data);
			break;
		case LEOPARDTUPLE_RECENTLY_DEAD:
		case LEOPARDTUPLE_DELETE_IN_PROGRESS:
			if (visible)
				xid = LeopardTupleHeaderGetUpdateXid(tuple->t_data);
			else
				xid = LeopardTupleHeaderGetXmin(tuple->t_data);

			if (TransactionIdPrecedes(xid, TransactionXmin))
			{
				/* This is like the LEOPARDTUPLE_DEAD case */
				Assert(!visible);
				return;
			}
			break;
		...
		default:
			 ...
			 */
			elog(ERROR, "unrecognized return value from LeopardTupleSatisfiesVacuum: %u", htsvResult);
