 ...
 *
 * mqh_bytes_written is the local copy of total bytes writtens.  And it is only
 * updated in the shared memory once we have written at least mq_min_send_size
 * more bytes compared to what we updated last in the shared memory.  Last
 * time updated bytes are maintained in mqh_last_sent_bytes, so that we can
 * avoid reading the shared memory variable everytime for computing when to
 * inform the reader next.
 *
...
static void shm_mq_update_bytes_written(shm_mq_handle *mqh);
...
shm_mq_sendv(shm_mq_handle *mqh, shm_mq_iovec *iov, int iovcnt, bool nowait)
{
	 ...
	 * enough data.
	 */
	if (mqh->mqh_bytes_written - mqh->mqh_last_sent_bytes >
		mq->mq_min_send_size)
		shm_mq_update_bytes_written(mqh);
				  ...
				  bool nowait, Size *bytes_written)
{
	...
	while (sent < nbytes)
	{
		...
		else
		{
			 ...
			 * will do that based on the value of the for the queue
			 * mq_min_send_size.
			 */
...
/*
 * Update the latest value of the bytes written in the shared memory.
 */
static void
shm_mq_update_bytes_written(shm_mq_handle *mqh)
{
	shm_mq *mq = mqh->mqh_queue;

	/*
	 * Update the shared memory value with the latest value in the local
	 * copy.  Also update the mqh_last_sent_bytes so that we know when we
	 * updated last.
	 *
	 * XXX we can avoid mqh_last_sent_bytes variable completely by keeping the
	 * delta in mqh->mqh_bytes_written.  But in that case we will have to
	 * freqnetly read the mq->mq_bytes_written from the shared memory which is
	 * not that bad but keeping local value shows better performance.
	 */
