 ...
 *
 * hio.c
 *	  POSTGRES leopard access method input/output code.
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/leopard/hio.c
 *
 *-------------------------------------------------------------------------
 */
...
/*
 * RelationPutLeopardTuple - place tuple at specified page
 *
 ...
 */
void
RelationPutLeopardTuple(Relation relation,
					 Buffer buffer,
					 LeopardTuple tuple,
					 bool token)
{
	 ...
	 * token set.
	 */
	Assert(!token || LeopardTupleHeaderIsSpeculative(tuple->t_data));
	 ...
	 */
	Assert(!((tuple->t_data->t_infomask & LEOPARD_XMAX_COMMITTED) &&
			 (tuple->t_data->t_infomask & LEOPARD_XMAX_IS_MULTI)));
	...
	if (!token)
	{
		...
		LeopardTupleHeader item = (LeopardTupleHeader) PageGetItem(pageHeader, itemId);
...
/*
 * For each leopard page which is all-visible, acquire a pin on the appropriate
					 ...
					 Buffer *vmbuffer1, Buffer *vmbuffer2)
{
	...
	while (1)
	{
		...
		need_to_pin_buffer1 = PageIsAllVisible(BufferGetPage(buffer1))
			&& !leopard_vismap_pin_ok(block1, *vmbuffer1);
		need_to_pin_buffer2 = buffer2 != InvalidBuffer
			&& PageIsAllVisible(BufferGetPage(buffer2))
			&& !leopard_vismap_pin_ok(block2, *vmbuffer2);
		...
		if (need_to_pin_buffer1)
			leopard_vismap_pin(relation, block1, vmbuffer1);
		if (need_to_pin_buffer2)
			leopard_vismap_pin(relation, block2, vmbuffer2);
 ...
 *	buffer will also be exclusive-locked.  (This case is used by leopard_update;
 *	the otherBuffer contains the tuple being updated.)
 *
 *	The reason for passing otherBuffer is that if two backends are doing
 *	concurrent leopard_update operations, a deadlock could occur if they try
 ...
 *	happen if space is freed in that page after leopard_update finds there's not
 ...
 *	if LEOPARD_INSERT_SKIP_FSM is specified, we just append a new empty page to
 *	the end of the relation if the tuple won't fit on the current target page.
 *	This can save some cycles when we know the relation is new and doesn't
 *	contain useful amounts of free space.
 *
 *	LEOPARD_INSERT_SKIP_FSM is also useful for non-WAL-logged additions to a
 *	relation, if the caller holds exclusive lock and is careful to invalidate
 *	relation's smgr_targblock before the first insertion --- that ensures that
 *	all insertions will occur into newly added pages and not be intermixed
 *	with tuples from other transactions.  That way, a crash can't risk losing
 *	any committed data of other transactions.  (See leopard_insert's comments
...
{
	bool		use_fsm = !(options & LEOPARD_INSERT_SKIP_FSM);
	 ...
	 */
	nearlyEmptyFreeSpace = MaxLeopardTupleSize -
		(MaxLeopardTuplesPerPage / 8 * sizeof(ItemIdData));
...
loop:
	while (targetBlock != InvalidBlockNumber)
	{
		...
		if (otherBuffer == InvalidBuffer)
		{
			...
			if (PageIsAllVisible(BufferGetPage(buffer)))
				leopard_vismap_pin(relation, targetBlock, vmbuffer);
	...
	if (options & LEOPARD_INSERT_FROZEN)
	{
		...
		leopard_vismap_pin(relation, BufferGetBlockNumber(buffer), vmbuffer);
	}
