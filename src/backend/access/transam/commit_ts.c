					 ...
					 RepOriginId nodeid, int pageno)
{
	int			slotno;
	int			i;
	LWLock	   *lock = SimpleLruPageNoGetLock(CommitTsCtl, pageno);
							 ...
							 RepOriginId *nodeid)
{
	...
	*ts = entry.time;
	...
	LWLockRelease(SimpleLruPageNoGetLock(CommitTsCtl, pageno));
	return *ts != 0;
}
...
ActivateCommitTs(void)
{
	...
	//FIXME: Maybe for commitTsShared data which is not related to SLRU we can
	// continue to use CommitTsLock
...
DeactivateCommitTs(void)
{
	 ...
	 * tidy.)
	 */
	SimpleLruControlLockAcquire(CommitTsCtl, LW_EXCLUSIVE);
	(void) SlruScanDirectory(CommitTsCtl, SlruScanDirCbDeleteAll, NULL);
	SimpleLruControlLockRelease(CommitTsCtl);
}
...
ExtendCommitTs(TransactionId newestXact)
{
	...
	lock = SimpleLruPageNoGetLock(CommitTsCtl, pageno);
