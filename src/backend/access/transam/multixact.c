	...
	*offptr = offset;
	...
	for (i = 0; i < nmembers; i++, offset++)
	{
		...
		if (pageno != prev_pageno)
		{
			 ...
			 * fall into the different SLRU bank then release the old bank lock
			 * and acquire lock on the new bank.
			 */
...
retry:
	...
	/*
	 * If the page is on different bank then the previous bank on which we are
	 * already holding the lock then release old lock and acquire new.
	 */
	lock = SimpleLruPageGetSLRULock(MultiXactOffsetCtl, pageno);
	if (lock != prevlock)
	{
	...
	else
	{
		...
		{
			/*
			 * SLRU pageno is changed so check whether this page is falling on
			 * different bank then on which we are already holding the lock and
			 * if so release the hold back lock and acquire on new bank.
			 */
			lock = SimpleLruPageGetSLRULock(MultiXactOffsetCtl, pageno);
			if (prevlock != lock)
			{
