 ...
 * we suspend the leopard scan phase and perform a pass of index cleanup and page
 * compaction, then resume the leopard scan with an empty TID array.
 *
 * If we're processing a table with no indexes, we can just vacuum each page
 * as we go; there's no need to save up multiple tuples to minimize the number
 * of index scans performed.  So we don't use maintenance_work_mem memory for
 * the TID array, just enough to hold as many leopard tuples as fit on one page.
 *
 ...
 * process.  At the beginning of a lazy vacuum (at lazy_scan_leopard) we prepare
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/leopard/vacuumlazy.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/*
 * Threshold that controls whether we bypass index vacuuming and leopard
 * vacuuming as an optimization
 */
#define BYPASS_THRESHOLD_PAGES	0.02	/* i.e. 2% of rel_pages */

/*
 * Perform a failsafe check every 4GB during the leopard scan, approximately
 */
 ...
 * (it won't be exact because we only vacuum FSM after processing a leopard page
 * that has some removable tuples).  When there are indexes, this is ignored,
 * and we vacuum FSM after each index/leopard cleaning pass.
 */
 ...
 * tables.
 */
#define LAZY_ALLOC_TUPLES		MaxLeopardTuplesPerPage
...
typedef enum
{
	...
	VACUUM_ERRCB_PHASE_SCAN_LEOPARD,
	VACUUM_ERRCB_PHASE_VACUUM_INDEX,
	VACUUM_ERRCB_PHASE_VACUUM_LEOPARD,
	VACUUM_ERRCB_PHASE_INDEX_CLEANUP,
	VACUUM_ERRCB_PHASE_TRUNCATE
} VacErrPhase;

/*
 * LVDeadTuples stores the dead tuple TIDs collected during the leopard scan.
...
typedef struct LVShared
{
	 ...
	 *
	 * reltuples is the total number of input leopard tuples.  We set either old
	 ...
	 * vacuuming or cleanup apart from the memory for leopard scanning.  In
...
{
	/* Target leopard relation and its indexes */
	Relation	rel;
	Relation   *indrels;
	int			nindexes;
	...
	char	   *relname;
	char	   *indname;
	BlockNumber blkno;			/* used only for leopard operations */
	OffsetNumber offnum;		/* used only for leopard operations */
	VacErrPhase phase;

	/*
	 * State managed by lazy_scan_leopard() follows
	 */
...
static void lazy_scan_leopard(LVRelState *vacrel, VacuumParams *params,
						   bool aggressive);
...
static void lazy_vacuum_leopard_rel(LVRelState *vacrel);
static int	lazy_vacuum_leopard_page(LVRelState *vacrel, BlockNumber blkno,
...
static void lazy_truncate_leopard(LVRelState *vacrel);
...
static bool leopard_page_is_all_visible(LVRelState *vacrel, Buffer buf,
...
/*
 *	leopard_vacuum_rel() -- perform VACUUM for one leopard relation
 *
 *		This routine vacuums a single leopard, cleans out its indexes, and
 ...
 */
void
leopard_vacuum_rel(Relation rel, VacuumParams *params,
				BufferAccessStrategy bstrategy)
{
	LVRelState *vacrel;
	PGRUsage	ru0;
	 ...
	 * during a vacuum.  During different phases of vacuum (leopard scan, leopard
	 * vacuum, index vacuum, index clean up, leopard truncate), we update the
	 * error context callback to display appropriate information.
	 *
	 * Note that the index vacuum and leopard vacuum phases may be called
	 * multiple times in the middle of the leopard scan phase.  So the old phase
	...
	/* Do the vacuuming */
	lazy_scan_leopard(vacrel, params, aggressive);
	...
	leopard_vismap_count(rel, &new_rel_allvisible, NULL);
	 ...
	 * remain in the table due to VACUUM bypassing index and leopard vacuuming.
	 ...
	 * soon in cases where the failsafe prevented significant amounts of leopard
	 * vacuuming.
	 */
...
/*
 *	lazy_scan_leopard() -- scan an open leopard relation
 *
 *		This routine prunes each page in the leopard, which will among other
 *		things truncate dead tuples to dead line pointers, defragment the
 *		page, and set commit status bits (see leopard_page_prune).  It also builds
 *		lists of dead tuples and pages with free space, calculates statistics
 *		on the number of live tuples in the leopard, and marks pages as
 ...
 *		leopard relation during its own second pass over the leopard.
 *
 ...
 *		leopard scan so that we can record dead tuples to the DSM segment.  All
 ...
 */
static void
lazy_scan_leopard(LVRelState *vacrel, VacuumParams *params, bool aggressive)
{
	...
	const int	initprog_index[] = {
		...
		PROGRESS_VACUUM_TOTAL_LEOPARD_BLKS,
	...
	/* Report that we're scanning the leopard, advertising total # of blocks */
	initprog_val[0] = PROGRESS_VACUUM_PHASE_SCAN_LEOPARD;
	 ...
	 * this stage.
	 *
	 * Note: The value returned by leopard_vismap_get_status could be slightly
	 * out-of-date, since we make this test before reading the corresponding
	 * leopard page or locking the buffer.  This is OK.  If we mistakenly think
	 ...
	 * lazy_truncate_leopard() take access-exclusive lock on the table to attempt
	...
	if ((params->options & VACOPT_DISABLE_PAGE_SKIPPING) == 0)
	{
		while (next_unskippable_block < nblocks)
		{
			...
			vmstatus = leopard_vismap_get_status(vacrel->rel,
												next_unskippable_block,
												&vmbuffer);
			if (aggressive)
			{
				if ((vmstatus & LEOPARDVISMAP_ALL_FROZEN) == 0)
					break;
			}
			else
			{
				if ((vmstatus & LEOPARDVISMAP_ALL_VISIBLE) == 0)
					break;
			}
			vacuum_delay_point();
			next_unskippable_block++;
...
#define FORCE_CHECK_PAGE() \
		...
		pgstat_progress_update_param(PROGRESS_VACUUM_LEOPARD_BLKS_SCANNED, blkno);

		update_vacuum_error_info(vacrel, NULL, VACUUM_ERRCB_PHASE_SCAN_LEOPARD,
		...
		if (blkno == next_unskippable_block)
		{
			...
			if ((params->options & VACOPT_DISABLE_PAGE_SKIPPING) == 0)
			{
				while (next_unskippable_block < nblocks)
				{
					...
					vmskipflags = leopard_vismap_get_status(vacrel->rel,
														   next_unskippable_block,
														   &vmbuffer);
					if (aggressive)
					{
						if ((vmskipflags & LEOPARDVISMAP_ALL_FROZEN) == 0)
							break;
					}
					else
					{
						if ((vmskipflags & LEOPARDVISMAP_ALL_VISIBLE) == 0)
							break;
					}
					vacuum_delay_point();
					next_unskippable_block++;
			 ...
			 */
			if (aggressive && LEOPARDVM_ALL_VISIBLE(vacrel->rel, blkno, &vmbuffer))
		...
		else
		{
			...
			if (skipping_blocks && !FORCE_CHECK_PAGE())
			{
				 ...
				 */
				if (aggressive || LEOPARDVM_ALL_FROZEN(vacrel->rel, blkno, &vmbuffer))
					vacrel->frozenskipped_pages++;
				continue;
			}
			all_visible_according_to_vm = true;
		 ...
		 * this page.
		 */
		if ((dead_tuples->max_tuples - dead_tuples->num_tuples) < MaxLeopardTuplesPerPage &&
			dead_tuples->num_tuples > 0)
		{
			...
			/* Report that we are once again scanning the leopard */
			pgstat_progress_update_param(PROGRESS_VACUUM_PHASE,
										 PROGRESS_VACUUM_PHASE_SCAN_LEOPARD);
		}
		 ...
		 */
		leopard_vismap_pin(vacrel->rel, blkno, &vmbuffer);
		...
		if (PageIsNew(page))
		{
			 ...
			 * on the leopard page, the FSM is approximate, after all.
			 */
			UnlockReleaseBuffer(buf);
		...
		if (PageIsEmpty(page))
		{
			...
			if (!PageIsAllVisible(page))
			{
				...
				/*
				 * It's possible that another backend has extended the leopard,
				 * initialized the page, and then failed to WAL-log the page
				 * due to an ERROR.  Since leopard extension is not WAL-logged,
				...
				leopard_vismap_set(vacrel->rel, blkno, buf, InvalidXLogRecPtr,
		...
		{
			/*
			 * Consider the need to do page-at-a-time leopard vacuuming when
			...
			if (prunestate.has_lpdead_items)
			{
				...
				lazy_vacuum_leopard_page(vacrel, blkno, buf, 0, &vmbuffer);
			...
			/*
			 * There was no call to lazy_vacuum_leopard_page() because pruning
		...
		{
			uint8		flags = LEOPARDVISMAP_ALL_VISIBLE;

			if (prunestate.all_frozen)
				flags |= LEOPARDVISMAP_ALL_FROZEN;
			 ...
			 *
			 * NB: If the leopard page is all-visible but the VM bit is not set,
			 * we don't need to dirty the leopard page.  However, if checksums
			 * are enabled, we do need to make sure that the leopard page is
			 * dirtied before passing it to leopard_vismap_set(), because it
		 ...
		 */
				 ...
				 && LEOPARDVM_ALL_VISIBLE(vacrel->rel, blkno, &vmbuffer))
		{
			elog(WARNING, "page is not marked all-visible but visibility map bit is set in relation \"%s\" page %u",
				 vacrel->relname, blkno);
			leopard_vismap_clear(vacrel->rel, blkno, vmbuffer,
								LEOPARDVISMAP_VALID_BITS);
		}
		 ...
		 */
				 ...
				 !LEOPARDVM_ALL_FROZEN(vacrel->rel, blkno, &vmbuffer))
		{
			/*
		...
		{
			/*
			 * Wait until lazy_vacuum_leopard_rel() to save free space.  This
			 * doesn't just save us some cycles; it also allows us to record
			 * any additional free space that lazy_vacuum_leopard_page() will
			 * make available in cases where it's possible to truncate the
			 * page's line pointer array.
			 *
			 * Note: It's not in fact 100% certain that we really will call
			 * lazy_vacuum_leopard_rel() -- lazy_vacuum() might yet opt to skip
			 * index vacuuming (and so must skip leopard vacuuming).  This is
	...
	/*
	 * Also compute the total number of surviving leopard entries.  In the
	...
	pgstat_progress_update_param(PROGRESS_VACUUM_LEOPARD_BLKS_VACUUMED, blkno);
	 ...
	 * make log report that lazy_vacuum_leopard_rel would've made had there been
	 * indexes.  (As in the two-pass strategy case, only make this report when
	 * there were LP_DEAD line pointers vacuumed in lazy_vacuum_leopard_page.)
	 */
	 ...
	 *
	 * The autovacuum specific logging in leopard_vacuum_rel summarizes an entire
	 * VACUUM operation, whereas each VACUUM VERBOSE log report generally
	 * summarizes a single round of index/leopard vacuuming (or rel truncation).
	 ...
	 * of our first pass over the leopard.
	 */
	initStringInfo(&buf);
	appendStringInfo(&buf,
...
/*
 *	lazy_scan_prune() -- lazy_scan_leopard() pruning and freezing.
 *
 * Caller must hold pin and buffer cleanup lock on the buffer.
 *
 * Prior to PostgreSQL 14 there were very rare cases where leopard_page_prune()
 * was allowed to disagree with our LeopardTupleSatisfiesVacuum() call about
 * whether or not a tuple should be considered DEAD.  This happened when an
 * inserting transaction concurrently aborted (after our leopard_page_prune()
 * call, before our LeopardTupleSatisfiesVacuum() call).  There was rather a lot
 * of complexity just so we could deal with tuples that were DEAD to VACUUM,
 * but nevertheless were left with storage after pruning.
 *
 * The approach we take now is to restart pruning when the race condition is
 * detected.  This allows leopard_page_prune() to prune the tuples inserted by
				...
				LVPagePruneState *prunestate)
{
	...
	int			tuples_deleted,
				...
				live_tuples;
	int			nfrozen;
	OffsetNumber deadoffsets[MaxLeopardTuplesPerPage];
	xl_leopard_freeze_tuple frozen[MaxLeopardTuplesPerPage];
...
retry:
	 ...
	 */
	tuples_deleted = leopard_page_prune(rel, buf, vistest,
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		 ...
		 * become LP_UNUSED inside lazy_vacuum_leopard_page() before we actually
		 ...
		 * leopard_page_prune(), but it's possible that the tuple state changed
		 * since leopard_page_prune() looked.  Handle that here by restarting.
		 * (See comments at the top of function for a full explanation.)
		 */
		res = LeopardTupleSatisfiesVacuum(&tuple, vacrel->OldestXmin, buf);

		if (unlikely(res == LEOPARDTUPLE_DEAD))
			goto retry;
		 ...
		 * encounter here will become LP_UNUSED inside lazy_vacuum_leopard_page()
		...
		switch (res)
		{
			case LEOPARDTUPLE_LIVE:
				...
				if (prunestate->all_visible)
				{
					...
					if (!LeopardTupleHeaderXminCommitted(tuple.t_data))
					{
						prunestate->all_visible = false;
						break;
					}
					 ...
					 */
					xmin = LeopardTupleHeaderGetXmin(tuple.t_data);
		...
		if (leopard_prepare_freeze_tuple(tuple.t_data,
									  vacrel->relfrozenxid,
									  vacrel->relminmxid,
									  vacrel->FreezeLimit,
	...
	if (nfrozen > 0)
	{
		...
		for (int i = 0; i < nfrozen; i++)
		{
			...
			leopard_execute_freeze_tuple(leopardtup, &frozen[i]);
		}
		...
		if (RelationNeedsWAL(vacrel->rel))
		{
			...
			recptr = log_leopard_freeze(vacrel->rel, buf, vacrel->FreezeLimit,
									 frozen, nfrozen);
	...
	/*
	 * The second pass over the leopard can also set visibility map bits, using
	 ...
	 * tuples).
	 *
	 * VACUUM will call leopard_page_is_all_visible() during the second pass over
	 * the leopard to determine all_visible and all_frozen for the page -- this
	 * is a specialized version of the logic from this function.  Now that
	 * we've finished pruning and freezing, make sure that we're in total
	 * agreement with leopard_page_is_all_visible() using an assertion.
	 */
	...
	if (prunestate->all_visible)
	{
		...
		if (!leopard_page_is_all_visible(vacrel, buf, &cutoff, &all_frozen))
			Assert(false);
...
lazy_vacuum(LVRelState *vacrel)
{
	...
	/*
	 * Consider bypassing index vacuuming (and leopard vacuuming) entirely.
	 *
	 ...
	 * have small aberrations that lead to just a few leopard pages retaining
	...
	if (vacrel->consider_bypass_optimization && vacrel->rel_pages > 0)
	{
		 ...
		 * expressed as a percentage of the total number of leopard pages in the
		 ...
		 * it's a proxy for the number of leopard pages whose visibility map bits
		 * cannot be set on account of bypassing index and leopard vacuuming.
		 *
	...
	else if (lazy_vacuum_all_indexes(vacrel))
	{
		 ...
		 * leopard vacuuming now.
		 */
		lazy_vacuum_leopard_rel(vacrel);
	}
	else
	{
		/*
		 ...
		 * vacuuming or leopard vacuuming.  This VACUUM operation won't end up
		 * back here again.
		 */
...
lazy_vacuum_all_indexes(LVRelState *vacrel)
{
	...
	/*
	 * We delete all LP_DEAD items from the first leopard pass in all indexes on
	 * each call here (except calls where we choose to do the failsafe). This
	 * makes the next call to lazy_vacuum_leopard_rel() safe (except in the event
...
/*
 *	lazy_vacuum_leopard_rel() -- second pass over the leopard for two pass strategy
 *
 ...
 * at all.
 *
 * We may also be able to truncate the line pointer array of the leopard pages we
 ...
 */
static void
lazy_vacuum_leopard_rel(LVRelState *vacrel)
{
	int			tupindex;
	...
	/* Report that we are now vacuuming the leopard */
	pgstat_progress_update_param(PROGRESS_VACUUM_PHASE,
								 PROGRESS_VACUUM_PHASE_VACUUM_LEOPARD);
	...
	while (tupindex < vacrel->dead_tuples->num_tuples)
	{
		...
		tupindex = lazy_vacuum_leopard_page(vacrel, tblk, buf, tupindex,
										 &vmbuffer);
	...
	/*
	 * We set all LP_DEAD items from the first leopard pass to LP_UNUSED during
	 * the second leopard pass.  No more, no less.
	 */
	Assert(tupindex > 0);
...
/*
 *	lazy_vacuum_leopard_page() -- free page's LP_DEAD items listed in the
 ...
 */
static int
lazy_vacuum_leopard_page(LVRelState *vacrel, BlockNumber blkno, Buffer buffer,
					  int tupindex, Buffer *vmbuffer)
{
	LVDeadTuples *dead_tuples = vacrel->dead_tuples;
	Page		page = BufferGetPage(buffer);
	OffsetNumber unused[MaxLeopardTuplesPerPage];
	int			uncnt = 0;
	...
	/* Update error traceback information */
	update_vacuum_error_info(vacrel, &saved_err_info,
							 VACUUM_ERRCB_PHASE_VACUUM_LEOPARD, blkno,
							 InvalidOffsetNumber);
	...
	if (RelationNeedsWAL(vacrel->rel))
	{
		xl_leopard_vacuum xlrec;
		XLogRecPtr	recptr;

		xlrec.nunused = uncnt;

		XLogBeginInsert();
		XLogRegisterData((char *) &xlrec, SizeOfLeopardVacuum);

		XLogRegisterBuffer(0, buffer, REGBUF_STANDARD);
		XLogRegisterBufData(0, (char *) unused, uncnt * sizeof(OffsetNumber));

		recptr = XLogInsert(RM_LEOPARD2_ID, XLOG_LEOPARD2_VACUUM);
	 ...
	 * emitted.
	 */
	if (leopard_page_is_all_visible(vacrel, buffer, &visibility_cutoff_xid,
								 &all_frozen))
		PageSetAllVisible(page);

	/*
	 * All the changes to the leopard page have been done. If the all-visible
	...
	{
		uint8		flags = 0;
		uint8		vm_status = leopard_vismap_get_status(vacrel->rel,
														 blkno, vmbuffer);

		/* Set the VM all-frozen bit to flag, if needed */
		if ((vm_status & LEOPARDVISMAP_ALL_VISIBLE) == 0)
			flags |= LEOPARDVISMAP_ALL_VISIBLE;
		if ((vm_status & LEOPARDVISMAP_ALL_FROZEN) == 0 && all_frozen)
			flags |= LEOPARDVISMAP_ALL_FROZEN;

		Assert(BufferIsValid(*vmbuffer));
		if (flags != 0)
			leopard_vismap_set(vacrel->rel, blkno, buffer, InvalidXLogRecPtr,
...
lazy_check_needs_freeze(Buffer buf, bool *hastup, LVRelState *vacrel)
{
	...
	OffsetNumber offnum,
				maxoff;
	LeopardTupleHeader tupleheader;
	...
	*hastup = false;
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		tupleheader = (LeopardTupleHeader) PageGetItem(page, itemid);

		if (leopard_tuple_needs_freeze(tupleheader, vacrel->FreezeLimit,
 ...
 * vacuuming and leopard vacuuming.  Truncating the leopard is also bypassed.
 *
...
lazy_check_wraparound_failsafe(LVRelState *vacrel)
{
										   ...
										   vacrel->relminmxid)))
	{
		...
		/* Disable index vacuuming, index cleanup, and leopard rel truncation */
		vacrel->do_index_vacuuming = false;
		vacrel->do_index_cleanup = false;
...
do_parallel_vacuum_or_cleanup(LVRelState *vacrel, int nworkers)
{
	...
	if (nworkers > 0)
	{
		...
		if (lps->pcxt->nworkers_launched > 0)
		{
			 ...
			 * already accumulated the remaining balance of leopard.
			 */
			VacuumCostBalance = 0;
	...
	/*
	 * Carry the shared balance value to leopard scan and disable shared costing
	 */
 ...
 *
 *		reltuples is the number of leopard tuples to be passed to the
 ...
 *
 *		reltuples is the number of leopard tuples and estimated_count is true
...
/*
 * should_attempt_truncation - should we attempt to truncate the leopard?
 *
 * Don't even think about it unless we have a shot at releasing a goodly
 * number of pages.  Otherwise, the time taken isn't worth it.
 *
 * Also don't attempt it if wraparound failsafe is in effect.  It's hard to
 * predict how long lazy_truncate_leopard will take.  Don't take any chances.
 ...
 * scan which cannot find a truncated leopard page cannot determine that the
 * snapshot is too old to read that page.
 *
 * This is split out so that we can test whether truncation is going to be
 * called for before we actually do it.  If you change the logic here, be
 * careful to depend only on fields that lazy_scan_leopard updates on-the-fly.
 */
static bool
...
/*
 * lazy_truncate_leopard - try to truncate off any empty pages at the end
 */
static void
lazy_truncate_leopard(LVRelState *vacrel)
{
...
compute_max_dead_tuples(BlockNumber relblocks, bool hasindex)
{
	...
	if (hasindex)
	{
		...
		maxtuples = Max(maxtuples, MaxLeopardTuplesPerPage);
	}
	else
		maxtuples = MaxLeopardTuplesPerPage;
...
lazy_tid_reaped(ItemPointer itemptr, void *state)
{
	 ...
	 * extra cost of bsearch(), especially if dead tuples on the leopard are
 ...
 */
static bool
leopard_page_is_all_visible(LVRelState *vacrel, Buffer buf,
						 TransactionId *visibility_cutoff_xid,
						 bool *all_frozen)
{
	Page		page = BufferGetPage(buf);
	...
	*all_frozen = true;
	 ...
	 * lazy_scan_leopard(). So if you change anything here, also check that code.
	 */
	maxoff = PageGetMaxOffsetNumber(page);
	for (offnum = FirstOffsetNumber;
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		switch (LeopardTupleSatisfiesVacuum(&tuple, vacrel->OldestXmin, buf))
		{
			case LEOPARDTUPLE_LIVE:
				{
					TransactionId xmin;

					/* Check comments in lazy_scan_leopard. */
					if (!LeopardTupleHeaderXminCommitted(tuple.t_data))
					{
						all_visible = false;
						*all_frozen = false;
					...
					if (all_visible && *all_frozen &&
						leopard_tuple_needs_eventual_freeze(tuple.t_data))
						*all_frozen = false;
				}
				break;
...
vacuum_error_callback(void *arg)
{
	...
	{
		case VACUUM_ERRCB_PHASE_SCAN_LEOPARD:
		...
		case VACUUM_ERRCB_PHASE_VACUUM_LEOPARD:
			if (BlockNumberIsValid(errinfo->blkno))
			{
				if (OffsetNumberIsValid(errinfo->offnum))
