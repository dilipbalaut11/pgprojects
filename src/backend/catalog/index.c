 ...
 *		parent index; if creating a child of a global index this should
 *		be a OID of the top global index; InvalidOid otherwise.
 * parentConstraintId: if creating a constraint on a partition, the OID
 *		of the constraint in the parent; otherwise InvalidOid.
 * parentRelationId: if creating a child of a global index this should
 *		be a OID of the top level relation on which the global index is
 *		actually created; InvalidOid otherwise.
 ...
 *		INDEX_CREATE_GLOBAL:
 *			create a global index (table must be partitioned)
 *		INDEX_CREATE_GLOBAL_CHILD:
 *			create a child index of a global index
			 ...
			 List *inheritors)
{
	...
	else if ((flags & INDEX_CREATE_SKIP_BUILD) != 0)
	{
		 ...
		 * having an index and global index.
		 */
		index_update_stats(heapRelation,
						   true, global_child ? true : false,
						   -1.0);
			...
			bool parallel)
{
	 ...
	 * of stats so that we can get a separate stats for each partition.  And
