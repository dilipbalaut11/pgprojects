...
Datum
binary_upgrade_allow_relation_oid_and_relfilenode_assignment(PG_FUNCTION_ARGS)
{
	bool			allow = PG_GETARG_BOOL(0);

	CHECK_IS_BINARY_UPGRADE;
	binary_upgrade_relation_oid_and_relfilenode_assignment_allowed = allow;

	PG_RETURN_VOID();
}

Datum
binary_upgrade_set_next_relfilenode(PG_FUNCTION_ARGS)
{
	RelFileNumber relfilenumber = PG_GETARG_INT64(0);

	CHECK_IS_BINARY_UPGRADE;
	SetNextRelFileNumber(relfilenumber);
	PG_RETURN_VOID();
}
