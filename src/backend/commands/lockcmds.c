							 ...
							 void *arg)
{
	...
	/*
	 * Note: Conflict log tables are deliberately NOT blocked here, even though
	 * other direct DDL on them is rejected elsewhere. pg_dump relies on being
	 * able to take an ACCESS SHARE lock on these tables to safely dump their
	 * definitions during a binary upgrade, so we permit LOCK commands on them
	 * and treat them like ordinary tables here. It's true that a strong lock
	 * (ShareLock or above) on such a table would conflict with the
	 * RowExclusiveLock taken by the apply worker's inserts and could stall
	 * conflict logging as well as the apply worker for as long as it is held.
	 * But locking a system-managed conflict log table is an unusual thing to
	 * do, and it doesn't seem worth the trouble of filtering by lock mode here.
	 */
