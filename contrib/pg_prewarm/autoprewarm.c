...
apw_dump_now(bool is_bgworker, bool dump_unlogged)
{
	...
	for (num_blocks = 0, i = 0; i < NBuffers; i++)
	{
		...
		{
			block_info_array[num_blocks].database = bufHdr->tag.rlocator.dbNode;
			block_info_array[num_blocks].tablespace = bufHdr->tag.rlocator.spcNode;
			block_info_array[num_blocks].filenode = bufHdr->tag.rlocator.relNode;
