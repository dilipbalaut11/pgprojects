...
static void DecodeLargeObjectInsert(LogicalDecodingContext *ctx, XLogRecordBuffer *buf);
...
DecodeLargeObjectInsert(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	...
	data_copy = palloc(chunk_datalen);
...
#if 0
/*
 * Helper function to decode a pg_largeobject UPDATE record into a CMD_LO_WRITE change.
 */
static void
DecodeLargeObjectUpdate(LogicalDecodingContext *ctx, XLogRecordBuffer *buf,
	xl_heap_update *xlrec, RelFileLocator target_locator)
{
	XLogReaderState *r = buf->record;
	ReorderBufferChange *change;
	Size		datalen;
	...
	/* Basic filtering - dbOid check handled in heap_decode */
	if (!(xlrec->flags & XLH_UPDATE_CONTAINS_NEW_TUPLE))
		return; /* We must have the full new image for the data chunk */

	/* 2. Extract tuple data from WAL record (new tuple is at block 0) */
	tupledata = XLogRecGetBlockData(r, 0, &datalen);
	if (datalen == 0)
		return;
