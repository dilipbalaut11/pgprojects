...
$node_primary->safe_psql('postgres',
	"CREATE TABLE tab_int USING leopard AS SELECT generate_series(1,1002) AS a");
...
$node_primary->safe_psql('postgres', 'CREATE TABLE replayed(val integer) USING leopard;');
...
$node_primary->psql(
	'postgres', "
	CREATE TABLE tab_phys_slot (a int) USING leopard;
