	...
	if (stmt->partbound)
	{
		...
		foreach(cell, idxlist)
		{
			...
			if (RelationIsPartitionGlobalIndex(idxRel))
			{
				...
				Assert(get_rel_relkind(parentindex) == RELKIND_GLOBAL_INDEX);
			}
					...
					bool run_as_table_owner)
{
	...
	foreach(cell, rels)
	{
		...
		/*
		 * Process the list of indexes and collect the Oids of the top global
		 * indexes.
		 */
		foreach_oid(indexoid, oids)
		{
			Relation	index = index_open(indexoid, ShareLock);
			Oid			idxid = InvalidOid;

			if (index->rd_rel->relkind == RELKIND_GLOBAL_INDEX)
				idxid = index->rd_index->indexrelid;
			else if (index->rd_rel->relkind == RELKIND_GLOBAL_PARTITION_INDEX)
				idxid = index->rd_index->indtopindexid;

			if (OidIsValid(idxid))
			{
				Assert(get_rel_relkind(idxid) == RELKIND_GLOBAL_INDEX);
				globalindexes = list_append_unique_oid(globalindexes, idxid);
			}
...
AttachToGlobalIndexes(List **wqueue, Relation rel, List *reloids)
{
	...
	foreach_oid(indexoid, indexoids)
	{
		...
		globalindexoids = lappend_oid(globalindexoids, RelationGetRelid(irel));
 ...
 */
static void
DropPartitionGlobalIndexes(Relation rel, List *childreloids)
{
	List	   *indexes = RelationGetIndexList(rel);

	/*
	 * All indexes under the child relation which are pointing to the parent
	 * should be dropped.
	 */
	foreach_oid(idxid, indexes)
	{
		Relation idx = index_open(idxid, AccessExclusiveLock);

		if (!RelationIsPartitionGlobalIndex(idx))
		{
...
DetachFromGlobalIndexes(List *indexoids, List *reloids)
{
	foreach_oid(indexoid, indexoids)
	{
		...
		InvalidateIndexPartitionEntries(reloids, indexoid, InvalidOid);
...
AttachPartitionEnsureIndexes(List **wqueue, Relation rel, Relation attachrel)
{
	...
	foreach(cell, idxes)
	{
		...
		if (idxRel->rd_rel->relkind != RELKIND_PARTITIONED_INDEX &&
			...
			idxRel->rd_rel->relkind != RELKIND_GLOBAL_PARTITION_INDEX)
		{
			index_close(idxRel, AccessShareLock);
			continue;
		}
		...
		for (int i = 0; i < list_length(attachRelIdxs); i++)
		{
			...
			if (!RelationIsGlobalIndexOrChild(idxRel) &&
				CompareIndexInfo(attachInfos[i], info,
								 ...
								 attmap))
			{
				...
				/* bingo. */
				if (!RelationIsGlobalIndexOrChild(attachrelIdxRels[i]))
						...
						Oid defaultPartOid)
{
	...
	foreach(cell, indexes)
	{
		...
		if (RelationIsGlobalIndexOrChild(idx))
		{
...
ATExecAttachPartitionIdx(List **wqueue, Relation parentIdx, RangeVar *name)
{
	...
	if (currParent != RelationGetRelid(parentIdx))
	{
		...
		/* All good -- do it */
		if (!RelationIsGlobalIndexOrChild(partIdx))
...
LockPartitionsForGlobalIndex(Relation rel, LOCKMODE lockmode)
{
	...
	foreach_oid(indexid, indexoidlist)
	{
		...
		if (!RelationIsGlobalIndex(idxrel) &&
			!RelationIsPartitionGlobalIndex(idxrel))
		{
			index_close(idxrel, AccessShareLock);
			continue;
		}
