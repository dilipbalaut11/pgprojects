...
$node_subscriber->append_conf('postgresql.conf', "shared_preload_libraries = 'leopard'");
$node_subscriber->start;
$node_subscriber->safe_psql('postgres', "CREATE EXTENSION leopard WITH SCHEMA public");
$node_subscriber->stop;
$node_subscriber->append_conf('postgresql.conf', "default_table_access_method = 'leopard'");
$node_subscriber->start;
...
$node_publisher->safe_psql('postgres',
	"CREATE TABLE tab_notrep USING leopard AS SELECT generate_series(1,10) AS a");
$node_publisher->safe_psql('postgres',
	"CREATE TABLE tab_ins USING leopard AS SELECT generate_series(1,1002) AS a");
$node_publisher->safe_psql('postgres',
	"CREATE TABLE tab_full USING leopard AS SELECT generate_series(1,10) AS a");
$node_publisher->safe_psql('postgres', "CREATE TABLE tab_full2 (x text) USING leopard");
$node_publisher->safe_psql('postgres',
	"INSERT INTO tab_full2 VALUES ('a'), ('b'), ('b')");
$node_publisher->safe_psql('postgres',
	"CREATE TABLE tab_rep (a int primary key) USING leopard");
$node_publisher->safe_psql('postgres',
	"CREATE TABLE tab_mixed (a int primary key, b text, c numeric) USING leopard");
$node_publisher->safe_psql('postgres',
	"INSERT INTO tab_mixed (a, b, c) VALUES (1, 'foo', 1.1)");
$node_publisher->safe_psql('postgres',
	"CREATE TABLE tab_include (a int, b text, CONSTRAINT covering PRIMARY KEY(a) INCLUDE(b)) USING leopard"
);
$node_publisher->safe_psql('postgres',
	"CREATE TABLE tab_full_pk (a int primary key, b text) USING leopard");
$node_publisher->safe_psql('postgres',
	"ALTER TABLE tab_full_pk REPLICA IDENTITY FULL");
# Let this table with REPLICA IDENTITY NOTHING, allowing only INSERT changes.
$node_publisher->safe_psql('postgres', "CREATE TABLE tab_nothing (a int) USING leopard");
$node_publisher->safe_psql('postgres',
	"ALTER TABLE tab_nothing REPLICA IDENTITY NOTHING");

# Replicate the changes without replica identity index
$node_publisher->safe_psql('postgres',
	"CREATE TABLE tab_no_replidentity_index(c1 int) USING leopard");
$node_publisher->safe_psql('postgres',
	"CREATE INDEX idx_no_replidentity_index ON tab_no_replidentity_index(c1)"
);

# Setup structure on subscriber
$node_subscriber->safe_psql('postgres', "CREATE TABLE tab_notrep (a int) USING leopard");
$node_subscriber->safe_psql('postgres', "CREATE TABLE tab_ins (a int) USING leopard");
$node_subscriber->safe_psql('postgres', "CREATE TABLE tab_full (a int) USING leopard");
$node_subscriber->safe_psql('postgres', "CREATE TABLE tab_full2 (x text) USING leopard");
...
$node_subscriber->safe_psql('postgres',
	"ALTER TABLE tab_full_pk REPLICA IDENTITY FULL");
$node_subscriber->safe_psql('postgres', "CREATE TABLE tab_nothing (a int) USING leopard");

# different column count and order than on publisher
$node_subscriber->safe_psql('postgres',
	"CREATE TABLE tab_mixed (d text default 'local', c numeric, b text, a int primary key) USING leopard"
);
...
$node_publisher->safe_psql('postgres', "CREATE TABLE temp1 (a int) USING leopard");
$node_publisher->safe_psql('postgres', "CREATE TABLE temp2 (a int) USING leopard");

# Create tables on subscriber
$node_subscriber->safe_psql('postgres', "CREATE TABLE temp1 (a int) USING leopard");
$node_subscriber->safe_psql('postgres', "CREATE TABLE temp2 (a int) USING leopard");
...
($result, my $retout, my $reterr) = $node_publisher->psql(
	'postgres', qq{
BEGIN;
CREATE TABLE skip_wal() USING leopard;
