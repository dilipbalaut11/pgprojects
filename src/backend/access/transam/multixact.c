				   ...
				   int nmembers, MultiXactMember *members)
{
	...
	lock = SimpleLruPageNoGetLock(MultiXactOffsetCtl, pageno);
	...
	*offptr = offset;
	...
	for (i = 0; i < nmembers; i++, offset++)
	{
		...
		{
			lock = SimpleLruPageNoGetLock(MultiXactMemberCtl, pageno);
			if (lock != prevlock)
			{
				if (prevlock != NULL)
...
BootStrapMultiXact(void)
{
	...
	lock = SimpleLruPageNoGetLock(MultiXactOffsetCtl , 0);
	LWLockAcquire(lock, LW_EXCLUSIVE);
	...
	lock = SimpleLruPageNoGetLock(MultiXactMemberCtl , 0);
	LWLockAcquire(lock, LW_EXCLUSIVE);
...
TrimMultiXact(void)
{
	...
	if (entryno != 0)
	{
		int			slotno;
		MultiXactOffset *offptr;
		LWLock	   *lock = SimpleLruPageNoGetLock(MultiXactOffsetCtl, pageno);
	...
	if (flagsoff != 0)
	{
		...
		int			memberoff;
		LWLock	   *lock = SimpleLruPageNoGetLock(MultiXactMemberCtl, pageno);
...
find_multixact_start(MultiXactId multi, MultiXactOffset *result)
{
	...
	offptr += entryno;
	offset = *offptr;
	LWLockRelease(SimpleLruPageNoGetLock(MultiXactOffsetCtl, pageno));
