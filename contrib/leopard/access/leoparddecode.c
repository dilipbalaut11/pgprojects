...
static void LeopardDecodeInsert(LogicalDecodingContext *ctx, XLogRecordBuffer *buf);
static void LeopardDecodeUpdate(LogicalDecodingContext *ctx, XLogRecordBuffer *buf);
static void LeopardDecodeDelete(LogicalDecodingContext *ctx, XLogRecordBuffer *buf);
static void LeopardDecodeTruncate(LogicalDecodingContext *ctx, XLogRecordBuffer *buf);
static void LeopardDecodeMultiInsert(LogicalDecodingContext *ctx, XLogRecordBuffer *buf);
static void LeopardDecodeSpecConfirm(LogicalDecodingContext *ctx, XLogRecordBuffer *buf);
...
static void LeopardDecodeXLogTuple(char *data, Size len, ReorderBufferTupleBuf *tup);
...
/*
 * Handle rmgr LEOPARD2_ID records for DecodeRecordIntoReorderBuffer().
 */
static void
DecodeHeap2Op(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	uint8		info = XLogRecGetInfo(buf->record) & XLOG_LEOPARD_OPMASK;
	...
	switch (info)
	{
		case XLOG_LEOPARD2_MULTI_INSERT:
			if (!ctx->fast_forward &&
				...
				LeopardDecodeMultiInsert(ctx, buf);
			break;
		case XLOG_LEOPARD2_NEW_CID:
			{
				xl_leopard_new_cid *xlrec;

				xlrec = (xl_leopard_new_cid *) XLogRecGetData(buf->record);
				LeopardSnapBuildProcessNewCid(builder, xid, buf->origptr, xlrec);
		...
		case XLOG_LEOPARD2_LOCK_UPDATED:
			break;
		default:
			elog(ERROR, "unexpected RM_LEOPARD2_ID record type: %u", info);
	}
}

/*
 * Handle rmgr LEOPARD_ID records for DecodeRecordIntoReorderBuffer().
 */
static void
DecodeHeapOp(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	...
	switch (info)
	{
		...
		case XLOG_LEOPARD_UPDATE:
			if (SnapBuildProcessChange(builder, xid, buf->origptr))
				LeopardDecodeUpdate(ctx, buf);
			break;

		case XLOG_LEOPARD_DELETE:
			if (SnapBuildProcessChange(builder, xid, buf->origptr))
				LeopardDecodeDelete(ctx, buf);
			break;

		case XLOG_LEOPARD_TRUNCATE:
			if (SnapBuildProcessChange(builder, xid, buf->origptr))
				LeopardDecodeTruncate(ctx, buf);
			break;
		...
		case XLOG_LEOPARD_CONFIRM:
			if (SnapBuildProcessChange(builder, xid, buf->origptr))
				LeopardDecodeSpecConfirm(ctx, buf);
			break;
		...
		default:
			elog(ERROR, "unexpected RM_LEOPARD_ID record type: %u", info);
			break;
	}
}
...
/*
 * Parse XLOG_LEOPARD_INSERT (not MULTI_INSERT!) records into tuplebufs.
 *
 * Deletes can contain the new tuple.
 */
static void
LeopardDecodeInsert(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	Size		datalen;
	char	   *tupledata;
	Size		tuplelen;
	...
	xlrec = (xl_leopard_insert *) XLogRecGetData(r);

	/*
	 * Ignore insert records without new tuples (this does happen when
	 * raw_leopard_insert marks the TOAST record as LEOPARD_INSERT_NO_LOGICAL).
	 */
	if (!(xlrec->flags & XLLEOPARD_INSERT_CONTAINS_NEW_TUPLE))
		return;
	...
	if (!(xlrec->flags & XLLEOPARD_INSERT_IS_SPECULATIVE))
	...
	tuplelen = datalen - SizeOfLeopardHeader;

	change->data.tp.newtuple =
		ReorderBufferGetTupleBuf(ctx->reorder, tuplelen);

	LeopardDecodeXLogTuple(tupledata, datalen, change->data.tp.newtuple);
...
/*
 * Parse XLOG_LEOPARD_UPDATE and XLOG_LEOPARD_HOT_UPDATE, which have the same layout
 * in the record, from wal into proper tuplebufs.
 *
 * Updates can possibly contain a new tuple and the old primary key.
 */
static void
LeopardDecodeUpdate(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	XLogReaderState *r = buf->record;
	...
	xlrec = (xl_leopard_update *) XLogRecGetData(r);
	...
	if (xlrec->flags & XLLEOPARD_UPDATE_CONTAINS_NEW_TUPLE)
	{
		Size		datalen;
		Size		tuplelen;
		...
		LeopardDecodeXLogTuple(data, datalen, change->data.tp.newtuple);
	}
	...
	if (xlrec->flags & XLH_UPDATE_CONTAINS_OLD)
	{
		...
		data = XLogRecGetData(r) + SizeOfLeopardUpdate;
		datalen = XLogRecGetDataLen(r) - SizeOfLeopardUpdate;
		tuplelen = datalen - SizeOfLeopardHeader;

		change->data.tp.oldtuple =
			ReorderBufferGetTupleBuf(ctx->reorder, tuplelen);

		LeopardDecodeXLogTuple(data, datalen, change->data.tp.oldtuple);
	}
...
/*
 * Parse XLOG_LEOPARD_DELETE from wal into proper tuplebufs.
 *
 * Deletes can possibly contain the old primary key.
 */
static void
LeopardDecodeDelete(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	XLogReaderState *r = buf->record;
	...
	xlrec = (xl_leopard_delete *) XLogRecGetData(r);
	...
	{
		Size		datalen = XLogRecGetDataLen(r) - SizeOfLeopardDelete;
		Size		tuplelen = datalen - SizeOfLeopardHeader;

		Assert(XLogRecGetDataLen(r) > (SizeOfLeopardDelete + SizeOfLeopardHeader));

		change->data.tp.oldtuple =
			ReorderBufferGetTupleBuf(ctx->reorder, tuplelen);

		LeopardDecodeXLogTuple((char *) xlrec + SizeOfLeopardDelete,
...
/*
 * Parse XLOG_LEOPARD_TRUNCATE from wal
 */
static void
LeopardDecodeTruncate(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	XLogReaderState *r = buf->record;
	xl_leopard_truncate *xlrec;
	ReorderBufferChange *change;

	xlrec = (xl_leopard_truncate *) XLogRecGetData(r);
...
/*
 * Decode XLOG_LEOPARD2_MULTI_INSERT_insert record into multiple tuplebufs.
 *
 * Currently MULTI_INSERT will always contain the full tuples.
 */
static void
LeopardDecodeMultiInsert(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	XLogReaderState *r = buf->record;
	...
	xlrec = (xl_leopard_multi_insert *) XLogRecGetData(r);
	...
	for (i = 0; i < xlrec->ntuples; i++)
	{
		...
		tuple->tuple.t_len = datalen + SizeofLeopardTupleHeader;

		memset(header, 0, SizeofLeopardTupleHeader);

		memcpy((char *) tuple->tuple.t_data + SizeofLeopardTupleHeader,
			   (char *) data,
			   datalen);
		 ...
		 * xl_multi_insert_tuple record emitted by one leopard_multi_insert()
		 * call.
		 */
		if (xlrec->flags & XLLEOPARD_INSERT_LAST_IN_MULTI &&
			(i + 1) == xlrec->ntuples)
			change->data.tp.clear_toast_afterwards = true;
		else
...
/*
 * Parse XLOG_LEOPARD_CONFIRM from wal into a confirmation change.
 *
 * This is pretty trivial, all the state essentially already setup by the
 * speculative insertion.
 */
static void
LeopardDecodeSpecConfirm(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	XLogReaderState *r = buf->record;
	ReorderBufferChange *change;
	RelFileNode target_node;
...
/*
 * Read a LeopardTuple as WAL logged by leopard_insert, leopard_update and leopard_delete
 * (but not by leopard_multi_insert) into a tuplebuf.
 *
 * The size 'len' and the pointer 'data' in the record need to be
 * computed outside as they are record specific.
 */
static void
LeopardDecodeXLogTuple(char *data, Size len, ReorderBufferTupleBuf *tuple)
{
	xl_leopard_header xlhdr;
	int			datalen = len - SizeOfLeopardHeader;
	LeopardTupleHeader header;
